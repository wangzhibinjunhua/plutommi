...
void mmi_data_unit_test(void)
{
    ...
    /*----------------------------------------------------------------*/
    /* test init file */
    result = mmi_data_init_record_store(&unit_test_record_store, (CHAR*) L"D:\\db_file.dat", 128);
    /*
     * // add records
     * mmi_ucs2cpy( (CHAR*)data, (CHAR*)L"data record store:(1) add a record done!" );
     * result = mmi_data_add_record( &unit_test_record_store, &record_id, data );
     * mmi_ucs2cpy( (CHAR*)data, (CHAR*)L"data record store:(2) add a record done!" );
     * result = mmi_data_add_record( &unit_test_record_store, &record_id, data );
     * mmi_ucs2cpy( (CHAR*)data, (CHAR*)L"data record store:(3) add a record done!" );
     * result = mmi_data_add_record( &unit_test_record_store, &record_id, data );
     * mmi_ucs2cpy( (CHAR*)data, (CHAR*)L"data record store:(4) add a record done!" );
     * result = mmi_data_add_record( &unit_test_record_store, &record_id, data );
     * 
     * // get records
     * memset( data, 0, 128 );
    ...
    /* set records */
    record_id = 0;
    mmi_ucs2cpy((CHAR*) data, (CHAR*) L"data record store:(01) set a record done!");
    result = mmi_data_set_record(&unit_test_record_store, record_id, data, TRUE);
    record_id = 1;
    mmi_ucs2cpy((CHAR*) data, (CHAR*) L"data record store:(02) set a record done!");
    result = mmi_data_set_record(&unit_test_record_store, record_id, data, TRUE);
    record_id = 2;
    mmi_ucs2cpy((CHAR*) data, (CHAR*) L"data record store:(03) set a record done!");
    result = mmi_data_set_record(&unit_test_record_store, record_id, data, TRUE);
    record_id = 3;
    mmi_ucs2cpy((CHAR*) data, (CHAR*) L"data record store:(04) set a record done!");
    result = mmi_data_set_record(&unit_test_record_store, record_id, data, TRUE);
...
static S32 mmi_data_scan_db_file(mmi_data_record_store_type *context)
{
    ...
    if ((fd = FS_Open(context->db_file, FS_CREATE | FS_ATTR_HIDDEN)) >= 0)
    {
        ...
        FS_Close(fd);
        MMI_TRACE(SRV_PHB_TRACE_GROUP, TRC_SRV_PHB_DATA_MGR_SCAN_DB_FILE_FINISH, context->db_file[0], context->record_num, context->max_record_num,
                    MAX_RECORD_SIZE, MAX_RECORD_NUM);
        return 0;
    }
    else
    {
        ...
        context->is_active = 0;
        MMI_TRACE(SRV_PHB_TRACE_GROUP, TRC_SRV_PHB_DATA_MGR_SCAN_DB_FILE_FAIL, context->db_file[0], fd, MAX_RECORD_SIZE, MAX_RECORD_NUM);
        return (S32) fd;
    }
}
 ...
 *  S32
 *****************************************************************************/
S32 mmi_data_init_record_store(mmi_data_record_store_type *context, CHAR *db_file, S32 record_size)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    mmi_ucs2ncpy((CHAR*) context->db_file, (CHAR*) db_file, SRV_FMGR_PATH_MAX_LEN);
    context->db_file[SRV_FMGR_PATH_MAX_LEN] = 0;    /* make sure it is null terminated even if the source db_file is longer than FMGR_MAX_PATH_LEN */
...
S32 mmi_data_add_record(mmi_data_record_store_type *context, U16 *record_id, void *data)
{
    ...
    {
        /* can't open the db file, something wrong with it, scan db file try to recover error */
        if ((result = mmi_data_scan_db_file(context)) != 0)
        {
            return result;
        }
...
S32 mmi_data_check_space_for_set_record(mmi_data_record_store_type *context, U16 record_id)
{
    ...
    mmi_ucs2ncpy((CHAR*) dir, (const CHAR*)context->db_file, 3);     /* copy "X:\" */
