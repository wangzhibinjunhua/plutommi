...
extern void srv_btsco_open_stream_callback(U8 profile, mdi_result result, U16 error_cause);
extern void srv_btsco_close_stream_callback(U8 profile, mdi_result result, U16 error_cause);
#endif /* __MMI_BT_AUDIO_SUPPORT__ */
...
void mdi_audio_bt_close_stream_ind(void *msg)
{
    ...
    if (state == MDI_AUDIO_BT_STATE_OPEN || state == MDI_AUDIO_BT_STATE_WAIT_CLOSE)
    {
        ...
        if (handler != NULL)
        {
            handler(BT_HFP, mdi_audio_get_event(result), 0);
        }
    }
}
...
void mdi_audio_bt_open_stream_ind(void *msg)
{
    ...
    if (state == MDI_AUDIO_BT_STATE_IDLE || state == MDI_AUDIO_BT_STATE_WAIT_OPEN)
    {
        ...
        if (handler != NULL)
        {
            handler(BT_HFP, MDI_AUDIO_SUCCESS, 0);
        }
    }
}
...
void mdi_audio_bt_open_stream_cnf(void *msg)
{
    ...
    if (state == MDI_AUDIO_BT_STATE_WAIT_OPEN)
    {
        ...
        if (handler != NULL)
        {
            handler(BT_HFP, mdi_audio_get_event(result), msg_p->error_cause);
        }
    }
}
...
MMI_BOOL mdi_audio_bt_close_stream(U8 profile)
{
    ...
    {
        mdi_audio_bt_cntx.pending_connect = MMI_FALSE;
        mdi_audio_bt_cntx.open_hdlr( profile, MDI_AUDIO_FAIL, 0);
    }
...
void mdi_fmr_power_on_with_path(U8 path, mdi_ext_callback handler, void *user_data)
{
    ...
    if ((! mdi_audio_is_idle()) 
     #ifdef __BT_DIALER_SUPPORT__
      || (srv_btsco_is_sco_open() && srv_bt_cm_is_profile_connected(SRV_BT_CM_HSP_CONNECTION))
     #endif /* __BT_DIALER_SUPPORT__ */
      || (interrupt_cb_handler != NULL))
    {
        mdi_audio_stop_all();
