...
#ifdef __MMI_CLOG_ATCMD_SUPPORT__
    srv_clog_at_notify_update(SRV_CLOG_AT_UPDATE_UPD, 
            cntx, 
            SRV_CLOG_SI_GET_P(cur_si_log), 
            (srv_clog_log_identity_struct*)(&(req_data->log_data.data.call_log)),
            cur_si_log);
#endif
...
#ifdef __MMI_CLOG_ATCMD_SUPPORT__
    srv_clog_pi_p2i(SRV_CLOG_SI_GET_P(comp->si_tbl->data[sit_idx]), &idt);
#endif
...
#ifdef __MMI_CLOG_ATCMD_SUPPORT__
    srv_clog_at_notify_update(SRV_CLOG_AT_UPDATE_DEL, cntx, job->init_value, &idt, NULL);
#endif
