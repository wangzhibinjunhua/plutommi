...
static S32 srv_clog_si_log_si2cmn_call(srv_clog_si_struct *sort_info, U32 fields, srv_clog_log_struct *log)
{
    ...
    if (SRV_CLOG_CHECK_BIT(fields, SRV_CLOG_LOG_FIELD_NAME) || SRV_CLOG_CHECK_BIT(fields, SRV_CLOG_LOG_FIELD_PHB_ID))
    {
        ...
        else
        {
            ...
            if (SRV_CLOG_CLT_IDX_DIALED == 
                SRV_CLOG_PI_P2IDX(SRV_CLOG_SI_GET_P(sort_info), SRV_CLOG_LSP_CLT, SRV_CLOG_LSP_LT_POS))
            {
                ...
                ret = srv_sms_get_mailbox_name(
                        (srv_sms_sim_enum)(MMI_GSM|SRV_CLOG_IDX2BIT(SRV_CLOG_PI_P2IDX(SRV_CLOG_SI_GET_P(sort_info), SRV_CLOG_LSP_SIM_ID, SRV_CLOG_LSP_SIM_ID_POS))), 
                        (PU16)log_num, 
                        (PU16)mailbox_name);

                if (ret && mmi_ucs2strlen((CHAR *)mailbox_name))
                {
                    mmi_ucs2ncpy((CHAR *)call_log->name,
                        (CHAR *)mailbox_name, (U32)SRV_CLOG_NAME_MAX_LEN);
                }
            }
        }
...
S32 srv_clog_si_log_si2cmn(srv_clog_si_struct *sort_info, U32 fields, srv_clog_log_struct *log)
{
    ...
    if ( SRV_CLOG_CHECK_BIT(fields, SRV_CLOG_LOG_FIELD_PHB_ID) )
    {
        ...
        ret = srv_phb_get_number_by_number_id(phb_num, SRV_CLOG_SILOG_GET_PHB_ID(sort_info), SRV_CLOG_CID_MAX_LEN);
        ...
        if(!ret)
        {
            ...
            srv_clog_pi_p2idx(SRV_CLOG_SILOG_GET_TYPE_ID(sort_info), &cate_idx, &sim_idx, &lt_idx, &st_idx);
            mmi_asc_n_to_ucs2((CHAR *)log_num, call_number, SRV_CLOG_CID_MAX_LEN);

            name_len = SRV_CLOG_NAME_MAX_LEN>SRV_SMS_MAX_PROFILE_NAME_LEN?SRV_SMS_MAX_PROFILE_NAME_LEN:SRV_CLOG_NAME_MAX_LEN;
                
            if (SRV_CLOG_TSP_CLT_DIALED & SRV_CLOG_SILOG_GET_TYPE_ID(sort_info))
            {
                U8 mailbox_name[(SRV_SMS_MAX_PROFILE_NAME_LEN + 1) * ENCODING_LENGTH];
                ...
                memset(mailbox_name, 0, SRV_SMS_MAX_PROFILE_NAME_LEN * ENCODING_LENGTH);
                ret = srv_sms_get_mailbox_name(
                    (srv_sms_sim_enum)(MMI_GSM|sim_idx), 
                    (PU16)log_num, 
                    (PU16)mailbox_name);
                if (ret && mmi_ucs2strlen((CHAR *)mailbox_name))
                {
