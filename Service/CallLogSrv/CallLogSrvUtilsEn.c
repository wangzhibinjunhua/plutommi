...
void srv_clog_dump(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (!srv_clog_cntx_p || !SRV_CLOG_CHECK_BIT(srv_clog_get_debug_mode(), SRV_CLOG_DEBUG_MODE_CNTX_DUMP))
        return;
    ...
    MMI_TRACE(
        ...
        srv_clog_cntx_p->state,
        srv_clog_cntx_p->max_log_id,
        srv_clog_cntx_p->max_pid,
        srv_clog_cntx_p->flags);
    SRV_CLOG_TRC((SRV_CLOG_TS
                  (SRV_CLOG_TRC_INFO, TRC_SRV_CLOG_DUMP_CNTX_BASE_INFO,
                   "[CLogSrv]context base_info(state=%d,max_log_id=%d,max_pid=%d,flags=0x%x)"), srv_clog_cntx_p->state,
                  srv_clog_cntx_p->max_log_id, srv_clog_cntx_p->max_pid, srv_clog_cntx_p->flags));
...
void srv_clog_stl_dump(void)
{
    ...
    if (!SRV_CLOG_CHECK_BIT(srv_clog_get_debug_mode(), SRV_CLOG_DEBUG_MODE_CNTX_DUMP))
        return;
    if (!srv_clog_cntx_p || !srv_clog_cntx_p->comp || !srv_clog_cntx_p->comp->style_mgr)
        return;
    mgr = srv_clog_cntx_p->comp->style_mgr;
...
void srv_clog_st_dump(void)
{
    ...
    if (!srv_clog_cntx_p || !srv_clog_cntx_p->comp)
        return;
...
void srv_clog_si_dump(void)
{
    ...
    if (!SRV_CLOG_CHECK_BIT(srv_clog_get_debug_mode(), SRV_CLOG_DEBUG_MODE_CNTX_DUMP))
        return;
    if (!srv_clog_cntx_p || !srv_clog_cntx_p->comp || !srv_clog_cntx_p->comp->si_tbl)
        return;
...
void srv_clog_si_dump_log_by_idx(U32 index)
{
    ...
    si_log = srv_clog_si_tbl_index(si_tbl, index);
...
void srv_clog_si_dump_log(U32 index, void *log_data)
{
    ...
    local_sec = applib_dt_sec_utc_to_local(si_log->timestamp[SRV_CLOG_LASTEST_TIME_IDX]);
    applib_dt_utc_sec_2_mytime(local_sec, &app_time, MMI_FALSE);

    srv_clog_pi_p2idx(SRV_CLOG_SILOG_GET_TYPE_ID(si_log), &cate_idx, &sim_idx, &lt_idx, &st_idx);
    ...
    MMI_TRACE(
        ...
        app_time.nMin,
        app_time.nSec,
        SRV_CLOG_SILOG_GET_UNREADNUM(si_log),
        SRV_CLOG_SILOG_GET_TOTALNUM(si_log));

    SRV_CLOG_TRC((SRV_CLOG_TS
                  (SRV_CLOG_TRC_INFO, TRC_SRV_CLOG_SI_DUMP_LOG_VALUE,
                   "[CLogSrv]%d: id=%d,p=0x%x(%d:%d:%d:%d),pid=0x%x(%d:%d:%d), %d-%d-%d %d:%d:%d, num=%d/%d"),
                  index, SRV_CLOG_SILOG_GET_LOG_ID(si_log), SRV_CLOG_SILOG_GET_TYPE_ID(si_log), cate_idx, sim_idx, lt_idx, st_idx,
                  ...
                  SRV_CLOG_SILOG_GET_UNREADNUM(si_log), SRV_CLOG_SILOG_GET_TOTALNUM(si_log)));

    srv_clog_si_log_get_cid(si_log, cid, SRV_CLOG_CID_MAX_LEN);
    ...
    SRV_CLOG_TRC((SRV_CLOG_TS
                 ...
                 "[CLogSrv]srv_clog_si_dump_log---> %d: cid=%s,name=%s"), index, cid,
                 NULL));
}
...
void srv_clog_idx_dump(void)
{
    ...
    if (!srv_clog_cntx_p || !srv_clog_cntx_p->comp || !srv_clog_cntx_p->comp->idx_tbl)
        return;
    idx_tbl = srv_clog_cntx_p->comp->idx_tbl;
 ...
 *  void
 *****************************************************************************/
void srv_clog_pi_p2idx(U8 types, U32 *cate_idx, U32 *sim_idx, U32 *lt_idx, U32 *st_idx)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    *cate_idx = SRV_CLOG_CATE_CALL;
    *sim_idx = SRV_CLOG_IDX2BIT(SRV_CLOG_PI_P2IDX(types, SRV_CLOG_TSP_SIM_ID_GRP, SRV_CLOG_TSP_SIM_ID_POS));
    *lt_idx = SRV_CLOG_PI_P2IDX(types, SRV_CLOG_TSP_CLT_GRP, SRV_CLOG_TSP_CLT_POS);;
    *st_idx = SRV_CLOG_CALL_TYPE_VOICE;
}
 ...
 *  void
 *****************************************************************************/
void srv_clog_pi_p2i(U8 types, srv_clog_log_identity_struct *identity)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    identity->sim_id = SRV_CLOG_IDX2BIT(SRV_CLOG_PI_P2IDX(types, SRV_CLOG_TSP_SIM_ID_GRP, SRV_CLOG_TSP_SIM_ID_POS));
    identity->log_type = SRV_CLOG_PI_P2IDX(types, SRV_CLOG_TSP_CLT_GRP, SRV_CLOG_TSP_CLT_POS);
    identity->sub_type = SRV_CLOG_CALL_TYPE_VOICE;
}
 ...
 *  void
 *****************************************************************************/
void srv_clog_pi_i2p(srv_clog_log_identity_struct *identity, U8 *types)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    SRV_CLOG_PI_IDX2P(*types, (identity->log_type), SRV_CLOG_TSP_CLT_POS);
    
    SRV_CLOG_BIT2IDX(identity->sim_id, idx);
    SRV_CLOG_PI_IDX2P(*types, idx, SRV_CLOG_TSP_SIM_ID_POS);
}
 ...
 *  
 *****************************************************************************/
S32 srv_clog_pi_is_ip_sub(srv_clog_log_identity_struct *idt, U8 types)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  
 *****************************************************************************/
U32 srv_clog_pi_get_num_limit_by_p(U32 num_style, U8 types, srv_clog_log_identity_struct *type_idt)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    srv_clog_pi_p2i(types, type_idt);
 ...
 *  
 *****************************************************************************/
U32 srv_clog_wcs_n_to_BCD(CHAR *pOutBuffer, WCHAR *pInBuffer, U32 len)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    kal_uint8 tmp_cid[SRV_CLOG_CID_MAX_LEN + 1];
    
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    mmi_wcs_n_to_asc((CHAR*) tmp_cid, pInBuffer, len);
    return srv_phb_convert_to_bcd((U8 *)pOutBuffer, (U8 *)tmp_cid,  NVRAM_CLOG_CID_BCD_MAX_LEN);  
}
