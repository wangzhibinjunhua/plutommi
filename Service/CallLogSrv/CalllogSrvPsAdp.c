...
void srv_clog_log_si2l4c(const srv_clog_si_struct *si_log, l4c_phb_ln_entry_struct *ln)
{
    ...
    srv_clog_cid_clog2l4c((const CHAR*)si_log->cid, SRV_CLOG_CALL_TYPE_IDX_VOICE, ln);

    {
        ...
        srv_phb_get_name_by_number_id(ucs_name, si_log->phb_id, SRV_CLOG_NAME_MAX_LEN);

#else /* __MMI_CLOG_PHB_SYNC_SUPPORT__ */

        WCHAR ucs_cid[SRV_CLOG_CID_MAX_LEN + 1];

        mmi_asc_n_to_wcs(ucs_cid,  (CHAR*)si_log->cid, SRV_CLOG_CID_MAX_LEN);
        srv_phb_get_name_by_number(ucs_cid,ucs_name, SRV_CLOG_NAME_MAX_LEN);
    ...
    srv_clog_time_utc2rtc(si_log->timestamp[SRV_CLOG_LASTEST_TIME_IDX], &ln->time);

    ln->count = si_log->total_num;
    if (type_id & SRV_CLOG_TSP_CLT_MISSED)
    {
        unread_num = SRV_CLOG_SILOG_GET_UNREADNUM(si_log);
        ln->call_status = (unread_num == 0 ? 0 : 1);
    }
    else
        ln->call_status = 0;

    ln->call_duration = si_log->duration[SRV_CLOG_LASTEST_TIME_IDX];
}
...
#else /* __MMI_CLOG_SLIM_FOR_MT6261__ */
    cmn_log->cate = SRV_CLOG_CATE_IDX_CALL;
#endif /* __MMI_CLOG_SLIM_FOR_MT6261__ */
