...
static S32 srv_clog_phb_is_sync_sup(srv_clog_cntx_struct *cntx, U16 evt_id);
...
#ifndef __MMI_CLOG_SLIM_FOR_MT6261__
static S32 srv_clog_phb_submit_refresh_job(srv_clog_cntx_struct *cntx);
#endif /* __MMI_CLOG_SLIM_FOR_MT6261__ */
...
#ifndef __MMI_CLOG_SLIM_FOR_MT6261__
static void srv_clog_phb_handle_add(srv_clog_cntx_struct *cntx, srv_clog_phb_para_struct *para);
static void srv_clog_phb_handle_delete(srv_clog_cntx_struct *cntx, srv_clog_phb_para_struct *para);
static void srv_clog_phb_handle_update(srv_clog_cntx_struct *cntx, srv_clog_phb_para_struct *para);
#else /* __MMI_CLOG_SLIM_FOR_MT6261__ */
...
#ifndef __MMI_CLOG_SLIM_FOR_MT6261__
static void srv_clog_phb_update_name(srv_clog_phb_para_struct *para, srv_clog_si_struct *si_log);
static void srv_clog_phb_update_match(
                ...
                SRV_PHB_ID phb_id,
                WCHAR *name);
static void srv_clog_phb_notify(srv_clog_phb_para_struct *para, U32 fields, srv_clog_si_struct *si_log);
#else /* __MMI_CLOG_SLIM_FOR_MT6261__ */
...
void srv_clog_phb_dump(void)
{
    ...
    if (!SRV_CLOG_CHECK_BIT(srv_clog_get_debug_mode(), SRV_CLOG_DEBUG_MODE_PHB_PARA_DUMP))
        return;
#ifndef __MMI_CLOG_SLIM_FOR_MT6261__
    if (!cntx || !srv_clog_is_init() || !g_srv_clog_phb_para || !g_srv_clog_phb_para->evt)
#else /* __MMI_CLOG_SLIM_FOR_MT6261__ */
    ...
    SRV_CLOG_DUMP_LOG(
        g_srv_clog_phb_para->si_tbl_idx,
        srv_clog_si_index(cntx->comp->si_tbl, g_srv_clog_phb_para->si_tbl_idx));
#else /* __MMI_CLOG_SLIM_FOR_MT6261__ */
    SRV_CLOG_DUMP_LOG(
        g_srv_clog_phb_para->si_tbl_idx,
...
MMI_RET srv_clog_phb_sync_evt_hdlr(mmi_event_struct *evt)
{
    ...
    if (!srv_clog_phb_is_sync_sup(cntx, evt->evt_id))
        return MMI_RET_OK;

    if (!srv_clog_is_ready())
    {
        srv_clog_phb_submit_phb_evt_job(phb_evt);
        return MMI_RET_OK;
    }
...
static S32 srv_clog_phb_is_sync_sup(srv_clog_cntx_struct *cntx, U16 evt_id)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    srv_clog_stl_get_style(cntx->comp->style_mgr, SRV_CLOG_STYLE_TYPE_PHB_SYNC, &sync_style, NULL);
    if (!sync_style)
        return MMI_FALSE;
...
static void srv_clog_phb_handle_evt(srv_phb_op_evt_struct *evt)
{
    ...
    {
        case EVT_ID_PHB_ADD_CONTACT:
            srv_clog_phb_handle_add(cntx, &para);
            break;

        case EVT_ID_PHB_DEL_CONTACT:
            srv_clog_phb_handle_delete(cntx, &para);
            break;

        case EVT_ID_PHB_UPD_CONTACT:
            srv_clog_phb_handle_update(cntx, &para);
            break;
    ...
    srv_clog_si_flash(cntx->comp->si_tbl, &start_sit_idx, &saved_num, &failed_num);
    if (start_sit_idx == SRV_CLOG_LOG_MAX_NUM && saved_num == (U32) para.dirty_num)
    {
    #ifdef __SRV_CLOG_DUMP__
        srv_clog_phb_set_dump_value(NULL);
    #endif 
    ...
    srv_clog_phb_submit_refresh_job(cntx);
...
S32 srv_clog_phb_job_hdlr(srv_clog_cntx_struct *cntx, srv_clog_job_struct *job)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (job->job_state == SRV_CLOG_JOB_STATE_INIT || job->job_state == SRV_CLOG_JOB_STATE_READY)
    {
    ...
    if (job->proc_state == SRV_CLOG_JOB_UPD_PHB_STATE_INIT)
    {
        job->cursor = 0;
        job->proc_state = SRV_CLOG_JOB_UPD_PHB_STATE_UPDATING;
    }
    if (job->proc_state == SRV_CLOG_JOB_UPD_PHB_STATE_UPDATING)
    {
...
/*****************************************************************************
 * FUNCTION
 *  srv_clog_phb_submit_refresh_job
 * DESCRIPTION
 *  
 * PARAMETERS
 *  cntx        [?]     
...
static S32 srv_clog_phb_submit_refresh_job(srv_clog_cntx_struct *cntx)
{
    ...
    comp = srv_clog_get_cur_comp(cntx, 0);
    job = srv_clog_job_submit_tail(comp->job_mgr, SRV_CLOG_JOB_TYPE_UPDATE_PHB_EVT);
    if (!job)
        return SRV_CLOG_RET_QUEUE_FULL;
    job->job_state = SRV_CLOG_JOB_STATE_READY;
    job->proc_state = SRV_CLOG_JOB_UPD_PHB_STATE_INIT;
    job->handle = 0;

    srv_clog_process_new_job(cntx, comp->job_mgr, job);
...
static void srv_clog_phb_handle_add(srv_clog_cntx_struct *cntx, srv_clog_phb_para_struct *para)
{
    ...
    total_num = srv_clog_si_total_num(si_tbl);
    ...
    for (i = 0, count = 0; i < SRV_CLOG_LOG_MAX_NUM && count < total_num; i++)
    {
        ...
        cur_asc_cid = SRV_CLOG_SI_GET_CID(si_log);
        if (SRV_CLOG_IS_STR_EMPTY(cur_asc_cid))
            continue;

        mmi_asc_n_to_ucs2((CHAR*) cur_w_cid, cur_asc_cid, SRV_CLOG_CID_MAX_LEN);
        for (j = 0; j < para->max_cid_num; j++)
        {
            ...
            srv_clog_phb_update_match(
                para,
                si_log,
                SRV_CLOG_LOG_FIELD_PHB_ID | SRV_CLOG_LOG_FIELD_NAME,
                num_tbl[j].phb_id,
                para->name);
        }
    }
...
static void srv_clog_phb_handle_delete(srv_clog_cntx_struct *cntx, srv_clog_phb_para_struct *para)
{
    ...
    SRV_PHB_ID phb_id;
    U16 store_index;
    srv_clog_si_tbl_struct *si_tbl = cntx->comp->si_tbl;
    ...
    for (i = 0, count = 0; i < SRV_CLOG_LOG_MAX_NUM && count < total_num; i++)
    {
        ...
        MMI_TRACE(SRV_CLOG_TRC_INFO, TRC_SRV_CLOG_PHB_HANDLE_DELETE, i, phb_id, SRV_CLOG_SI_GET_P(si_log));
...
static void srv_clog_phb_handle_update(srv_clog_cntx_struct *cntx, srv_clog_phb_para_struct *para)
{
    ...
    for (i = 0, j = 0; i < SRV_CLOG_LOG_MAX_NUM && j < total_num; i++)
    {
        ...
        if (!srv_clog_si_is_valid(si_log))
            continue;
        j++;
        if (SRV_CLOG_IS_STR_EMPTY(SRV_CLOG_SI_GET_CID(si_log)))
            continue;
        ...
        if (para->changed_cid_num == 0 && !para->name_changed)
            break;
    }
}
...
static void srv_clog_phb_update_free_log(srv_clog_phb_para_struct *para, srv_clog_si_struct *si_log)
{
     ...
     * but the store index is valid and still can still tell use which store_index is deleted
     */
    if (SRV_CLOG_PHB_ID2IDX(SRV_CLOG_SI_GET_PHB_ID(si_log)) == para->evt->store_index)
    {
        srv_clog_phb_update_match(
            para,
            si_log,
    ...
    mmi_asc_n_to_wcs(w_cid, SRV_CLOG_SI_GET_CID(si_log), SRV_CLOG_CID_MAX_LEN);
...
static void srv_clog_phb_update_owned_log(srv_clog_phb_para_struct *para, srv_clog_si_struct *si_log)
{
    ...
    /* check if name changed if not check name when matching some previous logs */
    if (!para->check_name)
    {
        name = SRV_CLOG_SI_GET_NAME(si_log);
        if (SRV_CLOG_IS_STR_EMPTY(para->name) && SRV_CLOG_IS_STR_EMPTY(name) ||
            !SRV_CLOG_IS_STR_EMPTY(para->name) && !SRV_CLOG_IS_STR_EMPTY(name) &&
            0 == mmi_wcscmp((const WCHAR*)para->name, (const WCHAR*)name))
        {
            para->name_changed = MMI_FALSE;
        }
        para->check_name = MMI_TRUE;
    }

    for (i = 0; i < para->max_cid_num; i++)
    {
        /* Check if phb_id is matched */
        if (!SRV_CLOG_IS_SAME_PHB_ID(SRV_CLOG_SI_GET_PHB_ID(si_log), para->num_tbl[i].phb_id))
            continue;
        ...
        if (!para->num_tbl[i].cid_changed)
        {
            ...
            srv_clog_phb_update_name(para, si_log);
            break;
        }
        ...
        for (j = 0; j < para->max_cid_num; j++)
        {
            ...
            if (para->name_changed)
                fields = SRV_CLOG_LOG_FIELD_PHB_ID | SRV_CLOG_LOG_FIELD_NAME;
            else
                fields = SRV_CLOG_LOG_FIELD_PHB_ID;
            ...
            srv_clog_phb_update_match(para, si_log, fields, para->num_tbl[j].phb_id, para->name);
            break;
        }
        /* Not find, clear both the link and the name. */
        if (j == para->max_cid_num)
        {
            srv_clog_phb_update_match(
                para,
                si_log,
        ...
        /* already find matched phb_id and handle it, it's done now, try to handle next si_log in log list */
        break;
    }
    /* Not find any matched phb_id in log but store index matched */
    if (i == para->max_cid_num)
    {
        srv_clog_phb_update_name(para, si_log);
    }
}
...
static void srv_clog_phb_update_name(srv_clog_phb_para_struct *para, srv_clog_si_struct *si_log)
{
    ...
    srv_clog_phb_update_match(para, si_log, SRV_CLOG_LOG_FIELD_NAME, para->invalid_phb_id, para->name);
}
                ...
                WCHAR *name)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    srv_clog_si_update_phb_info(si_log, fields, phb_id, name, NULL);
    SRV_CLOG_SET_BIT(SRV_CLOG_SI_GET_P(si_log), SRV_CLOG_LDP_DIRTY);
    para->dirty_num++;
    /* If change phb_id only, no need notify ATCmd */
    if (fields != SRV_CLOG_LOG_FIELD_PHB_ID)
       srv_clog_phb_notify(para, fields, si_log);
}
...
static void srv_clog_phb_notify(srv_clog_phb_para_struct *para, U32 fields, srv_clog_si_struct *si_log)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    srv_clog_emit_update_log_event(SRV_CLOG_INVALID_ELM, fields, si_log);

#ifdef __MMI_CLOG_ATCMD_SUPPORT__
    srv_clog_pi_p2i(SRV_CLOG_SI_GET_P(si_log), &idt);
    srv_clog_at_notify_update(SRV_CLOG_AT_UPDATE_UPD, cntx, SRV_CLOG_SI_GET_P(si_log), &idt, si_log);
#endif
}
...
#ifdef __MMI_CLOG_ATCMD_SUPPORT__
    srv_clog_pi_p2i(SRV_CLOG_SILOG_GET_TYPE_ID(si_log), &idt);
    srv_clog_at_notify_update(SRV_CLOG_AT_UPDATE_UPD, srv_clog_cntx_p, SRV_CLOG_SILOG_GET_TYPE_ID(si_log), &idt, si_log);
#endif
}
