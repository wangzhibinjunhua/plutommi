...
*/
static S32 srv_clog_idx_sort_by_log_id(void *src, void *dst);
...
static S32 srv_clog_idx_check_merge(srv_clog_idx_tbl_struct *idx_tbl, srv_clog_add_log_req_struct *req_data);
static srv_clog_full_enum srv_clog_idx_check_full(
                            srv_clog_idx_tbl_struct *idx_tbl,
                            srv_clog_si_struct *si_log,
                            srv_clog_log_identity_struct *type_idt);
static S32 srv_clog_idx_find_oldest(srv_clog_idx_tbl_struct *idx_tbl, srv_clog_add_log_req_struct *req_data);
static S32 srv_clog_idx_base_get_oldest(srv_clog_idx_tbl_struct *idx_tbl, srv_clog_add_log_req_struct *req_data);
static S32 srv_clog_idx_base_get_oldest_by_idt(srv_clog_idx_tbl_struct *idx_tbl, srv_clog_add_log_req_struct *req_data);
...
static S32 srv_clog_idx_sort_by_log_id(void *src, void *dst)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    srv_clog_si_struct *src_node = (srv_clog_si_struct*) src;
    srv_clog_si_struct *dst_node = (srv_clog_si_struct*) dst;
        ...
        srv_clog_num_info_struct *num_info)
{
    ...
    {
        si_log = srv_clog_si_tbl_index(idx_tbl->si_tbl, idx_list[i]);
        SRV_CLOG_IT_ASSERT(srv_clog_si_log_is_valid(si_log));
        types = SRV_CLOG_SILOG_GET_TYPE_ID(si_log);
        if (!srv_clog_pi_is_ip_sub(idt, types))
            continue;
        
        num_info->num[SRV_CLOG_LOG_NUM_TYPE_TOTAL_LOGS]++;
        num_info->num[SRV_CLOG_LOG_NUM_TYPE_UNREAD_NUM] += SRV_CLOG_SILOG_GET_UNREADNUM(si_log);
    }
    return SRV_CLOG_RET_OK;
}
 ...
 *  void
 *****************************************************************************/
static void srv_clog_idx_init_idx_info_idt(srv_clog_idx_tbl_struct *idx_tbl, srv_clog_idx_info_enum info_type, S32 init_all)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
static S32 srv_clog_idx_reset_l2(srv_clog_idx_tbl_struct *idx_tbl)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    return srv_clog_idx_filter_reset(idx_tbl, SRV_CLOG_IDX_INFO_FILTER);
}
...
static S32 srv_clog_idx_sort_l1(srv_clog_idx_tbl_struct *idx_tbl)
{
    ...
    for (i = 0; i < SRV_CLOG_LOG_MAX_NUM; i++)
    {
        ...
        if (!srv_clog_si_log_is_valid(si_log))
            continue;
        if (!srv_clog_pi_is_ip_sub(&(idx_tbl->idx_info[SRV_CLOG_IDX_INFO_BASE]->idt), SRV_CLOG_SILOG_GET_TYPE_ID(si_log)))
            continue;
            ...
            U32 si_tbl_idx)
{
    ...
    srv_clog_idx_info_struct *idx_info;
    SRV_CLOG_COMPARE_FUNC sort_func = srv_clog_idx_sort_by_log_id;
...
static S32 srv_clog_idx_filter_reset(srv_clog_idx_tbl_struct *idx_tbl, srv_clog_idx_info_enum info_type)
{
    ...
    if (srv_clog_idx_is_same_sort(idx_base, idx_flt))
    {
        ...
        {
            si_log = srv_clog_si_tbl_index(si_tbl, idx_base_data[i]);
            if (srv_clog_pi_is_ip_sub(flt_idt, SRV_CLOG_SILOG_GET_TYPE_ID(si_log)))
            {
        ...
        U32 *next_idx)
{
    ...
    {
        si_log = srv_clog_si_tbl_index(si_tbl, idx_list[i]);
        if (!srv_clog_si_log_is_valid(si_log))
            continue;
        ids[j] = SRV_CLOG_SILOG_GET_LOG_ID(si_log);
        j++;
    }
    *ids_size = j;
    if (i < total_num)
        ...
        U32 *si_tbl_idx)
{
    ...
    for (i = 0; i < total_num; i++)
    {
        if (idx_info_data[i] == SRV_CLOG_IDX_INVALID_IDX)
            continue;
        if (log_id != SRV_CLOG_SILOG_GET_LOG_ID(si_tbl_data[idx_info_data[i]]))
            continue;
        *idx_tbl_idx = i;
        if (si_tbl_idx)
...
SRV_CLOG_ID srv_clog_idx_idx2id(srv_clog_idx_tbl_struct *idx_tbl, srv_clog_idx_info_enum info_type, U32 index)
{
    ...
    si_log = srv_clog_si_tbl_index(idx_tbl->si_tbl, sit_idx);
    if (srv_clog_si_log_is_valid(si_log))
        log_id = SRV_CLOG_SILOG_GET_LOG_ID(si_log);

    SRV_CLOG_IT_TRACE((SRV_CLOG_TRC_INFO, TRC_SRV_CLOG_IDX_IDX2ID, 
        info_type, index, sit_idx, log_id, SRV_CLOG_SILOG_GET_TYPE_ID(si_log)));
    return log_id;
}
            ...
            srv_clog_log_struct *log)
{
    ...
    for (i = 0, j = last_idx; i < max_size; i++, j++)
    {
        ...
        if (idx_data[j] == SRV_CLOG_IDX_INVALID_IDX)
            continue;
        si_log = srv_clog_si_tbl_index(si_tbl, idx_data[j]);
        if (!srv_clog_si_log_is_valid(si_log))
            continue;
        if (log_id == SRV_CLOG_SILOG_GET_LOG_ID(si_log))
        {
...
srv_clog_si_struct *srv_clog_idx_get_last_log(srv_clog_idx_tbl_struct *idx_tbl, srv_clog_log_identity_struct *idt)
{
    ...
    for (i = 0, j = 0; i < SRV_CLOG_LOG_MAX_NUM && j < total_num; i++)
    {
        ...
        if (!srv_clog_si_log_is_valid(sort_info))
            continue;
        j++;
        if (srv_clog_pi_is_ip_sub(idt, SRV_CLOG_SILOG_GET_TYPE_ID(sort_info)))
        {
            if (SRV_CLOG_SILOG_GET_LOG_ID(sort_info) > log_id)
            {
                idx = i;
                log_id = SRV_CLOG_SILOG_GET_LOG_ID(sort_info);
            }
        }
    }
 ...
 *  
 *****************************************************************************/
S32 srv_clog_idx_add_prepare(srv_clog_idx_tbl_struct *idx_tbl, srv_clog_add_log_req_struct *req_data)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (!req_data->need_merge)
    {
        ...
        {
            ret = srv_clog_si_tbl_find_best_slot(idx_tbl->si_tbl, &req_data->best_sit_idx);
            SRV_CLOG_ASSERT(ret == SRV_CLOG_RET_OK);
        }
    }
    ...
    MMI_TRACE(
        ...
        SRV_CLOG_SILOG_GET_LOG_ID(&req_data->si_log),
 ...
 * DESCRIPTION
 *   check if need merge, merge to which one, if add to new slot
 * PARAMETERS
 *  idx_tbl         [?]     
 *  req_data        [?]     
 * RETURNS
...
static S32 srv_clog_idx_check_merge(srv_clog_idx_tbl_struct *idx_tbl, srv_clog_add_log_req_struct *req_data)
{
    ...
    if (SRV_CLOG_CHECK_BIT(merge_style, SRV_CLOG_STYLE_MERGE_IN_SIM_ID))
        match_props |= SRV_CLOG_TSP_SIM_ID_GRP;
    if (SRV_CLOG_CHECK_BIT(merge_style, SRV_CLOG_STYLE_MERGE_IN_LOG_TYPE))
        match_props |= SRV_CLOG_TSP_CLT_GRP;
...
//    if (SRV_CLOG_CHECK_BIT(merge_style, SRV_CLOG_STYLE_MERGE_IN_SUB_TYPE))
    ...
    for (i = 0; i < total_num; i++)
    {
        si_log = srv_clog_si_tbl_index(si_tbl, idx_info_data[i]);
        /* If the scope not matched just skip it */
        if (!SRV_CLOG_PI_PP_CMP(SRV_CLOG_SILOG_GET_TYPE_ID(&req_data->si_log), SRV_CLOG_SILOG_GET_TYPE_ID(si_log), match_props))
            continue;

        ret = srv_clog_si_log_check_merge(&req_data->si_log, si_log, merge_style);
        if (ret)    /* can merge */
        {
            req_data->old_log_id = SRV_CLOG_SILOG_GET_LOG_ID(si_log);
            req_data->old_type = SRV_CLOG_SILOG_GET_TYPE_ID(si_log);
            ...
            if (!req_data->sup_thread)
            {
                ...
                srv_clog_si_log_get_num(si_log, &log_total_num, &log_ur_num);
                srv_clog_si_log_update_num(&req_data->si_log, log_total_num, log_ur_num, MMI_TRUE);
            }
            break;
        }
                            ...
                            srv_clog_log_identity_struct *type_idt)
{
    ...
    {
        type_limit = srv_clog_pi_get_num_limit_by_p(num_style, SRV_CLOG_SILOG_GET_TYPE_ID(si_log), type_idt);
...
static S32 srv_clog_idx_find_oldest(srv_clog_idx_tbl_struct *idx_tbl, srv_clog_add_log_req_struct *req_data)
{
    ...
    switch (req_data->is_full)
    {
        ...
        case SRV_CLOG_FULL_TYPE:
            ret = srv_clog_idx_base_get_oldest_by_idt(idx_tbl, req_data);
            break;
...
static S32 srv_clog_idx_base_get_oldest(srv_clog_idx_tbl_struct *idx_tbl, srv_clog_add_log_req_struct *req_data)
{
    ...
    si_log = srv_clog_si_tbl_index(idx_tbl->si_tbl, req_data->old_sit_idx);
    req_data->old_log_id = SRV_CLOG_SILOG_GET_LOG_ID(si_log);
    req_data->old_type = SRV_CLOG_SILOG_GET_TYPE_ID(si_log);
...
static S32 srv_clog_idx_base_get_oldest_by_idt(srv_clog_idx_tbl_struct *idx_tbl, srv_clog_add_log_req_struct *req_data)
{
    ...
    {
        si_log = srv_clog_si_tbl_index(si_tbl, idx_info_data[i]);
        if (srv_clog_pi_is_ip_sub(&req_data->type_idt, SRV_CLOG_SILOG_GET_TYPE_ID(si_log)))
        {
            req_data->old_idxi_idx = i;
 ...
 *  
 *****************************************************************************/
S32 srv_clog_idx_add_update(srv_clog_idx_tbl_struct *idx_tbl, srv_clog_add_log_req_struct *req_data)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
        ...
        U32 idx_start_idx)
{
    ...
    {
        if (srv_clog_si_log_check_match(idx_tbl->si_tbl, idx_tbl_data[i], req_data, &log_id))
        {
