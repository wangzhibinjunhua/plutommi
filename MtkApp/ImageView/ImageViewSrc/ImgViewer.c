...
static MMI_BOOL g_is_phone_mem_enabled;
...
if(total_drv_num > 1)
{
	/* total_drv_num > 1, then Phone memory is enabled/accessible. */
	g_is_phone_mem_enabled = MMI_TRUE;
}
else
{
	g_is_phone_mem_enabled = MMI_FALSE;
}

#ifdef __MMI_IV_PLUTO_SLIM__
if(g_is_phone_mem_enabled)
{
	/* Enable storage if phone memory is enabled because Storage is the only option menu item that will be shown in PLUTO SLIM projects*/
	opt_menu_empty = MENU_ID_IMGVIEW_EMPTY_FOLDER_OPTION;
}
#else
	opt_menu_empty = MENU_ID_IMGVIEW_EMPTY_FOLDER_OPTION;
#endif
