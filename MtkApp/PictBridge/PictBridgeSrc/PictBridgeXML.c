    ...
    for (i = 0; i < PICT_XML_TAG_MAX; i++)
    {
        ...
        switch(i)
        {
            ...
            case PICT_XML_TAG_LAYOUTS:
                ...
                {
                    xml_attr_value = (U32) atoi(*(attr + 1)); /* 1 is the space after attribute */
                    ...
                    else if (xml_attr_value == PICT_XML_VAL_PAPER_L)
                    {
                        g_pict_bridge_xml.xml_attr = 5; //PICT_PAPER_L
                    }
                    else if (xml_attr_value == PICT_XML_VAL_PAPER_2L)
                    {
                        g_pict_bridge_xml.xml_attr = 6; // PICT_PAPER_2L
                    }
                    else
                    {
                        g_pict_bridge_xml.xml_attr = 7; //PICT_PAPER_MAX; /* not supported paper by phone */
                    }
                }
                break;
        ...
        switch(i)
        {
            ...
            case PICT_XML_TAG_ABORT_JOB:
                    ...
                    mmi_frm_send_ilm(MOD_USB,MSG_ID_MMI_USB_CLOSE_JOB_REQ,NULL,NULL);
                        ...
                        mmi_dmui_lawmo_report(MMI_DMUI_APP_ID_PICT, MMI_DMUI_CMD_LAWMO_PARTIALLYLOCK, MMI_DMUI_RESULT_SUCCESS);
                    #endif
#endif
...
static void mmi_pict_read_xml_data_element(void *data, const char *el, const char *value, kal_int32 len, kal_int32 error)
{
    ...
    for (i = 0; i < PICT_XML_TAG_MAX; i++)
    {
        ...
        /* for only one data, it's data length is 8, for multiply data, it assume there is only one space between data  */
        ...
        switch(i)
        {
            ...
            case PICT_XML_TAG_RESULT:
                ...
                if (g_pict_bridge_xml.xml_result != PICT_XML_VAL_OK)
                {
                    ...
                    {
                        mmi_pict_entry_print_done(MMI_FALSE, (U16) ((g_pict_bridge_xml.xml_result == PICT_XML_VAL_NOT_EXECUTED) ? STR_ID_PICT_CMD_NOT_EXECUTED : STR_GLOBAL_UNSUPPORTED_FORMAT));
                    }
                }
                break;
            ...
            case PICT_XML_TAG_PAPER_SIZES:
                for (j = 0; j < xml_data_count; j++)
                {
                    ...
                    else if (xml_data_value == PICT_XML_VAL_PAPER_L)
                    {
                        g_pict_bridge_xml.paper_sizes |= PICT_PAPER_L;
                    }
                    else if (xml_data_value == PICT_XML_VAL_PAPER_2L)
                    {
                        g_pict_bridge_xml.paper_sizes |= PICT_PAPER_2L;
                    }
                    else
                    {
            ...
            case PICT_XML_TAG_LAYOUTS:
                for (j = 0; j < xml_data_count; j++)
                {
                    ...
                    else if (strncmp(value + j * (PICT_VALUE_LEN + 1), PICT_XML_VAL_LAYOUT_FULL, 8) == 0)
                    {
                        g_pict_bridge_xml.layouts[g_pict_bridge_xml.xml_attr] |= PICT_LAYOUT_FULL;
                    }
                    else
                    {
                        /* unsupported layouts */
            ...
            case PICT_XML_TAG_ERROR_STATUS:
                ...
                if (g_pict_bridge_context.printing != PICT_PRINTING_NONE)
                {
                    ...
                    {
                        wgui_status_icon_bar_set_icon_display(STATUS_ICON_PRINT); /* show print icon */
            ...
            case PICT_XML_TAG_ERROR_REASON:
                ...
                else
                {
                    if (g_pict_bridge_context.printing == PICT_PRINTING_ERROR)
                    {
                        ...
                        {
                            g_pict_bridge_context.error_string_id = STR_ID_PICT_ERROR_ASK;
                        }
                        /* the index of wrong picture */
                        g_pict_bridge_context.error_index = g_pict_bridge_context.curr_index;
...
void mmi_pict_generate_ansii_8_dot_3_filename(U8 *output_ansii, U8* input_ucs2)
{
    ...
    srv_fmgr_path_remove_filename((WCHAR*)path);
    mmi_ucs2cat(path, (S8*) input_ucs2);

    fs_handle = FS_Open((U16*) path, FS_READ_ONLY);
    if (fs_handle <= 0)
    {
        mmi_ucs2_to_asc((S8*)output_ansii, (S8*) PICT_DEFAULT_FILENAME);
        return;
    }
...
void mmi_pict_prepare_xml_get_capability(void)
{
    ...
    switch (g_pict_bridge_context.xml_capability_index)
    {
        ...
        case PICT_XML_TAG_LAYOUTS:
            ...
            else if ((g_pict_bridge_xml.paper_sizes & PICT_PAPER_L) && !(g_pict_bridge_context.xml_layout_index & PICT_PAPER_L))
            {
                strcat(g_pict_bridge_context.xml_operation_cmd, "<layouts paperSize=\"51010000\"/>");
                g_pict_bridge_context.xml_layout_index |= PICT_PAPER_L;
            }
            else if ((g_pict_bridge_xml.paper_sizes & PICT_PAPER_2L) && !(g_pict_bridge_context.xml_layout_index & PICT_PAPER_2L))
            {
                strcat(g_pict_bridge_context.xml_operation_cmd, "<layouts paperSize=\"51020000\"/>");
                g_pict_bridge_context.xml_layout_index |= PICT_PAPER_2L;
            }
            else
            {
                strcat(g_pict_bridge_context.xml_operation_cmd, "<layouts paperSize=\"51000000\"/>");
...
void mmi_pict_prepare_xml_start_job(void)
{
    ...
    /*----------------------------------------------------------------*/
    U8 cnt, i;
    U8 ansii_filename[SRV_FMGR_PATH_MAX_LEN + 1];
    ...
    if (g_pict_bridge_context.printing_capability[1] == 0)
    {
        do
        {
            cnt = 0;
            ...
            if (g_pict_bridge_xml.paper_sizes & PICT_PAPER_L)
            {
                if (cnt == g_pict_bridge_setting.paper_size)
                {
                    strcat(g_pict_bridge_context.xml_operation_cmd, "<paperSize>51010000</paperSize>");
                    break;
                }
                else
                {
            ...
            if (g_pict_bridge_xml.paper_sizes & PICT_PAPER_2L)
            {
                if (cnt == g_pict_bridge_setting.paper_size)
                {
                    strcat(g_pict_bridge_context.xml_operation_cmd, "<paperSize>51020000</paperSize>");
                    break;
                }
                else
                {
    ...
    if (g_pict_bridge_setting.page_format == 0) /* standard */
    {
        ...
        for (i = 0; i < g_pict_bridge_context.multi_file_select; i++)
        {
            ...
            sprintf(g_pict_bridge_context.xml_operation_cmd + strlen(g_pict_bridge_context.xml_operation_cmd), "%d/%d/%d",
                    g_pict_bridge_context.multi_file_date[i].Year1980 + 1980,
                    g_pict_bridge_context.multi_file_date[i].Month,
                    g_pict_bridge_context.multi_file_date[i].Day);
            strcat(g_pict_bridge_context.xml_operation_cmd, "</date>");
        ...
        strcat(g_pict_bridge_context.xml_operation_cmd, "<fileName>");
        if (mmi_pict_is_filename_not_alphanumeric((U8*) srv_fmgr_path_get_filename_ptr((WCHAR*) g_pict_bridge_context.printing_filepath)))
        {
            mmi_ucs2_to_asc((S8*) ansii_filename, (S8*) PICT_DEFAULT_FILENAME);
        }
        else
        {
            mmi_ucs2_to_asc((S8*) ansii_filename, (S8*)srv_fmgr_path_get_filename_ptr((WCHAR*) g_pict_bridge_context.printing_filepath));
        }
        strcat(g_pict_bridge_context.xml_operation_cmd, (S8*) ansii_filename);
        strcat(g_pict_bridge_context.xml_operation_cmd, "</fileName>");
        strcat(g_pict_bridge_context.xml_operation_cmd, "<date>");
        sprintf(g_pict_bridge_context.xml_operation_cmd + strlen(g_pict_bridge_context.xml_operation_cmd), "%d/%d/%d",
                g_pict_bridge_context.printing_filedate.Year1980 + 1980,
                g_pict_bridge_context.printing_filedate.Month,
                g_pict_bridge_context.printing_filedate.Day);
        strcat(g_pict_bridge_context.xml_operation_cmd, "</date>");
        strcat(g_pict_bridge_context.xml_operation_cmd, "<copies>");
        sprintf((S8*) ansii_value, "%03d", g_pict_bridge_setting.number_of_copies + 1);
        strcat(g_pict_bridge_context.xml_operation_cmd, (S8*) ansii_value);
