...
void mmi_fmsr_alarm_expire_ind(U16 index, BOOL power_on_record)
{
    ...
    g_fmsr_cntx.rec.format = (data_ptr->format >= FMSR_REC_FORMAT_NUM) ? FMSR_FORMAT_WAV : data_ptr->format;
#elif defined(__I2S_AWB_RECORD__)
    g_fmsr_cntx.rec.format = (data_ptr->format >= FMSR_REC_FORMAT_NUM) ? FMSR_FORMAT_AWB : data_ptr->format;
#elif defined(__I2S_DAF_RECORD__)
    g_fmsr_cntx.rec.format = (data_ptr->format >= FMSR_REC_FORMAT_NUM) ? FMSR_FORMAT_DAF : data_ptr->format;
#else 
    MMI_ASSERT(0);
#endif 
...
BOOL mmi_fmsr_alarm_expire_check_file(void *ptr)
{
    ...
    if (g_fmsr_cntx.rec.type == FMSR_TYPE_NEW
#if !defined(__FMRADIO_I2S_SUPPORT__) || defined(__I2S_WAV_RECORD__)
        || g_fmsr_cntx.rec.format == FMSR_FORMAT_WAV
#endif 
#if defined(__I2S_DAF_RECORD__)
        || g_fmsr_cntx.rec.format == FMSR_FORMAT_DAF
#endif 
        )
    {
...
#endif 
#if defined(__I2S_DAF_RECORD__)
    if (g_fmsr_cntx.rec.format == FMSR_FORMAT_DAF)
    {
        mmi_fmsr_util_get_storage_path((UI_string_type) g_fmsr_cntx.rec.full_path, g_fmsr_cntx.rec.drive);
        mmi_fmsr_util_get_new_file((UI_string_type) g_fmsr_cntx.rec.full_path, g_fmsr_cntx.rec.format);
    }
...
void mmi_fmsr_entry_recording(void)
{
    ...
    srv_backlight_turn_on(1);
    AlmDisableSPOF();
    /* Block medium priority notification screen */
    ...
    mmi_frm_set_destroy_scrn_callback(SCR_ID_FMSR_RECORDING, (HistoryDelCBPtr) mmi_fmsr_del_scr_callback);
   ...
   /*-- bettery --*/
    ...
    SetKeyHandler(mmi_fmsr_rec_release_RSK, KEY_END, KEY_EVENT_UP);
...
void mmi_fmsr_entry_show_msg(void)
{
    ...
    {
        EntryNewScreen(SCR_ID_FMSR_SHOW_MESSAGE, mmi_fmsr_exit_show_msg, NULL, NULL);
    }
...
void mmi_fmsr_exit_show_msg(void)
{
    ...
    if (!g_fmsr_cntx.rec.power_on_record)
    {
        ...
        {
            GenericExitScreen(SCR_ID_FMSR_SHOW_MESSAGE, mmi_fmsr_entry_show_msg);
        }
    }
#ifdef __MMI_SUBLCD__
    if (GetSubLCDScreenCountInHistory() > 0)
