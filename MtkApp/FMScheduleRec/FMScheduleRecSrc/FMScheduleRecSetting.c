...
#if !defined(__FMRADIO_I2S_SUPPORT__) || defined(__I2S_AMR_RECORD__) || defined(__I2S_AWB_RECORD__)
...
#elif defined(__I2S_WAV_RECORD__)
    g_fmsr_cntx.setting.format = (S32) (data_ptr->format >= FMSR_REC_FORMAT_NUM) ? FMSR_FORMAT_WAV : data_ptr->format;
#elif defined(__I2S_AWB_RECORD__)
    g_fmsr_cntx.setting.format = (S32) (data_ptr->format >= FMSR_REC_FORMAT_NUM) ? FMSR_FORMAT_AWB : data_ptr->format;
#elif defined(__I2S_DAF_RECORD__)
    g_fmsr_cntx.setting.format = (S32) (data_ptr->format >= FMSR_REC_FORMAT_NUM) ? FMSR_FORMAT_DAF : data_ptr->format;
#else
    MMI_ASSERT(0);
#endif 
#ifndef __FMRADIO_I2S_SUPPORT__
...
#else 
    g_fmsr_cntx.setting.quality = FMSR_QUALITY_HIGH;
#endif 
...
void mmi_fmsr_main_setting_get_string(void)
{
    ...
    fmsr_repeat_str[FMSR_REPEAT_DAYS] = GetString(STR_GLOBAL_CUSTOM);
    ...
    fmsr_rec_fomrat_str[FMSR_FORMAT_DAF] = GetString(STR_ID_FMSR_DAF);
#endif 
#ifndef __FMRADIO_I2S_SUPPORT__
    fmsr_rec_quality_str[FMSR_QUALITY_LOW] = GetString(STR_GLOBAL_LOW);
    fmsr_rec_quality_str[FMSR_QUALITY_HIGH] = GetString(STR_GLOBAL_HIGH);
#endif 
...
void mmi_fmsr_exit_main_setting(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    GenericExitInlineScreen(SCR_ID_FMSR_MAIN_SETTING, mmi_fmsr_entry_main_setting);
}
...
BOOL mmi_fmsr_main_setting_check_freq_and_file(void)
{
    ...
    {
        mmi_display_popup(
            get_string(STR_ID_FMSR_VALID_FREQUENCY),
            MMI_EVENT_FAILURE);
        return FALSE;
    }
    ...
    {
        mmi_display_popup(
            get_string(STR_ID_FMSR_FILE_EMPTY),
            MMI_EVENT_FAILURE);
        return FALSE;
    }
    ...
    {
        mmi_display_popup(
            get_string(STR_ID_FMSR_NO_CARD),
            MMI_EVENT_FAILURE);
        return FALSE;
    }
        ...
        U8 repeat)
{
    ...
    {
        mmi_display_popup(
            get_string(STR_ID_FMSR_END_TIME_INCORRECT),
            MMI_EVENT_FAILURE);
        return FALSE;
    }
        ...
        (year == curr_time.nYear && mon == curr_time.nMonth && day == curr_time.nDay && start_time <= end_time))
    {
        ...
        {
            mmi_display_popup(
                get_string(STR_ID_FMSR_START_TIME_INCORRECT),
                MMI_EVENT_FAILURE);
            return FALSE;
        }
    }
    ...
    for (i = 0; i < NUM_OF_FMSR; i++)
    {
        if (i != fmsr_list_highlight)
        {
                    ...
                    data_ptr->min2))
            {
                ...
                {
                    mmi_display_popup(
                        get_string(STR_ID_FMSR_TIME_OVERLAP),
                        MMI_EVENT_FAILURE);
                    return FALSE;
                }
            }
...
void mmi_fmsr_main_setting_save_and_set(void)
{
    ...
    {
        mmi_display_popup(
            get_string(STR_ID_FMSR_CUSTOM_DAYS_INCORRECT),
            MMI_EVENT_FAILURE);
        return;
    }
...
void mmi_fmsr_entry_chan_list(void)
{
	...
	wgui_cat1032_show(
		(WCHAR*)GetString(STR_ID_FMSR_CHAN_LIST),
		GetImage(GetRootTitleIcon(MENU_ID_FMSR_MAIN)),
...
void mmi_fmsr_exit_chan_manual(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    GenericExitInlineScreen(SCR_ID_FMSR_CHAN_MANUAL, mmi_fmsr_entry_chan_manual);
}
...
void mmi_fmsr_rec_setting_prepare_inline_data(void)
{
    ...
    g_fmsr_cntx.setting.is_changed = 1;
    g_fmsr_cntx.setting.rec_setting_changed = 0;
...
void mmi_fmsr_rec_setting_highlight_hdlr(S32 item_index)
{
    ...
    if (item_index == FMSR_REC_SETTING_FILE_NAME)
    {
        if (MMI_FALSE){}
    #if !defined(__FMRADIO_I2S_SUPPORT__) || defined(__I2S_AMR_RECORD__)
        else if ((g_fmsr_cntx.setting.type == FMSR_TYPE_APPEND) && (g_fmsr_cntx.setting.format == FMSR_FORMAT_AMR))
        {
            ChangeLeftSoftkey(STR_GLOBAL_SELECT, IMG_GLOBAL_OK);
...
void mmi_fmsr_entry_rec_setting(void)
{
    ...
    if (g_fmsr_cntx.setting.rec_setting_changed)
    {
        g_fmsr_cntx.setting.rec_setting_changed = 0;
        if (GetInlineDoneFlag(guiBuffer) == 0)
        {
...
void mmi_fmsr_exit_rec_setting(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    GenericExitInlineScreen(SCR_ID_FMSR_REC_SETTING, mmi_fmsr_entry_rec_setting);
}
...
#if !defined(__FMRADIO_I2S_SUPPORT__) || defined(__I2S_WAV_RECORD__)
    if (index == FMSR_FORMAT_WAV && g_fmsr_cntx.setting.type != FMSR_TYPE_NEW)
    {
        g_fmsr_cntx.setting.type = FMSR_TYPE_NEW;
        changed = TRUE;
    }
#endif 
#if defined(__I2S_DAF_RECORD__)
    if (index == FMSR_FORMAT_DAF && g_fmsr_cntx.setting.type != FMSR_TYPE_NEW)
    {
        g_fmsr_cntx.setting.type = FMSR_TYPE_NEW;
        changed = TRUE;
    }
...
#if !defined(__FMRADIO_I2S_SUPPORT__) || defined(__I2S_WAV_RECORD__) && (defined(__I2S_AMR_RECORD__) || defined(__I2S_AWB_RECORD__))
    if (index == FMSR_FORMAT_WAV
#if defined(__I2S_DAF_RECORD__)
        || index == FMSR_FORMAT_DAF
#endif 
        )
    {
        DisableInlineItem(&wgui_inline_items[FMSR_REC_SETTING_FILE_TYPE], FMSR_REC_SETTING_FILE_TYPE);
        DisableInlineItem(&wgui_inline_items[FMSR_REC_SETTING_FILE_CAP], FMSR_REC_SETTING_FILE_CAP);
    }
    else
    {
        EnableInlineItem(&wgui_inline_items[FMSR_REC_SETTING_FILE_TYPE], FMSR_REC_SETTING_FILE_TYPE);
        EnableInlineItem(&wgui_inline_items[FMSR_REC_SETTING_FILE_CAP], FMSR_REC_SETTING_FILE_CAP);
    }

    if ((g_fmsr_cntx.setting.type == FMSR_TYPE_APPEND) && (
    #if !defined(__FMRADIO_I2S_SUPPORT__) || defined(__I2S_AWB_RECORD__)
    #if defined(AMRWB_ENCODE) && defined(AMRWB_DECODE)
        (g_fmsr_cntx.setting.format == FMSR_FORMAT_AWB) ||
    #endif 
    #endif 
    #if !defined(__FMRADIO_I2S_SUPPORT__) || defined(__I2S_AMR_RECORD__)
        ...
        (MMI_TRUE)))
    {
        EnableInlineItem(&wgui_inline_items[FMSR_REC_SETTING_FILE_NAME], FMSR_REC_SETTING_FILE_NAME);
    }
    else
    {
        DisableInlineItem(&wgui_inline_items[FMSR_REC_SETTING_FILE_NAME], FMSR_REC_SETTING_FILE_NAME);
    }
#endif 
}
...
void mmi_fmsr_rec_setting_storage_hdlr(S32 index)
{
    ...
    if (g_fmsr_cntx.setting.drive != drv_letter)
    {
        ...
        if(g_fmsr_cntx.setting.type != FMSR_TYPE_NEW)
        {
            g_fmsr_cntx.setting.type = FMSR_TYPE_NEW;
            changed = TRUE;
        }
...
void mmi_fmsr_rec_setting_fill_inline_item(void)
{
   ...
   /*-- Storage Caption --*/
    SetInlineItemActivation(
        ...
        INLINE_ITEM_ACTIVATE_WITHOUT_KEY_EVENT,
        0);
    SetInlineItemCaption(&(wgui_inline_items[FMSR_REC_SETTING_STORAGE_CAP]), (U8*) GetString(STR_GLOBAL_STORAGE));
...
#if !defined(__FMRADIO_I2S_SUPPORT__) || defined(__I2S_WAV_RECORD__)
    if (g_fmsr_cntx.setting.format == FMSR_FORMAT_WAV
#if defined(__I2S_DAF_RECORD__)
        || g_fmsr_cntx.setting.format == FMSR_FORMAT_DAF
#endif 
        )
    {
        DisableInlineItem(&wgui_inline_items[FMSR_REC_SETTING_FILE_TYPE], FMSR_REC_SETTING_FILE_TYPE);
...
#endif 
#endif /* !defined(__FMRADIO_I2S_SUPPORT__) || defined(__I2S_AMR_RECORD__) || defined(__I2S_AMR_RECORD__) */
}
...
void mmi_fmsr_rec_setting_done(void)
{
    ...
    if ((g_fmsr_cntx.setting.format == FMSR_FORMAT_WAV
        ...
        || g_fmsr_cntx.setting.format == FMSR_FORMAT_DAF
#endif 
        ) && g_fmsr_cntx.setting.type == FMSR_TYPE_APPEND)
    {
        mmi_display_popup(
            get_string(STR_ID_FMSR_APPEND_ERORR),
            MMI_EVENT_FAILURE);
...
void mmi_fmsr_rec_select_file(void)
{
    ...
    if (MMI_FALSE) {}
#if !defined(__FMRADIO_I2S_SUPPORT__) || defined(__I2S_AMR_RECORD__)
    else if (g_fmsr_cntx.setting.format == FMSR_FORMAT_AMR)
    {
    ...
    mmi_fmgr_select_path_and_enter(
        APP_FMSCHEDULEREC,
        FMGR_SELECT_FILE | FMGR_SELECT_REPEAT | FMGR_FLAG_FIXED_PATH,
        filter,
        (S8*) path,
...
void mmi_fmsr_rec_select_file_done(void *fullname, int is_short)
{
    ...
    {
        mmi_display_popup(
            get_string(STR_ID_FMSR_FILENAME_TOO_LONG),
            MMI_EVENT_FAILURE);
    }
    else
    {
        ...
        g_fmsr_cntx.setting.rec_setting_changed = 1;
...
void mmi_fmsr_util_append_file_ext(UI_string_type filename, U8 format)
{
    ...
    switch (format)
    {
        ...
        case FMSR_FORMAT_DAF:
            mmi_ucs2ncat((S8*) filename, (S8*) FMSR_FILE_EXT_DAF, MAX_FMSR_FILE_NAME_LEN);
            break;
    #endif 
    }
}
