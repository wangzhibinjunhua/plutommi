...
extern void widget_clear_screen(void);
...
//extern mmi_hist_smallscreen_state_enum GetSmallScreenState(void);
...
void ctk_options_overwrite_RSK_callback(void); 
 ...
 *  
 *****************************************************************************/
static U32 ctk_convert_input_type(ctk_input_type_enum type)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    switch (type)
    {
        ...
        case CTK_INPUT_TYPE_ALPHANUMERIC_SENTENCECASE:
            return IMM_INPUT_TYPE_SENTENCE | override_flag;
        ...
        case CTK_INPUT_TYPE_ALPHA:
            return IMM_INPUT_TYPE_ENGLISH_SENTENCE_LOWERCASE | override_flag;

        case CTK_INPUT_TYPE_NUMBER:
            return IMM_INPUT_TYPE_NUMERIC;

        case CTK_INPUT_TYPE_DECIMAL:
            return IMM_INPUT_TYPE_DECIMAL_NUMERIC;
        ...
        case CTK_INPUT_TYPE_PHONE_NUMBER:
            ...
            return IMM_INPUT_TYPE_PHONE_NUMBER | INPUT_TYPE_PLUS_CHARACTER_HANDLING;

        case CTK_INPUT_TYPE_ALPHANUMERIC_SENTENCECASE_USE_ENCODING_BASED_LENGTH:
            return (IMM_INPUT_TYPE_SENTENCE | INPUT_TYPE_USE_ENCODING_BASED_LENGTH);

	      case CTK_INPUT_TYPE_URL:
            /* To Support Url input type in Ctk inline menu */
...
static void ctk_screen_general_entry_function(void)
{
	...
	//if(GetSmallScreenState() == MMI_HIST_SMALLSCREEN_ACTIVE)
    ...
    CTK_LOG(CTK_LOG_GROUP, MMI_CTK_SCREEN_GENERAL_ENTRY_FUNC);
    ...
    CTK_LOG(CTK_LOG_GROUP, MMI_CTK_SCREEN_GENERAL_ENTRY_FUNC_END);
}
...
static void ctk_screen_general_exit_handler(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    CTK_LOG(CTK_LOG_GROUP, MMI_CTK_SCREEN_GENERAL_EXIT_HDLR);
    CTK_DBG_ASSERT(mmiapi_is_MMI_task());
    ...
    CTK_LOG(CTK_LOG_GROUP, MMI_CTK_SCREEN_GENERAL_EXIT_HDLR_END);
}
...
static void ctk_screen_general_key_handler(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    CTK_LOG(CTK_LOG_GROUP, MMI_CTK_SCREEN_GENERAL_KEY_HDLR);
    CTK_DBG_ASSERT(mmiapi_is_MMI_task());
...
static void ctk_screen_setup_key_handler(ctk_screen_struct *this, ctk_layout_struct *layout_p)
{
    ...
    {
        _ctk_skip_key = (mmi_keypads_enum)keycode;
    }
...
static void ctk_screen_add_history(ctk_screen_struct *scr_p)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    CTK_LOG(CTK_LOG_GROUP, MMI_CTK_SCREEN_ADD_HIST);
    CTK_DBG_ASSERT(mmiapi_is_MMI_task());
...
void ctk_screen_close(ctk_screen_handle scrid)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    CTK_LOG(CTK_LOG_GROUP, MMI_CTK_SCREEN_CLOSE);
...
void ctk_screen_invoke_on_enter(ctk_screen_handle scrid)
{
    ...
    /*----------------------------------------------------------------*/
    ASSERT(IS_SCREEN(scrid));
    CTK_LOG(CTK_LOG_GROUP, MMI_CTK_SCREEN_INVOKE_ON_ENTER);
...
void ctk_screen_invoke_on_exit(ctk_screen_handle scrid)
{
    ...
    /*----------------------------------------------------------------*/
    ASSERT(IS_SCREEN(scrid));
    CTK_LOG(CTK_LOG_GROUP, MMI_CTK_SCREEN_INVOKE_ON_EXIT);
...
void ctk_screen_invoke_redraw(ctk_screen_handle scrid)
{
    ...
    /*----------------------------------------------------------------*/
    ASSERT(IS_SCREEN(scrid));
    CTK_LOG(CTK_LOG_GROUP, MMI_CTK_SCREEN_INVOKE_REDRAW);
    ...
    if (this->scr_state != CTK_SCREEN_STATE_ACTIVE)
    {
        ...
        CTK_LOG(CTK_LOG_GROUP, MMI_CTK_SCREEN_INVOKE_REDRAW_NOT_ACTIVE);
        return;
    }
...
extern void widget_ctk_msf_menu_edit_options_highlight_handler(U32 index);
/*****************************************************************************
 * FUNCTION
 *  ctk_screen_enter
 * DESCRIPTION
...
void ctk_screen_enter(ctk_screen_handle scrid)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    CTK_LOG(CTK_LOG_GROUP, MMI_CTK_SCREEN_ENTER);
    CTK_DBG_ASSERT(mmiapi_is_MMI_task());
    ...
    if(!(wgui_inputs_menu_index_in_editor_range() && is_editor_options_menu))
	{
    ctk_screen_setup_key_handler(this, layout);
	}
	if(is_editor_options_menu)
	{
		wgui_inputs_option_menu_register_key_callback(ctk_options_overwrite_RSK_callback);
		wgui_inputs_option_menu_register_highlight_callback(widget_ctk_msf_menu_edit_options_highlight_handler);
        ctk_options_overwrite_RSK_callback();
	}

    CTK_LOG(CTK_LOG_GROUP, MMI_CTK_SCREEN_ENTER_END);
}
...
void ctk_options_overwrite_RSK_callback(void)
{
	...
	SetKeyHandler(ctk_screen_general_key_handler, KEY_LEFT_ARROW, KEY_EVENT_DOWN);	
}

extern void widget_ctk_msf_menu_edit_options_highlight(ctk_screen_handle scrid, int index, int start_index, int item_number);
/*****************************************************************************
 * FUNCTION
 *  widget_ctk_msf_menu_edit_options_highlight_handler
 * DESCRIPTION
 ...
 *  void
 *****************************************************************************/
void widget_ctk_msf_menu_edit_options_highlight_handler(U32 index)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	...
	widget_ctk_msf_menu_edit_options_highlight(SCREEN_H(scr), index, start_item, item_number);
}
/*****************************************************************************
 * FUNCTION
 *  ctk_screen_redraw
...
void ctk_screen_redraw(ctk_screen_handle scrid)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    CTK_LOG(CTK_LOG_GROUP, MMI_CTK_SCREEN_REDRAW);
    CTK_DBG_ASSERT(mmiapi_is_MMI_task());

    if (this->scr_state != CTK_SCREEN_STATE_ACTIVE)
    {   /* User application might update the content of the CTK screen when it is not active */
        CTK_LOG(CTK_LOG_GROUP, MMI_CTK_SCREEN_REDRAW_NOT_ACTIVE);
        return;
    }
    ...
    CTK_LOG(CTK_LOG_GROUP, MMI_CTK_SCREEN_REDRAW_END);
}
...
void ctk_screen_exit(ctk_screen_handle scrid, ctk_screen_exit_enum exit_type)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    CTK_LOG(CTK_LOG_GROUP, MMI_CTK_SCREEN_EXIT);
    CTK_DBG_ASSERT(mmiapi_is_MMI_task());
    ...
    switch (exit_type)
    {
        ...
        case CTK_SCREEN_EXIT_PAUSE:
            CTK_LOG(CTK_LOG_GROUP, MMI_CTK_SCREEN_EXIT_ADD_HIST);
        ...
        case CTK_SCREEN_EXIT_PAUSE_NOT_IN_HISTORY:
            CTK_LOG(CTK_LOG_GROUP, MMI_CTK_SCREEN_EXIT_NO_IN_HIST);
    ...
    CTK_LOG(CTK_LOG_GROUP, MMI_CTK_SCREEN_EXIT_END);
}
 ...
 *  void
 *****************************************************************************/
static void ctk_layout_AddHistory(ctk_layout_struct *this)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Always put category history in lay_guiBuffer including Category57Screen */
    this->lay_guiBuffer = ctk_mem_alloc(LAYOUT_H(this), MAX_GUI_BUFFER);
    GetCategoryHistory(this->lay_guiBuffer);
}
...
static void ctk_inline_general_highlight_handler(S32 index)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    CTK_LOG(CTK_LOG_GROUP, MMI_CTK_INLINE_GENERAL_HILITE_HDLR);
...
static void ctk_inline_general_cancel_handler(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    CTK_LOG(CTK_LOG_GROUP, MMI_CTK_INLINE_GENERAL_CANCEL_HDLR);
...
static void ctk_inline_show(ctk_screen_struct *scr_p, ctk_layout_struct *lay_p)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    CTK_LOG(CTK_LOG_GROUP, MMI_CTK_INLINE_SHOW);
...
static void ctk_inline_close_display(ctk_screen_struct *scr_p, ctk_layout_struct *this)
{
    ...
    CTK_LOG(CTK_LOG_GROUP, MMI_CTK_INLINE_CLOSE_DISPLAY);
...
static void ctk_inline_backup_content(ctk_screen_struct *scr_p, ctk_layout_struct *lay_p)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    h = ctk_mem_alloc(OBJ_H(scr_p), sizeof(history));
 ...
 *  void
 *****************************************************************************/
void ctk_pcategory_general_highlight_handler(S32 index)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    CTK_LOG(CTK_LOG_GROUP, MMI_CTK_PCATEGORY_GENERAL_HILITE_HDLR);
...
static void ctk_pcategory_show(ctk_screen_struct *screen_p, ctk_layout_struct *layout_p)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    CTK_LOG(CTK_LOG_GROUP, MMI_CTK_PCATEGORY_SHOW, (unsigned int)this->pcat_show_fp);
...
static void ctk_pcategory_close_display(ctk_screen_struct *scr_p, ctk_layout_struct *layout_p)
{
    ...
    CTK_LOG(CTK_LOG_GROUP, MMI_CTK_PCATEGORY_CLOSE_DISPLAY);
