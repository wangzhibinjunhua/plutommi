...
extern abrepeat_main_scr_struct g_abr_mainscr;
extern U8 g_abrepeat_repeat_always_finish;
...
void mmi_abrepeat_do_pause_action(void)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    mdi_result aud_result;
    
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    aud_result = mdi_audio_pause(mmi_abrepeat_pause_callback_hdlr, NULL);
    if (aud_result == MDI_AUDIO_SUCCESS)
    {
        mmi_abrepeat_paused();
    }
    else if (aud_result == MDI_AUDIO_UNSUPPORTED_OPERATION)
    {
        /* pause action for ADIF AAC files are not supportted so keep it playing */
        g_abrepeat.main_selected_button = ABREPEAT_MAIN_PLAY;
        mmi_display_popup(get_string(STR_ID_ABREPEAT_CANNOT_PAUSE), MMI_EVENT_FAILURE);
    }
    else
    {
...
void mmi_abrepeat_play_completed(void)
{
    ...
    /* Rule: when audio completes, button actions will be reset */
    g_abrepeat.is_main_sel_btn_down         = FALSE;
    g_abrepeat.is_absection_sel_btn_down    = FALSE;

    mmi_abrepeat_redraw_absection_buttons(g_abrepeat.absection_sel_btn);
    ...
    else/*play mode*/
    {
        ...
        {   
            g_abrepeat_repeat_always_finish = 0;
            if(g_abrepeat.curr_play_section < 0)
            {
                g_abrepeat.prgbar_cur_offset = g_abrepeat.total_offset-1;
                mmi_abrepeat_redraw_progress_bar();
...
*  
* PARAMETERS
*  aud_result      [IN]        Audio result
* RETURNS
*  void
*****************************************************************************/
void mmi_abrepeat_pause_callback_hdlr(mdi_result aud_result, void *user_data)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
void mmi_abrepeat_do_resume_action(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    aud_result = mdi_audio_resume(mmi_abrepeat_resume_callback_hdlr, NULL);
    if (aud_result == MDI_AUDIO_SUCCESS)
    {
...
void mmi_abrepeat_resumed(void)
{
    ...
    mmi_abrepeat_redraw_main_buttons();
    //  only to deal with a rare case as MAUI_01353965
    if (g_abrepeat.cur_mode == AB_PLAY_MODE 
        && g_abrepeat.replay_btn_state == AB_REPLAY_ALWAYS
        && g_abrepeat.curr_play_section >= 0)
    {
        if (g_abrepeat.is_main_sel_btn_down == MMI_FALSE)
        {
...
*  void
*****************************************************************************/
void mmi_abrepeat_resume_callback_hdlr(mdi_result aud_result, void *user_data)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
void mmi_abrepeat_do_play_action(void)
{
    ...
    aud_result = mmi_abrepeat_do_play_action_without_handle_result();
    if (aud_result == MDI_AUDIO_SUCCESS)
    {
        mmi_abrepeat_played();
    }
    ...
    {
        mmi_abrepeat_stopped();
        if (aud_result != MDI_AUDIO_BLOCKED)
        {
...
S32 mmi_abrepeat_do_play_action_without_handle_result(void)
{
    ...
    //else
    {
        ...
        if (g_abrepeat.curr_play_section >= 0)
        {
            aud_result = mdi_audio_play_file_portion_with_vol_path(
        ...
        {
            aud_result = mdi_audio_play_file_with_vol_path(
                g_abrepeat.filefullname,
                DEVICE_AUDIO_PLAY_ONCE,
                mmi_abrepeat_build_cache_ptr(),
                mmi_abrepeat_play_callback_hdlr,
...
*  
* PARAMETERS
*  aud_result      [IN]        Audio result after play
* RETURNS
*  void
*****************************************************************************/
void mmi_abrepeat_play_callback_hdlr(mdi_result aud_result, void *user_data)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
*  
* PARAMETERS
*  aud_result      [IN]        Audio result id
* RETURNS
*  U16
*****************************************************************************/
U16 mmi_abrepeat_err_str(S32 aud_result)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
