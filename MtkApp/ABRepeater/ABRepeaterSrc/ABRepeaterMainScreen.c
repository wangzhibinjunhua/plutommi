...
/* to avoid draw progress bar in go back history process. */
...
void mmi_abrepeat_show_mainscreen(U16 title, U16 left_softkey, U16 right_softkey, S8 skin)
{
    ...
    gdi_get_alpha_blending_source_layer(&abrepeat_old_alpha_blending_src_layer);
    gdi_set_alpha_blending_source_layer(abrepeat_skin_layer);
#endif /* __GDI_MEMORY_PROFILE_2__ */ 
...
/*****************************************************************************
* FUNCTION
*  mmi_abrepeat_mainscr_LSK_up_hldr
* DESCRIPTION
*  Function for handling lsk release event
* PARAMETERS
*  void
* RETURNS
*  void
*****************************************************************************/
void mmi_abrepeat_mainscr_LSK_up_hldr(void)
{
...
/*****************************************************************************
* FUNCTION
*  mmi_abrepeat_mainscr_RSK_up_hldr
* DESCRIPTION
*  Function for handling rsk release event
* PARAMETERS
*  void
* RETURNS
*  void
*****************************************************************************/
void mmi_abrepeat_mainscr_RSK_up_hldr(void)
{
...
void mmi_abrepeat_redraw_RSK(BOOL bPressed)
{
    ...
    if(bPressed)
    {
        gui_set_text_color(g_abr_mainscr.text_fg_color);
    }
    else
    {
        gui_set_text_color(g_abr_mainscr.text_fg_color);
    }
    gui_set_text_border_color(g_abr_mainscr.text_border_color);

    
    //wgui_softkey_get_offset(g_abr_mainscr.main_RSK_text, &x1, &y1, MMI_RIGHT_SOFTKEY);
    truncated = wgui_softkey_get_offset(
                    (UI_string_type) g_abr_mainscr.main_RSK_text,
                    &x1,
                    &y1,
                    &truncated_width,
                    MMI_RIGHT_SOFTKEY,
                    mmi_frm_get_screen_rotate());
    if (truncated == MMI_FALSE)
    {
    ...
    else // truncated == MMI_TRUE
    {
            ...
            {
                // no need to use this API  when  truncated
                //gui_move_text_cursor(x1, y1);
            }
            gdi_get_alpha_blending_source_layer(&old_src_layer);
#ifdef __GDI_MEMORY_PROFILE_2__
...
void mmi_abrepeat_redraw_LSK(BOOL bPressed)
{
    ...
    truncated = wgui_softkey_get_offset(
                    (UI_string_type) g_abr_mainscr.main_LSK_text,
                    &x1,
                    &y1,
                    &truncated_width,
                    MMI_LEFT_SOFTKEY,
...
void mmi_abrepeat_redraw_main_screen(void)
{
    ...
    gui_measure_string((UI_string_type)GetString(STR_ID_ABREPEAT_TITLE), &str_w, &str_h);
    x1 = (UI_device_width - str_w)>>1;
    y1 = MMI_status_bar_height + str_h / 2;
	gui_set_text_clip(x1, y1 - 1, x1 + str_w, y1+ str_h);
    if (r2lMMIFlag)
    {
		x1 += str_w;
...
void mmi_abrepeat_redraw_main_title(void)
{
    ...
    S32 y2 = g_abr_mainscr.area_title.pos_y + g_abr_mainscr.area_title.height;
    S32 scroll_text_height = 0;
    S32 yy = 0;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    ...
    if (character_height > g_abr_mainscr.area_title.height)
    {
        scroll_text_height = character_height;
        y2 = g_abr_mainscr.area_title.pos_y + character_height;
    }
    else
    {
        scroll_text_height = g_abr_mainscr.area_title.height;
#if defined(__MMI_MAINLCD_320X480__)
...
#if defined(__MMI_MAINLCD_320X480__)
    yy = (g_abr_mainscr.area_title.height - character_height) / 2;
    yy += y1;
#else
    yy = y1;
#endif
    gui_create_scrolling_text(
            ...
            g_abr_mainscr.text_fg_color,
            g_abr_mainscr.text_border_color);
    ...
    {
        /* background */
        mmi_abrepeat_redraw_main_title_background(x1 - 1, y1 - 1, x2, y2 + 1);
        
        gui_set_text_color(g_abr_mainscr.text_fg_color); 
        gui_set_text_border_color(g_abr_mainscr.text_border_color);
        if (r2lMMIFlag)
        {
            gui_move_text_cursor(x1 + (g_abr_mainscr.area_title.width + w) / 2, yy);
        }
        else
        {
            gui_move_text_cursor(x1 + (g_abr_mainscr.area_title.width - w) / 2, yy);
...
#if(UI_DOUBLE_BUFFER_SUPPORT)
    gui_unlock_double_buffer();
    gui_BLT_double_buffer(x1, y1 - 1, x2, y2 + 1);
#endif /* (UI_DOUBLE_BUFFER_SUPPORT) */ 
}
...
void mmi_abrepeat_redraw_main_volumebar(void)
{
    ...
    dv = ((U32) g_abrepeat.volume + 1) * (g_abr_mainscr.volumebar.h -
        g_abr_mainscr.volumebar.ind_left) /
        MAX_VOL_LEVEL;
...
#elif defined(__MMI_MAINLCD_320X480__)
	gui_set_clip(x1, y1 - 1, x2, y1 - 1 + dv);
...
*  void
*****************************************************************************/
void mmi_abrepeat_redraw_main_button(U8 btn_idx, MMI_BOOL bPressed)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
void mmi_abrepeat_redraw_main_buttons(void)
{
    ...
    else
    {
        ...
        {
            //g_abrepeat.state != AB_STATE_IDLE, so g_abrepeat.main_selected_button has useful value
        }
    }
}
...
void mmi_abrepeat_exit_mainscreen(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	gdi_set_alpha_blending_source_layer(abrepeat_old_alpha_blending_src_layer);
...
void mmi_abrepeat_redraw_progress_bar(void)
{
    ...
    /*----------------------------------------------------------------*/
    /* to avoid draw progress bar in go back history process. */
    if (IsBackHistory || GetActiveScreenId() != SCR_ID_AB_MAIN)
    {
        return;
    }
...
void mmi_abrepeat_init_main_screen_images(S8 skin)
{
    ...
    g_abr_mainscr.vol_dec.disable_img = (PU8)GetImage(skinab_p->volume_dec_button_disable_img_id);
    ...
    g_abr_mainscr.vol_inc.disable_img = (PU8)GetImage(skinab_p->volume_inc_button_disable_img_id);
...
*  void
*****************************************************************************/
void mmi_abrepeat_pen_abort_hdlr(mmi_pen_point_struct pos) 
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
void mmi_abrepeat_pen_down_hdlr(mmi_pen_point_struct pos)
{
    ...
    if (mmi_abrepeat_pen_check_inside_area(&pos, &g_abr_mainscr.set_mode.area))
    {
        ...
        if (AB_SETTING_MODE == g_abrepeat.cur_mode)
        {
        	  if(g_mmi_abrepeat_section_deleted)
            {
...
* DESCRIPTION
*  Function to handle pen up operation. In this we check if the pen up is one the 
*  same button as the pen down and 
* PARAMETERS
*  pos     [IN]        
* RETURNS
*  void
...
void mmi_abrepeat_pen_up_hdlr(mmi_pen_point_struct pos)
{
    ...
    case AB_PEN_VOLUME_INC:
        if (mmi_abrepeat_pen_check_inside_area(&pos, &g_abr_mainscr.vol_inc.area))
        {
            mmi_abrepeat_press_inc_volume_up();
        }
        break;
    ...
    case AB_PEN_VOLUME_DEC: 
        if (mmi_abrepeat_pen_check_inside_area(&pos, &g_abr_mainscr.vol_dec.area))
        {
            mmi_abrepeat_press_dec_volume_up();
        }
        break;
    }
...
/*****************************************************************************
* FUNCTION
*  mi_abrepet_pen_repeat_hdlr
* DESCRIPTION
*  Function to handle volume repeat inc/dec 
* PARAMETERS
*  pos     [IN]        Pen position
* RETURNS
*  void
*****************************************************************************/
void mmi_abrepet_pen_repeat_hdlr(mmi_pen_point_struct pos)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
* PARAMETERS
*  v_cntrl         [IN]           Inc/Dec button
*  bPressed        [IN]           Pressed/Unpressed state
* RETURNS
*  void
*****************************************************************************/
void mmi_abrepeat_redraw_volumectrl(mmi_abrepeat_volumectrl_type_enum v_cntrl, BOOL bPressed)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    S32 x1, y1, x2, y2;
    abrepeat_up_down_icon_struct *volctrl_p = ((v_cntrl == AB_VOLUMECTRL_INC) ? &g_abr_mainscr.vol_inc : &g_abr_mainscr.vol_dec);
    MMI_BOOL is_disabled = 0;
...
#if !defined(__MMI_TOUCH_SCREEN__) && defined(__MMI_BACKGROUND_CALL__)
    /* in background play the volume keys are set to "call volume" 
     * so if touch screen is not present, there is no way to change the volume
     */
    ...
    if (is_disabled ||
        (v_cntrl == AB_VOLUMECTRL_INC && g_abrepeat.volume == MAX_VOL_LEVEL - 1) ||
        (v_cntrl == AB_VOLUMECTRL_DEC && g_abrepeat.volume == 0))
    {
        gui_show_image(x1, y1, volctrl_p->disable_img);
    }
    else
    {
