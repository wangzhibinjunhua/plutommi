...
{
    U8 subfolder_index;
    const U8 *p_subfolder;
	
} mmi_swflash_subfolder_struct;

typedef enum
{
	
	SWFLASH_SUB_FOLDER_ANIMATION,
		SWFLASH_SUB_FOLDER_GAME, 
		SWFLASH_SUB_FOLDER_KARAOK,
		SWFLASH_SUB_FOLDER_EXTENT,
		SWFLASH_SUB_FOLDER_OTHER,
		SWFLASH_SUB_FOLDER_END
		
}SWFLASH_SUBFOLDER_ID;

static const mmi_swflash_subfolder_struct mmi_swflash_subfolder[] = 
{
    {SWFLASH_SUB_FOLDER_ANIMATION, (const U8 *) L"Animation\\"},
    {SWFLASH_SUB_FOLDER_GAME, (const U8 *) L"Game\\"},
    {SWFLASH_SUB_FOLDER_KARAOK, (const U8 *)  L"KalaOK\\"},
    {SWFLASH_SUB_FOLDER_EXTENT, (const U8 *)  L"Movie\\"},
    {SWFLASH_SUB_FOLDER_OTHER, (const U8 *) L"Other\\"},
    {0,     NULL}
};
...
static MMI_BOOL mmi_swflash_wps_http_hdlr(void *MsgStruct);
static MMI_BOOL mmi_swflash_dl_process_ind_hdlr(void *MsgStruct);
static void mmi_swflash_send_wps_cancel_req(U8 reqeust_id);
static MMI_BOOL mmi_swflash_wps_cancel_hdlr(void *MsgStruct);
static void mmi_swflash_wps_set_channel_req(U8 reqeust_id);
static MMI_BOOL mmi_swflash_wps_set_channel_hdlr(void *MsgStruct);
static BOOL mmi_swflash_send_unset_channel_req(void);
static MMI_BOOL mmi_swflash_unset_channel_hdlr(void *MsgStruct);
...
static void mmi_swflash_entry_wap_push_storage(void);
static void mmi_swflash_replace_from_push_confirm(void);
static void mmi_swflash_exit_replace_confirm_from_wap_push(void);
static void mmi_swflash_save_file_from_wap_push(void);
static void mmi_swflash_get_vis_file_path(S8* filepath);
static void mmi_swflash_get_zdd_file_path(S8* zdd_file_path);
static void mmi_swflash_get_temp_file_path(S8* temp_file_path);
...
U8 imei_key_buf[MMI_SWFLASH_IMEI_LEN];
...
static U16* dl_file_path;
static U16* dl_rename_path;
static U8 g_cbm_app_id;
								 ...
								 PS8 mime_type_string)
{
    ...
    /*----------------------------------------------------------------*/
    /*read IMEI number*/
    raw_handle = FS_Open((const WCHAR *) file_path, FS_READ_ONLY);
    FS_Read(raw_handle, iv, 16, &len);
    FS_Close(raw_handle);
    if (strncmp((const S8*)iv , (PS8)"4074BCC61602B5BE", 16) == 0)
    {
		if( file_path  && strlen((kal_char*)file_path) > 0)
		{  
			dl_file_path = (PU16) OslMalloc(strlen((kal_char*)file_path)+1); 
			mmi_ucs2cpy((S8 *) dl_file_path, (const S8 *) file_path);
			mmi_swflash_entry_wap_push_storage();
			return;
		}
    }
    if (strncmp(url, (PS8)"mms", 3) == 0)
    {
        kal_wsprintf(
        ...
        if (result < 0)
        {
			...
			string_id = mmi_fmgr_util_get_fs_error_info(result, (S32*)&popup_type);
    ...
    nvram_get_imei_value(MMI_SWFLASH_IMEI_LEN, imei_key_buf, 1);
    ...
    {
		mmi_display_popup((UI_string_type)GetString(STR_ID_SWFLASH_ZDD_FILE_INVALID), MMI_EVENT_FAILURE);	
        return;
    }
    else
    {
        ...
        {
            mmi_display_popup((UI_string_type)GetString(STR_ID_SWFLASH_NOTIFY_TOO_BIG), MMI_EVENT_FAILURE);
            return;
        }
...
static MMI_BOOL mmi_swflash_parse_descriptor(S8 *file_path)
{
	 ...
	 if (g_re_download == MMI_FALSE)
	 {
		 ...
		 media_get_ext_buffer_cacheable(MOD_MMI, (void **)&zdd_buf, nsize);
	 ...
	 media_get_ext_buffer_cacheable(MOD_MMI, (void **)&zdd_data, sizeof(zdd_info_structure));
	 
	 /*engine API to get the content file name in GB format*/
	 zdd_size = zdd_get_name(zdd_data, (const char*)zdd_buf);
	 MMI_TRACE(MMI_MEDIA_TRC_G2_APP,MMI_SWFLASH_DL_ZDD_GET_NAME,zdd_size);
	 if (zdd_size < 0)
	 {  
		 if (g_re_download == MMI_FALSE)
		 {
		 ...
		 if(zdd_data)
		 {
			 media_free_ext_buffer(MOD_MMI, (void **)&zdd_data);
			 zdd_data = NULL;
		 }
		 return MMI_FALSE;
	 }
	 ...
	 MMI_TRACE(MMI_MEDIA_TRC_G2_APP,MMI_SWFLASH_DL_ZDD_INIT,zdd_size);
	 if (zdd_size < 0)
	 {
		 if (g_re_download == MMI_FALSE)
		 {
	 ...
	 MMI_TRACE(MMI_MEDIA_TRC_G2_APP,MMI_SWFLASH_DL_ZDD_PARSER,zdd_size);
	 if (zdd_size < 0)
	 {
		 if (g_re_download == MMI_FALSE)
		 {
...
void mmi_swflash_dl_init(void)
{
    ...
    g_cbm_app_id = cbm_register_app_id(STR_ID_SWFLASH_APP, GetRootTitleIcon(MENU_ID_SWFLASH_OPTION_DOWNLOAD)); 
}
...
*  void
*****************************************************************************/
static void mmi_swflash_get_storage_callback(S8 drv_letter)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                                                         */
    /*----------------------------------------------------------------*/
    ...
    if (drv_letter == 0)
    {
		...
		if(dl_file_path)
		{
			OslMfree( dl_file_path ); 
			dl_file_path = NULL;     
		}	
        return;
    }
	dl_rename_path = (PU16) OslMalloc(FMGR_MAX_PATH_LEN * ENCODING_LENGTH + ENCODING_LENGTH); 
    kal_wsprintf(
        (kal_wchar*)dl_rename_path,
        "%c:\\%w",
        drv_letter,
        FMGR_DEFAULT_FOLDER_RECEIVED);
    file_handle = FS_Open((const WCHAR *) dl_rename_path, FS_OPEN_DIR | FS_READ_ONLY);
    if (file_handle < 0)
    {
        result = FS_CreateDir((const WCHAR *) dl_rename_path);
    }
    else
    {
        FS_Close(file_handle);
    ...
    {        
		...
		if(dl_rename_path)
		{
			OslMfree( dl_rename_path ); 
			dl_rename_path = NULL;     
		}	
        return;
    }
    ptr = dl_file_path + mmi_ucs2strlen((const S8*)dl_file_path);
    while (ptr > dl_file_path)
    {
        if (*ptr == L'\\')
        {
            mmi_ucs2cpy((S8*) name_buffer, (S8*) (ptr + 1));
    ...
    mmi_ucs2cat((S8*) dl_rename_path, (const S8*) name_buffer);
    file_exist = mmi_swflash_dl_check_file_exist((S8 *)dl_rename_path);
    if (file_exist == MMI_TRUE)
    {
        mmi_swflash_replace_from_push_confirm();
    }
    else
    { 
        mmi_swflash_save_file_from_wap_push();
    }
}
...
*  mmi_swflash_entry_wap_push_storage
* DESCRIPTION
*  save select the storage screen
* PARAMETERS
*  void
* RETURNS
*  void
...
{  
    ...
    /* Code Body                                                                                               */
    /*----------------------------------------------------------------*/
    EntryNewScreen(SCR_ID_SWFLASH_WAP_PUSH_SELECT_STORSGE, NULL, mmi_swflash_entry_wap_push_storage, NULL);
    #ifdef __PLUTO_MMI_PACKAGE__
    mmi_fmgr_sel_drv_and_enter(APP_FILEMANAGER, mmi_swflash_get_storage_callback, 0);
    #endif
}
...
/*****************************************************************************
* FUNCTION
*   mmi_swflash_exit_replace_confirm_from_wap_push
* DESCRIPTION
*
* PARAMETERS
*   void
...
*   void
*****************************************************************************/
void mmi_swflash_exit_replace_confirm_from_wap_push(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    GenericExitScreen(SCR_ID_SWFLASH_REPLACE_VIEW_FROM_WAP_PUSH, mmi_swflash_replace_from_push_confirm);
...
static void mmi_swflash_replace_from_push_confirm(void)
{
    ...
    /* Code Body                                                                                               */
    /*----------------------------------------------------------------*/
    guiBuffer = GetCurrGuiBuffer(SCR_ID_SWFLASH_REPLACE_VIEW_FROM_WAP_PUSH);
    
    EntryNewScreen(SCR_ID_SWFLASH_REPLACE_VIEW_FROM_WAP_PUSH, mmi_swflash_exit_replace_confirm_from_wap_push, NULL, NULL);
    ...
    ShowCategory165Screen(
		...
		STR_GLOBAL_NO, 
		IMG_GLOBAL_NO, 
		get_string(STR_ID_SWFLASH_FILE_REPLACE),
    ...
    SetLeftSoftkeyFunction(mmi_swflash_save_file_from_wap_push, KEY_EVENT_UP);
...
static void mmi_swflash_save_file_from_wap_push(void)
{
    ...
    /* Code Body                                                                                               */
    /*----------------------------------------------------------------*/
    FS_Delete((const WCHAR *) dl_rename_path);
    result = FS_Move((const WCHAR *) dl_file_path, (const WCHAR *) dl_rename_path,  FS_MOVE_KILL | FS_MOVE_OVERWRITE, NULL,NULL, 0);
    ...
    DeleteScreenIfPresent(SCR_ID_SWFLASH_WAP_PUSH_SELECT_STORSGE);
    DeleteScreenIfPresent(SCR_ID_SWFLASH_REPLACE_VIEW_FROM_WAP_PUSH);
}
...
static void mmi_swflash_get_drive_callback(S8 drv_letter)
{
    ...
    S8 filepath[FMGR_MAX_PATH_LEN * ENCODING_LENGTH + ENCODING_LENGTH];   
    ...
    {
        swflash_content.swflash_info.is_save = MMI_TRUE;
		mmi_display_popup((UI_string_type) GetString(STR_ID_SWFLASH_NOTIFY_DISK_FULL), MMI_EVENT_FAILURE);
        return;
    }
    else
    {  
		mmi_swflash_get_vis_file_path(filepath);
        file_exsit = mmi_swflash_dl_check_file_exist((S8 *)filepath);
...
static void mmi_swflash_send_wps_http_req(U8 request_id, U8 http_method, S8 * url, S8 * post_data, S8 * file_path)
{
    ...
    http_req->channel_id = (U8)swflash_content.swflash_dl.channel_id;
    http_req->request_id = request_id + MMI_SWFLASH_DL_REQUEST_ID_BASE;
    swflash_content.swflash_dl.request_id =  request_id;	
    ...
    mmi_frm_set_protocol_event_handler(MSG_ID_WPS_HTTP_RSP, (PsIntFuncPtr) mmi_swflash_wps_http_hdlr, TRUE);
    mmi_frm_set_protocol_event_handler(MSG_ID_WPS_DL_PROGRESS_IND, (PsIntFuncPtr) mmi_swflash_dl_process_ind_hdlr, TRUE);
#endif
}
...
static MMI_BOOL mmi_swflash_wps_http_hdlr(void * MsgStruct)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	if(p->request_id!= (swflash_content.swflash_dl.request_id + MMI_SWFLASH_DL_REQUEST_ID_BASE))
		return FALSE;
...
static void mmi_swflash_send_wps_cancel_req(U8 request_id)
{
    ...
    cancel_req->channel_id = (U8)swflash_content.swflash_dl.channel_id;
    ...
    mmi_frm_set_protocol_event_handler(MSG_ID_WPS_CANCEL_RSP, (PsIntFuncPtr)mmi_swflash_wps_cancel_hdlr, TRUE);
#endif
}
...
static void mmi_swflash_download_main(S32 status, S32 request_id, S32 result)
{
	...
	case MMI_SWFLASH_SET_CHANNEL_RSP:
		...
		if (result != WPS_OK)
		{
			mmi_display_popup((UI_string_type) GetString(STR_ID_SWFLASH_NOTIFY_NETWORK_ERROR), MMI_EVENT_FAILURE);
			
			DeleteScreenIfPresent(SCR_ID_SWFLASH_PERCENTAGE_BAR);
			return;
		}
		mmi_swflash_get_temp_file_path(filepath);
		mmi_swflash_send_wps_http_req(MMI_SWFLASH_DL_OBJECT_REQUEST, WPS_HTTP_METHOD_GET, (S8 *)swflash_content.swflash_info.object_url, NULL, (S8 *)filepath);
		break;
	...
	case MMI_SWFLASH_DL_PROGRESS_IND:
		...
		else if (swflash_content.swflash_dl.dl_status == WPS_DL_STATUS_COMPLETE)
		{
			...
			swflash_content.swflash_dl.is_dl_finished = MMI_TRUE;
			is_space_enough = mmi_swflash_check_space(swflash_content.swflash_dl.storage_index, swflash_content.swflash_info.size);
			...
			if (is_vis_encrypt == MMI_FALSE)
			{
				...
				mmi_swflash_send_unset_channel_req();
				mmi_display_popup((UI_string_type) GetString(STR_ID_SWFLASH_NOTIFY_FILE_ERROR), MMI_EVENT_FAILURE);
				DeleteScreenIfPresent(SCR_ID_SWFLASH_PERCENTAGE_BAR);
				return;
			}
			...
			/*if complete the dl, the msg from WAP is in queue, but in mmi, the screen is interupt by others, 
			should not show player screen*/
			if(IsScreenPresent(SCR_ID_SWFLASH_PERCENTAGE_BAR))
			{
				break;
			}
...
static MMI_BOOL mmi_swflash_send_dl_progress_res(S32 request_id, S32 seq_number)
{
    ...
    progress_res->channel_id = (U8)swflash_content.swflash_dl.channel_id;
...
static void mmi_swflash_wps_set_channel_req(U8 reqeust_id)
{
    ...
    set_channel_req->app_id = g_cbm_app_id;
    set_channel_req->use_default = KAL_TRUE;
    set_channel_req->req_id = reqeust_id + MMI_SWFLASH_DL_REQUEST_ID_BASE;
    swflash_content.swflash_dl.request_id = reqeust_id;	
    ...
    mmi_frm_set_protocol_event_handler(MSG_ID_WPS_SET_CHANNEL_RSP, (PsIntFuncPtr) mmi_swflash_wps_set_channel_hdlr, TRUE);
#endif
}
...
static MMI_BOOL mmi_swflash_wps_set_channel_hdlr(void * MsgStruct)
{
    ...
    /* Code Body                                                                                               */
    /*----------------------------------------------------------------*/
	if(p->req_id!= (U32)(swflash_content.swflash_dl.request_id + MMI_SWFLASH_DL_REQUEST_ID_BASE))
		return FALSE;
    if (p->result == WPS_OK)
    {
...
static BOOL mmi_swflash_send_unset_channel_req(void)
{
    ...
    if (swflash_content.swflash_dl.channel_id >= 0)
    {
        ...
        unset_channel_req->channel_id = (U8)swflash_content.swflash_dl.channel_id;
        ...
        mmi_frm_set_protocol_event_handler(MSG_ID_WPS_UNSET_CHANNEL_RSP, (PsIntFuncPtr) mmi_swflash_unset_channel_hdlr, TRUE);
...
static MMI_BOOL mmi_swflash_unset_channel_hdlr(void *MsgStruct)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if(p->channel_id != swflash_content.swflash_dl.channel_id )
		return FALSE;
    if (p->result == WPS_OK)
    {
        swflash_content.swflash_dl.channel_id = -1;
...
static MMI_BOOL mmi_swflash_zdd_file_encrypt(U8 *mykey, FS_HANDLE dest, FS_HANDLE source, U32 file_len)
{
    ...
    media_get_ext_buffer_cacheable(MOD_MMI, (void **)&zdd_encrypt_buf, 2048);
    MMI_ASSERT(zdd_encrypt_buf != NULL);
    media_get_ext_buffer_cacheable(MOD_MMI, (void **)&zdd_decrypt_buf, 2048);
    MMI_ASSERT(zdd_decrypt_buf != NULL);
    in = zdd_encrypt_buf;
    out = zdd_decrypt_buf;
	
    che_init(&che_str, CHE_AES);
    che_key_action(&che_str, CHE_SET_KEY, mykey, MMI_SWFLASH_IMEI_LEN);
...
static MMI_BOOL mmi_swflash_vis_file_encrypt(U8 *mykey, FS_HANDLE dest, FS_HANDLE source, U32 file_len)
{
    ...
    /* Code Body                                                                                               */
    /*----------------------------------------------------------------*/
    media_get_ext_buffer_cacheable(MOD_MMI,(void **)&pindata, file_len);
    MMI_ASSERT(pindata != NULL);
    media_get_ext_buffer_cacheable(MOD_MMI,(void **)&poutdata, file_len);
    ...
    success = che_process(&che_str, CHE_AES, CHE_CBC, CHE_ENC, in, out, 16, KAL_FALSE);
	che_deinit(&che_str);
...
static MMI_BOOL mmi_swflash_zdd_file_decrypt(U8 * mykey, FS_HANDLE source, U32 file_size, U8 *encrypt_buf, U8 *decrypt_buf)
{
	...
	if (success == KAL_FALSE)
    {
        /*yingchun add*/ //che_deinit(&che_str);
        return MMI_FALSE;
    }
...
extern MMI_BOOL mmi_swflash_vis_decipher(FS_HANDLE input_handle, const S8 *filename, S32 file_nsize, U8 *outbuf, U16 * type_info, S32 *outsize)
{
    ...
    {
		nvram_get_imei_value(MMI_SWFLASH_IMEI_LEN, imei_key_buf, 1);	
        media_get_ext_buffer_cacheable(MOD_MMI,(void **)&pdata, file_nsize);
        ...
        out = outbuf;
        che_init(&che_str, CHE_AES);
        che_key_action(&che_str, CHE_SET_KEY, imei_key_buf, MMI_SWFLASH_IMEI_LEN);
...
void mmi_swflash_card_plug_out_hdlr(void)
{
    ...
    /* Code Body                                                                                               */
    /*----------------------------------------------------------------*/
    if (FS_GetDevStatus(swflash_content.swflash_dl.storage_index, FS_MOUNT_STATE_ENUM) != FS_NO_ERROR)
    {
...
/*****************************************************************************
* FUNCTION
*  mmi_swflash_dl_enable_interrupt_hdlr
* DESCRIPTION
*  the preprocess function of Jpush interrupt
* PARAMETERS
...
static U8 mmi_swflash_dl_enable_interrupt_hdlr(mmi_frm_int_event_type interrupt_event)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
/*****************************************************************************
* FUNCTION
*  mmi_swflash_dl_post_interrupt_hdlr
* DESCRIPTION
*  the post function of Jpush interrupt
* PARAMETERS
*  inMsg:the message come from framework
* RETURNS
*  void
...
static U8 mmi_swflash_dl_post_interrupt_hdlr(mmi_frm_int_event_type interrupt_event)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
static void mmi_swflash_entry_process_bar(void)
{
    ...
    /* Code Body                                                                                               */
    /*----------------------------------------------------------------*/
    if (CheckIsBackHistory() && !mmi_is_redrawing_bk_screens())
    {
        if (swflash_content.swflash_dl.is_dl_finished && swflash_content.swflash_dl.percentage == 99)
        {
            if (swflash_content.swflash_info.is_preview == MMI_FALSE)
            {
                mmi_swflash_view_confirm();
    ...
    /*kill the Jpush popup screen*/
    mmi_frm_block_interrupt_event_handler(MMI_FRM_INT_MMI_JAVA_TIMEALARM_PUSH, 
		mmi_swflash_dl_enable_interrupt_hdlr,
        mmi_swflash_dl_post_interrupt_hdlr);
}
...
static U8 mmi_swflash_process_bar_delete_callback(void *in_param)
{
    ...
    if (swflash_content.swflash_dl.dl_status != WPS_DL_STATUS_COMPLETE)
    {
        ...
        mmi_frm_clear_protocol_event_handler(MSG_ID_WPS_HTTP_RSP, (PsIntFuncPtr)mmi_swflash_wps_http_hdlr);
        mmi_frm_clear_protocol_event_handler(MSG_ID_WPS_DL_PROGRESS_IND, (PsIntFuncPtr)mmi_swflash_dl_process_ind_hdlr);
    }
    swflash_content.swflash_dl.dl_status = WPS_DL_STATUS_ABORT;
    swflash_content.swflash_dl.is_dl_run = MMI_FALSE;
    return FALSE;
...
void mmi_swflash_option_download(void)
{
    ...
    if (!wap_is_ready())
    {
		mmi_display_popup((UI_string_type) GetString(STR_ID_SWFLASH_NETWORK_NOT_READY), MMI_EVENT_FAILURE);
    ...
    {
		g_re_download = MMI_FALSE;
		mmi_display_popup((UI_string_type) GetString(STR_ID_SWFLASH_ZDD_READ_ONLY), MMI_EVENT_FAILURE);
		return;
    }
    ...
    else
    {
		g_re_download = MMI_FALSE;
        FS_Close(fs_ret);
		string_id = mmi_fmgr_util_get_fs_error_info(fs_ret, (S32*)&popup_type);
		mmi_display_popup((UI_string_type) GetString(string_id), popup_type);     
        return;
    }
...
static MMI_BOOL mmi_swflash_encrypt_vis_file(void)
{
    ...
    /*----------------------------------------------------------------*/
    mmi_swflash_get_vis_file_path(filepath);
    file_exist = mmi_swflash_dl_check_file_exist((S8 *)filepath);
    if (file_exist == MMI_TRUE)
    {
        attr = FS_GetAttributes((U16 *)filepath);
        if (attr & FS_ATTR_READ_ONLY)
        {
			FS_SetAttributes((U16 *)filepath, (U8) (attr & ~(FS_ATTR_READ_ONLY)));
        }        
    }
    vis_encrypt_handle = FS_Open((U16 *)filepath, FS_CREATE | FS_READ_WRITE);
	
    mmi_swflash_get_temp_file_path(filepath);
	
    vis_handle = FS_Open((U16 *)filepath, FS_READ_WRITE);
    ...
    MMI_TRACE(MMI_MEDIA_TRC_G2_APP,MMI_SWFLASH_DL_VIS_ENCRYPT_RESULT,encrypt_res);
    FS_Close(vis_handle);
    FS_Close(vis_encrypt_handle);
    FS_Delete((U16 *)filepath);
    if (encrypt_res == MMI_TRUE)
    {
		mmi_swflash_get_zdd_file_path(filepath);
        FS_Delete((U16 *)filepath);
		
		if(swflash_content.swflash_dl.zdd_re_dl_path != NULL)
			FS_Delete((U16 *)swflash_content.swflash_dl.zdd_re_dl_path);
		
		/* Fion Add, Start */
		mmi_fmgr_copy_new_file_path_after_app_move((PS8)swflash_content.swflash_dl.zdd_re_dl_path);
		/* Fion Add, End */
        return MMI_TRUE;
    }
    else
...
static MMI_BOOL mmi_swflash_encrypt_zdd_file(void)
{
    ...
    dest_handle = FS_Open((PU16)filepath, FS_CREATE | FS_READ_WRITE);
    ...
    {
        MMI_TRACE(MMI_MEDIA_TRC_G2_APP,MMI_SWFLASH_DL_ZDD_ENCRYPT_FAIL);
		mmi_display_popup((UI_string_type) GetString(STR_ID_SWFLASH_ZDD_FILE_SAVE_FAIL), MMI_EVENT_FAILURE);
...
static MMI_BOOL mmi_swflash_check_folder(void)
{
    ...
    /* Code Body                                                                                               */
    /*----------------------------------------------------------------*/
	memset((S8 *)path, 0, FMGR_MAX_PATH_LEN * ENCODING_LENGTH + ENCODING_LENGTH);
	mmi_ucs2cpy((S8*) path, (const S8*)MMI_SWFLASH_TEMP_VIS_PATH_WCHAR); 
	file_handle = FS_Open((U16 *)path, FS_OPEN_DIR | FS_READ_ONLY);
	if (file_handle < 0)
	{
	...
	kal_wsprintf(
		(kal_wchar*) path,
		"%c:\\%w",
		swflash_content.swflash_dl.storage_index,
	...
	for(i=0; i<5;  i++)
	{
		...
		kal_wsprintf(
			...
			swflash_content.swflash_dl.storage_index,
			FMGR_DEFAULT_FOLDER_SWFLASH);
		mmi_ucs2cat((S8*) path, (const S8*)mmi_swflash_subfolder[i].p_subfolder);
...
void mmi_swflash_exit_view_confirm(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    GenericExitScreen(SCR_ID_SWFLASH_PLAYBACK, mmi_swflash_view_confirm);
...
static void mmi_swflash_view_confirm(void)
{
    ...
    ShowCategory66Screen(
        ...
        STR_GLOBAL_NO,
        IMG_GLOBAL_NO,
        (U8*) get_string(STR_ID_SWFLASH_ASK_PLAYBACK),
        mmi_get_event_based_image(MMI_EVENT_QUERY),
        NULL);
...
void mmi_swflash_exit_replace_confirm(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    GenericExitScreen(SCR_ID_SWFLASH_REPLACE_VIEW, mmi_swflash_replace_confirm);
...
static void mmi_swflash_start_download(void)
{
    ...
    if (swflash_content.swflash_dl.channel_id >= 0)
    {
		mmi_display_popup((UI_string_type)GetString(STR_ID_SWFLASH_NETWORK_BUSY), MMI_EVENT_FAILURE);
    ...
    if (file_exist == MMI_TRUE)
    {
        ...
        if (attr & FS_ATTR_READ_ONLY)
        {
			mmi_display_popup((UI_string_type)GetString(FMGR_FS_READ_ONLY_ERROR_TEXT), MMI_EVENT_FAILURE);
    ...
    swflash_content.swflash_dl.is_dl_finished = MMI_FALSE;
    mmi_swflash_wps_set_channel_req(MMI_SWFLASH_SET_CHANNEL_REQ);
#endif
}
...
static void mmi_swflash_entry_player(void)
{
    ...
    {
		mmi_swflash_get_vis_file_path(filepath);
		mmi_swflash_set_file_full_path((S8*)filepath, NULL);
    }
    else
    {
		mmi_swflash_get_temp_file_path(filepath);
...
static void mmi_swflash_get_vis_file_path(S8* filepath)
{
    ...
    if (mmi_ucs2cmp((PS8)swflash_content.swflash_info.type, (PS8)L"vis") == 0)
    {
        mmi_ucs2cat((S8 *)filepath, (const S8 *)mmi_swflash_subfolder[SWFLASH_SUB_FOLDER_ANIMATION].p_subfolder);        
        mmi_ucs2cat((S8 *)filepath, (const S8 *)swflash_content.swflash_info.name);
        mmi_ucs2cat((S8 *)filepath, (PS8)L".vis");
    }
    else if (mmi_ucs2cmp((PS8)swflash_content.swflash_info.type, (PS8)L"kvis") == 0)
    {
        mmi_ucs2cat((S8 *)filepath, (const S8 *)mmi_swflash_subfolder[SWFLASH_SUB_FOLDER_KARAOK].p_subfolder);        
        mmi_ucs2cat((S8 *)filepath, (const S8 *)swflash_content.swflash_info.name);
        mmi_ucs2cat((S8 *)filepath, (PS8)L".kvis");
    }
    else if (mmi_ucs2cmp((PS8)swflash_content.swflash_info.type, (PS8)L"ivis") == 0)
    {
        mmi_ucs2cat((S8 *)filepath, (const S8 *)mmi_swflash_subfolder[SWFLASH_SUB_FOLDER_GAME].p_subfolder);        
        mmi_ucs2cat((S8 *)filepath, (const S8 *)swflash_content.swflash_info.name);
        mmi_ucs2cat((S8 *)filepath, (PS8)L".ivis");
    }
    else if (mmi_ucs2cmp((PS8)swflash_content.swflash_info.type, (PS8)L"msf") == 0)
    {
        mmi_ucs2cat((S8 *)filepath, (const S8 *)mmi_swflash_subfolder[SWFLASH_SUB_FOLDER_EXTENT].p_subfolder);        
        mmi_ucs2cat((S8 *)filepath, (const S8 *)swflash_content.swflash_info.name);
        mmi_ucs2cat((S8 *)filepath, (PS8)L".msf");      
    }
    else
    {
        mmi_ucs2cat((S8 *)filepath, (const S8 *)mmi_swflash_subfolder[SWFLASH_SUB_FOLDER_ANIMATION].p_subfolder);        
...
static void mmi_swflash_get_zdd_file_path(S8* zdd_file_path)
{
    ...
    if (mmi_ucs2cmp((PS8)swflash_content.swflash_info.type, (PS8)L"vis") == 0)
    {
        mmi_ucs2cat((S8 *)zdd_file_path, (const S8 *)mmi_swflash_subfolder[SWFLASH_SUB_FOLDER_ANIMATION].p_subfolder);        
    }
    else if (mmi_ucs2cmp((PS8)swflash_content.swflash_info.type, (PS8)L"kvis") == 0)
    {
        mmi_ucs2cat((S8 *)zdd_file_path, (const S8 *)mmi_swflash_subfolder[SWFLASH_SUB_FOLDER_KARAOK].p_subfolder);        
    }
    else if (mmi_ucs2cmp((PS8)swflash_content.swflash_info.type, (PS8)L"ivis") == 0)
    {
        mmi_ucs2cat((S8 *)zdd_file_path, (const S8 *)mmi_swflash_subfolder[SWFLASH_SUB_FOLDER_GAME].p_subfolder);        
    }
    else if (mmi_ucs2cmp((PS8)swflash_content.swflash_info.type, (PS8)L"msf") == 0)
    {
        mmi_ucs2cat((S8 *)zdd_file_path, (const S8 *)mmi_swflash_subfolder[SWFLASH_SUB_FOLDER_EXTENT].p_subfolder);        
    }
    else
    {
        mmi_ucs2cat((S8 *)zdd_file_path, (const S8 *)mmi_swflash_subfolder[SWFLASH_SUB_FOLDER_ANIMATION].p_subfolder);        
    }
    mmi_ucs2cat((S8 *)zdd_file_path, (const S8 *)swflash_content.swflash_info.name);
    mmi_ucs2cat((S8 *)zdd_file_path, (PS8)L".zdd");
}
...
static void mmi_swflash_get_temp_file_path(S8* temp_file_path)
{
    ...
    /* Code Body                                                                                               */
    /*----------------------------------------------------------------*/
    mmi_ucs2cpy((S8 *)temp_file_path, (const S8 *)MMI_SWFLASH_TEMP_VIS_PATH_WCHAR);     	
    mmi_ucs2cat((S8 *)temp_file_path, (const S8 *)swflash_content.swflash_info.name);
    mmi_ucs2cat((S8 *)temp_file_path, (PS8)L".zdd");
}
