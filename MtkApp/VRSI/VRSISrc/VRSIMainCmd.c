...
extern U16 vrsi_phb_index[MMI_PHB_ENTRIES];
// guoming temp add
extern S32 vrsi_phb_tag_index[MMI_PHB_ENTRIES];
...
void mmi_vrsi_init_central_add_all(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_MEDIA_TRC_G2_APP, MMI_VRSI_FUNC_CENTRAL_ADD_ALL, g_vrsi_cntx.vrsi_state);
        ...
        g_vrsi_cntx.vrsi_state == MMI_VRSI_STATE_ADDING_PAUSE)
    {
        ...
        {
            //failure
            mmi_popup_display_simple((UI_string_type)GetString(STR_ID_VRSI_INIT_FAIL),
        ...
        else
        {
		    ...
		    ret = srv_phb_oplib_build_contact_list(NULL, (U16 *)vrsi_phb_index, MMI_PHB_ENTRIES);
			...
			for (i = 0; i < ret; i++)
			{
					vrsi_phb_tag_index[i] = (S32)vrsi_phb_index[i];
			}
			vrsi_phb_entry_cnt = (U16)ret;
...
*/
    mmi_frm_send_ilm(MOD_MMI, PRT_MMI_VRSI_ADD_CENTRAL_REQ, NULL, NULL);
}
...
void mmi_vrsi_init_central_abort(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_MEDIA_TRC_G2_APP, MMI_VRSI_FUNC_CENTRAL_ABORT, g_vrsi_cntx.vrsi_state);
...
void mmi_vrsi_init_central_reset(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_MEDIA_TRC_G2_APP, MMI_VRSI_FUNC_CENTRAL_RESET, g_vrsi_cntx.vrsi_state);
...
void mmi_vrsi_init_central_change_language_reset(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_MEDIA_TRC_G2_APP, MMI_VRSI_FUNC_CENTRAL_CHANGE_LANG_RESET, g_vrsi_cntx.vrsi_state);
...
void mmi_vrsi_init_central_done(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_MEDIA_TRC_G2_APP, MMI_VRSI_FUNC_CENTRAL_ADD_DONE, g_vrsi_cntx.central_fail);
...
void mmi_vrsi_init_central_count_fail(U16 tag_num, U16 *id_list, S32 *long_list)
{
    ...
    for (i = 0; i < tag_num; i++)
    {
        if (id_list[i] == 0xffff)
        {
            ...
            MMI_TRACE(MMI_MEDIA_TRC_G2_APP, MMI_VRSI_FUNC_CENTRAL_CMD_FAIL, long_list[i], g_vrsi_cntx.central_fail);
        }
    }
}
...
void mmi_vrsi_init_central_sync_tag(void)
{
    ...
    if (tag_num > 0)
    {
        ...
        MMI_TRACE(MMI_MEDIA_TRC_G2_APP, MMI_VRSI_FUNC_CENTRAL_SYNC_TAG, result);
...
void mmi_vrsi_exit_init_processing(void)
{
    ...
    {
        kal_prompt_trace(MOD_MMI, "mmi_vrsi_exit_init_processing3");
        mmi_vrsi_rcg_close();
        return;
    }
    ...
    {
    kal_prompt_trace(MOD_MMI, "mmi_vrsi_exit_init_processing");
        return;
    }
    kal_prompt_trace(MOD_MMI, "mmi_vrsi_exit_init_processing2");
...
mmi_ret mmi_vrsi_event_proc(mmi_event_struct *evt)
{
     return MMI_RET_OK;
}
...
void mmi_vrsi_rcg_pre_entry(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
  //  g_gid_vrsi = GRP_ID_VRSI;
    kal_prompt_trace(MOD_MMI, "mmi_vrsi_rcg_pre_entry");
    if (!mmi_frm_group_is_present(GRP_ID_VRSI))
    {
        mmi_frm_group_create(GRP_ID_ROOT, GRP_ID_VRSI, mmi_vrsi_event_proc, NULL);
        mmi_frm_group_enter(GRP_ID_VRSI, MMI_FRM_NODE_SMART_CLOSE_FLAG);
    }
    
    kal_prompt_trace(MOD_MMI, "mmi_vrsi_rcg_pre_entry2");
    ...
    if (mmi_usb_is_in_mass_storage_mode())
    {
        ...
        mmi_popup_display_simple((UI_string_type)string_id, event_type,GRP_ID_VRSI,0);
        return;
    }
...
//to do:
...
#endif /* __SYNCML_SUPPORT__ */
    kal_prompt_trace(MOD_MMI, "mmi_vrsi_rcg_pre_entry3, enable_falg = %d",g_vrsi_cntx.rcg_enable);
    ...
    if (g_vrsi_cntx.rcg_enable)
    {
       ...
       mmi_frm_scrn_enter(GRP_ID_VRSI, SCR_ID_GLOBAL_DUMMY, NULL, NULL, 0);
        ShowCategoryDummyScreen();
       kal_prompt_trace(MOD_MMI, "mmi_vrsi_rcg_pre_entry4");
        /* Init VR procedure */
        mmi_vrsi_rcg_init(mmi_vrsi_entry_rcg);
    }
    else
    {
        mmi_frm_group_close(GRP_ID_VRSI);
    }
}
...
void mmi_vrsi_rcg_init(void (*entry_func) (void))
{
    ...
    kal_prompt_trace(MOD_MMI,"[VR]mmi_vrsi_rcg_init state = %d",g_vrsi_cntx.vrsi_state);
    ...
    else if (g_vrsi_cntx.vrsi_state == MMI_VRSI_STATE_IDLE)
    {
        ...
        {
            //failure
            mmi_popup_display_simple((UI_string_type)GetString(STR_ID_VRSI_INIT_FAIL), MMI_EVENT_FAILURE, GRP_ID_VRSI, 0);
        }
    }
...
void mmi_vrsi_entry_rcg(void)
{
    ...
    kal_prompt_trace(MOD_MMI, "mmi_vrsi_entry_rcg");
    mdi_audio_suspend_background_play();
    TurnOnBacklight(1);
    entry_ret = mmi_frm_scrn_enter(
       GRP_ID_VRSI, 
    ...
    if (mmi_frm_kbd_is_key_supported(KEY_CSK))
    {
        ...
        SetCenterSoftkeyFunction(mmi_vrsi_entry_help, KEY_EVENT_UP);
    }
    ...
    if (mmi_bt_sco_connect_sco(mmi_vrsi_bt_connect_sco_hdlr))
    {
...
void mmi_vrsi_exit_rcg(void)
{
    ...
    if ((scr_id == SCR_ID_VRSI_RCG_MAIN) ||
        ...
        (scr_id == SCR_ID_VRSI_OPEN_SCUT_PROMPT) )
    {
        return;
    }
...
void mmi_vrsi_rcg_close(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_MEDIA_TRC_G2_APP, MMI_VRSI_FUNC_RCG_CLOSE, g_vrsi_cntx.vrsi_state, g_vrsi_cntx.session_id);
    ...
    MMI_TRACE(MMI_MEDIA_TRC_G2_APP, MMI_VRSI_FUNC_RCG_CLOSE, count, 0);
    {
...
void mmi_vrsi_rcg_start_command(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    kal_prompt_trace(MOD_MMI, "mmi_vrsi_rcg_start_command");
    if (g_vrsi_cntx.vrsi_state == MMI_VRSI_STATE_RUNNING)
    {
        ...
        else if (result == MED_RES_VRSI_ERR_EVALUATE_LIMIT)
        {
            //failure
 ...
 *  void
 *****************************************************************************/
void mmi_vrsi_rcg_play_prompt_callback(mdi_result event, void *user_data)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
BOOL mmi_vrsi_rcg_check_vaild_mode(void)
{
    ...
    {
        mmi_vrsi_util_play_tts((U16*) GetString(STR_ID_VRSI_TTS_INVALID_CMD), mmi_frm_scrn_close_active_id);
        return FALSE;
    }
...
void mmi_vrsi_rcg_read_grammar1_result(void)
{
    ...
    MMI_TRACE(MMI_MEDIA_TRC_G2_APP, MMI_VRSI_FUNC_RCG_GRAMMAR1_RESULT, rcg_result.res_type, rcg_result.pre_cmd,
    ...
    else if (rcg_result.res_type == 3)
    {
        switch (rcg_result.pre_cmd)
        {
            case VRSI_CMD1_NAME_DIAL:
                ...
                if ((rcg_result.post_cmd >= VRSI_CMD1_MOBILE) && (rcg_result.post_cmd <= VRSI_CMD1_FAX3))
                {
    	...
    	//failure
    	mmi_popup_display_simple((UI_string_type)GetString(STR_ID_VRSI_RCG_NOT_FOUND),MMI_EVENT_FAILURE, GRP_ID_VRSI, 0);
}
...
void mmi_vrsi_rcg_read_grammar2_result(void)
{
    ...
    MMI_TRACE(MMI_MEDIA_TRC_G2_APP, MMI_VRSI_FUNC_RCG_GRAMMAR2_RESULT, rcg_result.res_type, rcg_result.pre_cmd,
               rcg_result.post_cmd, rcg_result.tag_num);
    ...
    if (rcg_result.res_type == 1)
    {
        ...
        else if (rcg_result.pre_cmd < VRSI_CMD2_MAX)
        {
            ...
            else
            {
                ...
                else if(rcg_result.pre_cmd == VRSI_CMD2_MOBILE2)
                {
                    mmi_ucs2cpy(str_prompt, GetString(STR_ID_VRSI_GRAMMAR2_PROMPT_MOBILE2));
                }
                else if(rcg_result.pre_cmd == VRSI_CMD2_MOBILE3)
                {
                    mmi_ucs2cpy(str_prompt, GetString(STR_ID_VRSI_GRAMMAR2_PROMPT_MOBILE3));
                }
                else if(rcg_result.pre_cmd == VRSI_CMD2_MOBILE4)
                {
                    mmi_ucs2cpy(str_prompt, GetString(STR_ID_VRSI_GRAMMAR2_PROMPT_MOBILE4));
                }
                else if (rcg_result.pre_cmd == VRSI_CMD2_HOME)
                {
                    mmi_ucs2cpy(str_prompt, GetString(STR_ID_VRSI_GRAMMAR2_PROMPT_HOME));
                }
                else if (rcg_result.pre_cmd == VRSI_CMD2_HOME2)
                {
                    mmi_ucs2cpy(str_prompt, GetString(STR_ID_VRSI_GRAMMAR2_PROMPT_HOME2));
                }
                else if (rcg_result.pre_cmd == VRSI_CMD2_HOME3)
                {
                    mmi_ucs2cpy(str_prompt, GetString(STR_ID_VRSI_GRAMMAR2_PROMPT_HOME3));
                }
                else if (rcg_result.pre_cmd == VRSI_CMD2_BUSINESS)
                {
                    mmi_ucs2cpy(str_prompt, GetString(STR_ID_VRSI_GRAMMAR2_PROMPT_OFFICE));
                }
                else if (rcg_result.pre_cmd == VRSI_CMD2_BUSINESS2)
                {
                    mmi_ucs2cpy(str_prompt, GetString(STR_ID_VRSI_GRAMMAR2_PROMPT_OFFICE2));
                }
                else if (rcg_result.pre_cmd == VRSI_CMD2_BUSINESS3)
                {
                    mmi_ucs2cpy(str_prompt, GetString(STR_ID_VRSI_GRAMMAR2_PROMPT_OFFICE3));
                }
                else if (rcg_result.pre_cmd == VRSI_CMD2_FAX)
                {
                    mmi_ucs2cpy(str_prompt, GetString(STR_ID_VRSI_GRAMMAR2_PROMPT_FAX));
                }
                else if (rcg_result.pre_cmd == VRSI_CMD2_FAX2)
                {
                    mmi_ucs2cpy(str_prompt, GetString(STR_ID_VRSI_GRAMMAR2_PROMPT_FAX2));
                }
                else if (rcg_result.pre_cmd == VRSI_CMD2_FAX3)
                {
                    mmi_ucs2cpy(str_prompt, GetString(STR_ID_VRSI_GRAMMAR2_PROMPT_FAX3));
                }
...
void mmi_vrsi_rcg_read_grammar3_result(void)
{
    ...
    MMI_TRACE(MMI_MEDIA_TRC_G2_APP, MMI_VRSI_FUNC_RCG_GRAMMAR3_RESULT, rcg_result.res_type, rcg_result.pre_cmd,
               rcg_result.post_cmd, rcg_result.tag_num);
...
void mmi_vrsi_rcg_read_grammar4_result(void)
{
    ...
    MMI_TRACE(MMI_MEDIA_TRC_G2_APP, MMI_VRSI_FUNC_RCG_GRAMMAR4_RESULT, rcg_result.res_type, rcg_result.pre_cmd,
               rcg_result.post_cmd, rcg_result.tag_num);
...
void mmi_vrsi_rcg_read_grammar5_result(void)
{
    ...
    MMI_TRACE(MMI_MEDIA_TRC_G2_APP, MMI_VRSI_FUNC_RCG_GRAMMAR5_RESULT, rcg_result.res_type, rcg_result.pre_cmd,
               rcg_result.post_cmd, rcg_result.tag_num);
 ...
 *  void
 *****************************************************************************/
void mmi_vrsi_mdi_callback_hdlr(mdi_result result, void *user_data)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_MEDIA_TRC_G2_APP, MMI_VRSI_FUNC_MDI_CALLBACK_HDLR, result, g_vrsi_cntx.vrsi_state);
...
void mmi_vrsi_result_error_hdlr(void)
{
    ...
    MMI_TRACE(MMI_MEDIA_TRC_G2_APP, MMI_VRSI_FUNC_RESULT_ERROR_HDLR, result, g_vrsi_cntx.curr_scenario,
               g_vrsi_cntx.curr_grammar);
    /*
     * Reach the trial limit of demo version
     */
    ...
    {
        	//failure
        mmi_popup_display_simple((UI_string_type)GetString(STR_GLOBAL_UNFINISHED),MMI_EVENT_FAILURE, GRP_ID_VRSI, 0);
    }
    /*
     *   Unknown error
     */
...
void mmi_vrsi_bt_set_ind_hdlr(void *msg)
{
    ...
    #endif /* defined(__USB_IN_NORMAL_MODE__) */
// to do: check get active scrn id
        ...
        if (g_vrsi_cntx.rcg_enable &&
            ...
            (mmi_frm_scrn_get_active_id() == IDLE_SCREEN_ID) &&
...
void mmi_vrsi_bt_set_ind_cnf(BOOL success)
{
   ...
   // msg_p = (mmi_at_general_res_req_struct*) OslConstructDataPtr(sizeof(mmi_at_general_res_req_struct));
    msg_p = OslAllocDataPtr(mmi_at_general_res_req_struct);
    if (success)
    {
...
*/
//    OslMsgSendExtQueue(&message);
    mmi_frm_send_ilm(MOD_MMI, MSG_ID_MMI_AT_GENERAL_RES_REQ, (oslParaType*) msg_p, NULL);
}
...
void mmi_vrsi_bt_set_req(BOOL is_activate)
{
    ...
    else
    {
        ...
        if ((srv_bt_cm_is_profile_connected(SRV_BT_CM_HFP_CONNECTION) || srv_bt_cm_is_profile_connected(SRV_BT_CM_HSP_CONNECTION)) && g_vrsi_init_by_bt)
        {
            vr_on_off = FALSE;
            g_vrsi_init_by_bt = FALSE;
        }
   ...
   // msg_p = (mmi_l4_bt_set_vr_req_struct*) OslConstructDataPtr(sizeof(mmi_l4_bt_set_vr_req_struct));
    msg_p = OslAllocDataPtr(mmi_l4_bt_set_vr_req_struct);
    if (vr_on_off)
    {
...
*/
//    OslMsgSendExtQueue(&message);
    mmi_frm_send_ilm(MOD_L4C, PRT_MMI_L4_BT_SET_VR_REQ, (oslParaType*) msg_p, NULL);
}
