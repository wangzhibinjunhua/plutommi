...
extern st_m3d_world *m3d_world;
	...
	avatar_osd_layout_struct g_avatar_osd_cntx =
	{
		...
		{TRUE,30, 23, 116, 24},		
		{
    		...
    		{TRUE,25,23,19,17,AVATAR_ICON_STATUS_ENABLE},	    /* top larrow */
    		{TRUE,134,23,19,17,AVATAR_ICON_STATUS_ENABLE}		/* top rarrow */
		}
	};
...
* Feature icon count 
************************************/
/* HEAD, HAIR, EYE, BROW, LIP, DECORATION, BODY */
static kal_int32 g_avatar_count_of_item[AVATAR_TOUCH_TOP_ICON_NUM] = 
{
    3, 5, 10, 6, 10, 4, 2
};
...
#define REC_2_Y2 (149)
	...
	avatar_osd_layout_struct g_avatar_osd_cntx =
	{
		...
		{TRUE,50, 23, 140, 24},
        {
    		...
    		{TRUE,25, 25,19,17,AVATAR_ICON_STATUS_ENABLE},	    /* top larrow */
    		{TRUE,195,25,19,17,AVATAR_ICON_STATUS_ENABLE}		/* top rarrow */
		}
	};
...
#define REC_2_Y2 (230)
	...
	avatar_osd_layout_struct g_avatar_osd_cntx =
	{
        ...
        {TRUE,50, 23, 140, 24},		
        {
            ...
            {TRUE,52, 9,19,17,AVATAR_ICON_STATUS_ENABLE},	        /* top larrow */
            {TRUE,249,9,19,17,AVATAR_ICON_STATUS_ENABLE}	        /* top rarrow */
        }
	};
...
#define MMI_AVATAR_TITLE_OFFSET_Y (9)
...
wgui_fmgr_thumb_image_cache_struct *g_avatar_image_cache_list_buffer;
wgui_fmgr_thumb_image_handle_struct *g_avatar_image_handle_list_buffer;
wgui_fmgr_image_cache_struct *avatar_image_cache_p ;
wgui_fmgr_delay_decode_struct *avatar_delay_decoder_p;
 ...
 * BOOL
 *****************************************************************************/
static void mmi_avatar_delay_decode_image(S32 idx)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/    
    ...
    wgui_fmgr_thumb_image_handle_struct *image_handle;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    /* get filename */
    mmi_avatar_get_filename_by_idx(avatar_delay_decoder_p[idx].item_idx, &buffer_UCS2[0]);
    ...
    gdi_image_util_fit_bbox(
        avatar_delay_decoder_p[idx].box_width - 2,
        avatar_delay_decoder_p[idx].box_height - 2,
        img_width,
        img_height,
        &resized_offset_x,
        &resized_offset_y,
    ...
    draw_pos_x = avatar_delay_decoder_p[idx].box_offset_x + 1 + resized_offset_x;
    draw_pos_y = avatar_delay_decoder_p[idx].box_offset_y + 1 + resized_offset_y;
    resized_width--;
    ...
    {
        /* Get pointer fist */
        image_handle = avatar_image_cache_p->image_handle_list[handle_idx];

        /* Assign value */
        image_handle->is_used = TRUE;
        image_handle->draw_pos_x = draw_pos_x;
        image_handle->draw_pos_y = draw_pos_y;
        image_handle->resized_offset_x = resized_offset_x;
        image_handle->resized_offset_y = resized_offset_y;
        image_handle->resized_width = resized_width;
        image_handle->resized_height = resized_height;
        image_handle->box_offset_x =    avatar_delay_decoder_p[idx].box_offset_x;
        image_handle->box_offset_y =    avatar_delay_decoder_p[idx].box_offset_y;
        image_handle->box_width =       avatar_delay_decoder_p[idx].box_width;
        image_handle->box_height =      avatar_delay_decoder_p[idx].box_height;
        image_handle->icon_offset_x =   avatar_delay_decoder_p[idx].icon_offset_x;
        image_handle->icon_offset_y =   avatar_delay_decoder_p[idx].icon_offset_y;
        image_handle->icon_width =      avatar_delay_decoder_p[idx].icon_width;
        image_handle->icon_height =     avatar_delay_decoder_p[idx].icon_height;
        image_handle->icon_ptr =        avatar_delay_decoder_p[idx].icon_ptr;
        image_handle->menuitem_idx = (U16) avatar_delay_decoder_p[idx].item_idx;
        image_handle->is_hilighted =    avatar_delay_decoder_p[idx].is_hilighted;
        image_handle->ui_style =        avatar_delay_decoder_p[idx].ui_style;
    }
    else
    {
        gdi_layer_blt_previous(0, 0, UI_device_width - 1, UI_device_height - 1);
...
static void mmi_avatar_entry_list_screen(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_avatar_select_path_and_enter(MMI_AVATAR_APP_EDITOR, GetRootTitleIcon(MENU_ID_AVATAR_EDITOR), mmi_avatar_entry_editor_screen);
}
...
static void mmi_avatar_entry_editor_screen_internal(void)
{
    ...
    if (avatar_cntx_p->app_mem_pool==NULL)
    {
        avatar_cntx_p->app_mem_pool 
            = applib_mem_ap_alloc_framebuffer(APPLIB_MEM_AP_ID_AVATAR, MMI_AVATAR_APP_MEMORY_SIZE );
    ...
    /* create layers */
    {
        ...
        * Init OSD layer
        ******************************************************************/
        avatar_cntx_p->osd_layer_buf_ptr = applib_mem_screen_alloc_framebuffer(MMI_AVATAR_OSD_BUFFER_SIZE);
        gdi_layer_create_using_outside_memory(
            0,
            0,
            LCD_WIDTH,
        ...
        * Init texture layer
        ******************************************************************/
        avatar_cntx_p->image_buffer_ptr = applib_mem_screen_alloc_framebuffer(MMI_AVATAR_TEXTURE_BUFFER_SIZE);
    ...
    {       
        ...
        for (icon_idx = AVATAR_TOUCH_HEAD; icon_idx <= AVATAR_TOUCH_BODY; icon_idx ++)
        {
            avatar_cntx_p->item_page[icon_idx] = 0;
        }
  
        avatar_cntx_p->selected_top_object = AVATAR_TOUCH_HEAD;
        avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_HEAD].is_press = AVATAR_ICON_STATUS_CLICKED;
        mmi_avatar_bottom_icon_page_change(0);   
    }
    mmi_avatar_reset_icon();
...
static void mmi_avatar_entry_select_animation_screen(void)
{
    ...
    ******************************************************************/
    ...
    /* set rsk to ENABLE */    
    avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_RSK].is_press = AVATAR_ICON_STATUS_ENABLE;
    mmi_avatar_reset_icon();
    mmi_avatar_draw_osd();
    gdi_layer_push_and_set_active(avatar_cntx_p->drawing_layer_handle);
 ...
 *  void
 *****************************************************************************/
void mmi_avatar_highlight_app(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
void mmi_avatar_list_highlight_hdlr(S32 idx)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
void mmi_avatar_highlight_new(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
void mmi_avatar_highlight_edit(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
void mmi_avatar_highlight_play(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
void mmi_avatar_highlight_delete(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
void mmi_avatar_highlight_delete_all(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
void mmi_avatar_highlight_rename(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
void mmi_avatar_highlight_save(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
void mmi_avatar_highlight_exit(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
static void mmi_avatar_draw_osd(void)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    avatar_touch_obj_enum icon_idx;
    avatar_img_id_enum resource_id;
    pBOOL right_area_enable = TRUE;
    ...
    if (avatar_cntx_p->state == AVATAR_STATE_DISPLAYING || avatar_cntx_p->state == AVATAR_STATE_SELECT_ANIMATION)
    {
        ...
        /* TOP_LARROW, TOP_RARROW */
        resource_id = IMG_ID_AVATAR_TOP_LARROW_ON;
        for (icon_idx = AVATAR_TOUCH_TOP_LARROW; icon_idx <= AVATAR_TOUCH_TOP_RARROW; icon_idx ++)
        {
            if(avatar_osd_cntx_p->touch_object[icon_idx].is_press == AVATAR_ICON_STATUS_ENABLE)
            {
                gdi_image_draw_id(avatar_osd_cntx_p->touch_object[icon_idx].offset_x,
                                  avatar_osd_cntx_p->touch_object[icon_idx].offset_y,
                                  resource_id);
            }
            else if(avatar_osd_cntx_p->touch_object[icon_idx].is_press == AVATAR_ICON_STATUS_CLICKED)
            {
                gdi_image_draw_id(avatar_osd_cntx_p->touch_object[icon_idx].offset_x + 2,
                                  avatar_osd_cntx_p->touch_object[icon_idx].offset_y + 2,
                                  resource_id + 1);
            }
            resource_id += 2;
        }
    ...
    else
    {
        ...
        /* Top icon */
        resource_id = IMG_ID_AVATAR_HEAD_SELECTED;
        for (icon_idx = AVATAR_TOUCH_HEAD; icon_idx <= AVATAR_TOUCH_BODY; icon_idx ++)
        {
            if(avatar_osd_cntx_p->touch_object[icon_idx].is_press == AVATAR_ICON_STATUS_CLICKED)
            {
                gdi_image_draw_id(avatar_osd_cntx_p->touch_object[icon_idx].offset_x,
                                  avatar_osd_cntx_p->touch_object[icon_idx].offset_y,
                                  resource_id);
        ...
        /* Right icon */
        if (right_area_enable)
        {
            /* Magnify, Minify, MoveUp, MoveDown, Stretch, Shrink, LARROW, RARROW */
            resource_id = IMG_ID_AVATAR_MAGNIFY_ON;
            for (icon_idx = AVATAR_TOUCH_MAGNIFY; icon_idx <= AVATAR_TOUCH_RARROW; icon_idx ++)
            {
                if (icon_idx != AVATAR_TOUCH_COLOR)
                {
                    if(avatar_osd_cntx_p->touch_object[icon_idx].is_press == AVATAR_ICON_STATUS_ENABLE)
                    {
                        gdi_image_draw_id(avatar_osd_cntx_p->touch_object[icon_idx].offset_x,
            ...
            /* Color */
            if(avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_COLOR].is_press == AVATAR_ICON_STATUS_ENABLE)
            {
                switch (avatar_cntx_p->selected_top_object)
                {
                    case AVATAR_TOUCH_HEAD:      
                    gdi_image_draw_id(avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_COLOR].offset_x,
                                      avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_COLOR].offset_y,
...
static void mmi_avatar_draw_osd_sofekey(void)
{
    ...
    S32 rsk_offset_y = 0;
    S32 truncated_width;
    MMI_BOOL need_truncated = MMI_FALSE;
    S32 str_width;
    S32 str_height;
    PS8 str_ptr;
    U16 lsk_id = 0;
    ...
    if (avatar_cntx_p->state != AVATAR_STATE_DISPLAYING)
    {
        ...
        need_truncated = wgui_softkey_get_offset(
 ...
 //       }
        ...
        if (avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_LSK].is_press == AVATAR_ICON_STATUS_CLICKED)
        {
            lsk_offset_x++;
            lsk_offset_y++;
        }
 ...
 //           lsk_offset_y );
        ...
        avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_LSK].offset_x = lsk_offset_x;
        avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_LSK].offset_y = lsk_offset_y;
        avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_LSK].width = str_width;
        avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_LSK].height = str_height;
    }
    /* rsk */

    #ifndef __MMI_TOUCH_SCREEN__
    if (((avatar_cntx_p->selected_top_object != AVATAR_TOUCH_HAIR) 
        && !((avatar_cntx_p->selected_top_object == AVATAR_TOUCH_DECORATION) && (avatar_cntx_p->avatar_list[avatar_cntx_p->selected_avatar_id].selected_item[AVATAR_TOUCH_DECORATION] ==0)))
    ...
    #endif
    {
        ...
        if (avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_RSK].is_press == AVATAR_ICON_STATUS_CLICKED)
        {
            rsk_offset_x++;
            rsk_offset_y++;
        }
        ...
        avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_RSK].offset_x = rsk_offset_x;
        avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_RSK].offset_y = rsk_offset_y;
        avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_RSK].width = str_width;
        avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_RSK].height = str_height;
    }
}
...
static void mmi_avatar_draw_bottom_icon(void)
{
    ...
    int icon_idx = avatar_cntx_p->selected_top_object;
    avatar_img_id_enum resource_id;
    
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    start_idx = (avatar_cntx_p->item_page[icon_idx]) * MMI_AVATAR_ITOM_COUNT_PER_PAGE;
    count_of_itom = g_avatar_count_of_item[icon_idx];
    ...
    {
        if (i == avatar_cntx_p->selected_item_idx && avatar_cntx_p->is_highlighted)
        {
            /* decide Y position */
    ...
    /* LARROW, RARROW */
    resource_id = IMG_ID_AVATAR_LARROW_ON;
    for (icon_idx = AVATAR_TOUCH_LARROW; icon_idx <= AVATAR_TOUCH_RARROW; icon_idx ++)
    {
        if(avatar_osd_cntx_p->touch_object[icon_idx].is_press == AVATAR_ICON_STATUS_ENABLE)
        {
            gdi_image_draw_id(avatar_osd_cntx_p->touch_object[icon_idx].offset_x,
...
static S16 mmi_avatar_save_avatar_setting(mmi_avatar_avatar_struct* avatar_list_p,U32 avatar_id)
{
    ...
    U32 written;
    S8 UCS2_buffer[SRV_FMGR_PATH_BUFFER_SIZE*2];
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
...
static void mmi_avatar_load_avatar_default_setting(void)
{
    ...
    /* Reset state as ENABLED */
    for (icon_idx = AVATAR_TOUCH_HEAD; icon_idx <= AVATAR_TOUCH_BODY; icon_idx ++)
    {
        avatar_cntx_p->avatar_list[avatar_cntx_p->selected_avatar_id].selected_item[icon_idx] = 0;
    }
...
static void mmi_avatar_apply_change(S16 old_itom_idx, S16 new_itom_idx)
{
    ...
    {
        case AVATAR_TOUCH_HEAD:     
        node = (st_m3d_node*) M3D_Object3D_find((st_m3d_object3d*)m3d_world, MMI_AVATAR_M3D_HEAD_START_ID + old_itom_idx);
        node2 = (st_m3d_node*) M3D_Object3D_find((st_m3d_object3d*)avatar_cntx_p->m3d_world_head, MMI_AVATAR_M3D_HEAD_START_ID + new_itom_idx);
        parent = (st_m3d_group*) M3D_Object3D_find((st_m3d_object3d*)m3d_world,
                    MMI_AVATAR_M3D_HEAD_PARENT_ID);
        ...
        /* do the change thing */
        /* switch head mesh */
        node = (st_m3d_node*) M3D_Object3D_find((st_m3d_object3d*)m3d_world,MMI_AVATAR_M3D_HAIR_START_ID + old_itom_idx);
        node2 = (st_m3d_node*) M3D_Object3D_find((st_m3d_object3d*)avatar_cntx_p->m3d_world_hair,MMI_AVATAR_M3D_HAIR_START_ID + new_itom_idx);
        parent = (st_m3d_group*) M3D_Object3D_find((st_m3d_object3d*)m3d_world,
            MMI_AVATAR_M3D_HAIR_PARENT_ID);
        ...
        node = (st_m3d_node*) M3D_Object3D_find((st_m3d_object3d*)m3d_world,MMI_AVATAR_M3D_BODY_START_ID + old_itom_idx);
        node2 = (st_m3d_node*) M3D_Object3D_find((st_m3d_object3d*)avatar_cntx_p->m3d_world_body,MMI_AVATAR_M3D_BODY_START_ID + new_itom_idx);
        parent = (st_m3d_group*) M3D_Object3D_find((st_m3d_object3d*)m3d_world,
                    MMI_AVATAR_M3D_BODY_PARENT_ID);
...
static void mmi_avatar_apply_facial_texture_change(void)
{
    ...
    eyeid = avatar_cntx_p->avatar_list[avatar_cntx_p->selected_avatar_id].selected_item[AVATAR_TOUCH_EYE];
    browid = avatar_cntx_p->avatar_list[avatar_cntx_p->selected_avatar_id].selected_item[AVATAR_TOUCH_BROW];
    lipid = avatar_cntx_p->avatar_list[avatar_cntx_p->selected_avatar_id].selected_item[AVATAR_TOUCH_LIP];
    decorationid =avatar_cntx_p->avatar_list[avatar_cntx_p->selected_avatar_id].selected_item[AVATAR_TOUCH_DECORATION];
    headid = avatar_cntx_p->avatar_list[avatar_cntx_p->selected_avatar_id].selected_item[AVATAR_TOUCH_HEAD];
    ...
    node = (st_m3d_node*) M3D_Object3D_find((st_m3d_object3d*)m3d_world,
                                            MMI_AVATAR_M3D_HEAD_START_ID + headid);
    app = M3D_Mesh_getAppearance((st_m3d_mesh *)node, 1);
    
    t2d = app->textures[0];
    i2d = t2d->image;
    /* set ABM source layer handle */
    old_abm_src_layer_handle = gdi_set_alpha_blending_source_layer(avatar_cntx_p->image_buffer_handle);
...
static void mmi_avatar_apply_setting_to_avatar(void)
{
    ...
    dressid = avatar_cntx_p->avatar_list[avatar_cntx_p->selected_avatar_id].selected_dress;
    headid = avatar_cntx_p->avatar_list[avatar_cntx_p->selected_avatar_id].selected_item[AVATAR_TOUCH_HEAD];
    hairid = avatar_cntx_p->avatar_list[avatar_cntx_p->selected_avatar_id].selected_item[AVATAR_TOUCH_HAIR];
    bodyid = avatar_cntx_p->avatar_list[avatar_cntx_p->selected_avatar_id].selected_item[AVATAR_TOUCH_BODY];
    
    /* Head */
    if (headid != 0)
    {
        node = (st_m3d_node*) M3D_Object3D_find((st_m3d_object3d*)m3d_world, MMI_AVATAR_M3D_HEAD_START_ID);
        node2 = (st_m3d_node*) M3D_Object3D_find((st_m3d_object3d*)avatar_cntx_p->m3d_world_head, 
                                                 MMI_AVATAR_M3D_HEAD_START_ID + headid);
        parent = (st_m3d_group*) M3D_Object3D_find((st_m3d_object3d*)m3d_world,
                    2);
    ...
    if (hairid != 0)
    {
        node = (st_m3d_node*) M3D_Object3D_find((st_m3d_object3d*)m3d_world, MMI_AVATAR_M3D_HAIR_START_ID);
        node2 = (st_m3d_node*) M3D_Object3D_find((st_m3d_object3d*)avatar_cntx_p->m3d_world_hair,MMI_AVATAR_M3D_HAIR_START_ID + hairid);
        parent = (st_m3d_group*) M3D_Object3D_find((st_m3d_object3d*)m3d_world,
            1);
    ...
    if (bodyid != 0)
    {
        node = (st_m3d_node*) M3D_Object3D_find((st_m3d_object3d*)m3d_world, MMI_AVATAR_M3D_BODY_START_ID);
        node2 = (st_m3d_node*) M3D_Object3D_find((st_m3d_object3d*)avatar_cntx_p->m3d_world_body,MMI_AVATAR_M3D_BODY_START_ID + bodyid);
        parent = (st_m3d_group*) M3D_Object3D_find((st_m3d_object3d*)m3d_world,
            3);
...
static void mmi_avatar_load_avatar_list(void)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    S32 i, current_idx, head_idx, prev_idx, next_idx, delete_count = 0;
    S16 ret;
    ...
    /* Load avatar list */
    current_idx = avatar_cntx_p->meta_manager.head;

    for (i=0;i<avatar_cntx_p->meta_manager.total_count;i++)
    {

        ret = mmi_avatar_load_avatar_setting(&(avatar_cntx_p->avatar_list[current_idx]), current_idx);
        /* The file does not exit. */
        if (ret <0)
        {
            mmi_avatar_update_setting_mh(avatar_cntx_p->meta_manager.meta_header[current_idx].serial_number);

            /*************************************************
             * Update meta data
             *************************************************/   
            head_idx = avatar_cntx_p->meta_manager.head;
            prev_idx = avatar_cntx_p->meta_manager.meta_header[current_idx].prev;
            next_idx = avatar_cntx_p->meta_manager.meta_header[current_idx].next;

            /* if first one */
            if (current_idx == head_idx)
            {       
                avatar_cntx_p->meta_manager.head = (next_idx ==(S16)0xffff) ? 0 : next_idx;
            }
            else
            {
        ...
        {
            current_idx = avatar_cntx_p->meta_manager.meta_header[current_idx].next;
        }
    }

    avatar_cntx_p->meta_manager.total_count -= delete_count;
    mmi_avatar_save_meta_manager();
}
...
static void mmi_avatar_saving_timeout(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    DisplayPopup((PU8) GetString(STR_GLOBAL_SAVED), 
 ...
 *  void
 *****************************************************************************/
static void mmi_avatar_deleting_timeout(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    DisplayPopup((PU8) GetString(STR_GLOBAL_DELETED), 
                        IMG_GLOBAL_ACTIVATED, 
                        1, 
                        ST_NOTIFYDURATION, 
                        0 
...
static void mmi_avatar_delete_all_avatar_confirm(void)
{
    ...
    DisplayConfirm(
        ...
        STR_GLOBAL_NO,
        IMG_GLOBAL_NO,
        get_string(STR_ID_AVATAR_DELETE_ALL_ASK),
        IMG_GLOBAL_QUESTION,
        WARNING_TONE);
...
static void mmi_avatar_delete_avatar_confirm(void)
{
    ...
    DisplayConfirm(
        ...
        STR_GLOBAL_NO,
        IMG_GLOBAL_NO,
        get_string(STR_ID_AVATAR_DELETE_ASK),
        IMG_GLOBAL_QUESTION,
        WARNING_TONE);
...
static void mmi_avatar_delete_avatar(void)
{
    ...
    if (file_handle >= 0)
    {
        /* clear all keys */
        ClearInputEventHandler(MMI_DEVICE_KEY);
        DisplayPopupCallBack((PU8) GetString(STR_GLOBAL_DELETING), 
                        IMG_GLOBAL_DELETED, 
                        1, 
                        ST_NOTIFYDURATION, 
                        0, 
...
static void mmi_avatar_set_animation(void)
{
    ...
    avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_LSK].is_press = AVATAR_ICON_STATUS_ENABLE;
    mmi_avatar_draw_osd();
    if (avatar_cntx_p->state == AVATAR_STATE_SELECT_ANIMATION)
                ...
                U16 ui_style)
{
    ...
    S32 draw_pos_x, draw_pos_y;
    S32 i, idx;
    FuncPtr decode_func[10] = 
    {
        mmi_avatar_delay_decode_0,
        mmi_avatar_delay_decode_1,
        mmi_avatar_delay_decode_2,
        mmi_avatar_delay_decode_3,
        mmi_avatar_delay_decode_4,
        mmi_avatar_delay_decode_5,
        mmi_avatar_delay_decode_6,
        mmi_avatar_delay_decode_7,
        mmi_avatar_delay_decode_8,
        mmi_avatar_delay_decode_9
    };
    ...
    else
    {
        ...
        idx = avatar_cntx_p->delay_decode_index;

        avatar_delay_decoder_p[idx].item_idx            = item_idx;                               \
        avatar_delay_decoder_p[idx].menuitem_offset_x   = menuitem_offset_x;                      \
        avatar_delay_decoder_p[idx].menuitem_offset_y   = menuitem_offset_y;                      \
        avatar_delay_decoder_p[idx].box_offset_x        = box_offset_x;                           \
        avatar_delay_decoder_p[idx].box_offset_y        = box_offset_y;                           \
        avatar_delay_decoder_p[idx].box_width           = box_width;                              \
        avatar_delay_decoder_p[idx].box_height          = box_height;                             \
        avatar_delay_decoder_p[idx].icon_offset_x       = icon_offset_x;                          \
        avatar_delay_decoder_p[idx].icon_offset_y       = icon_offset_y;                          \
        avatar_delay_decoder_p[idx].icon_width          = icon_width;                             \
        avatar_delay_decoder_p[idx].icon_height         = icon_height;                            \
        avatar_delay_decoder_p[idx].icon_ptr            = icon_ptr;                               \
        avatar_delay_decoder_p[idx].is_hilighted        = is_hilighted;                           \
        avatar_delay_decoder_p[idx].ui_style            = 0; 
        
        /* start delay decode time */    
        StartTimer(AVATAR_DELAY_DECODE_0 + idx, AVATAR_UI_DELAY_DECODE_DURATION, decode_func[idx]);
...
static void mmi_avatar_pen_down_hdlr(mmi_pen_point_struct pos)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    U16 start_idx = 0,count_of_item = 0,current_page = 0;
    S16 i;
    S16 current_item = 0, old_item = 0;
    S32 offset_x,offset_y;
    avatar_touch_obj_enum icon_idx;
    ...
    /* LSK */
    if (mmi_avatar_touch_scr_hit_test(pos.x, pos.y, &(avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_LSK])) == MMI_TRUE)
    {
    ...
    /* RSK */
    if (mmi_avatar_touch_scr_hit_test(pos.x, pos.y, &(avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_RSK])) == MMI_TRUE)
    {
    ...
    if (avatar_cntx_p->state == AVATAR_STATE_DISPLAYING || avatar_cntx_p->state == AVATAR_STATE_SELECT_ANIMATION)
    {
        for (icon_idx = AVATAR_TOUCH_TOP_LARROW; icon_idx <= AVATAR_TOUCH_TOP_RARROW; icon_idx ++)
        {
            /* check if hit */
            if(avatar_osd_cntx_p->touch_object[icon_idx].is_press == AVATAR_ICON_STATUS_ENABLE &&
               mmi_avatar_touch_scr_hit_test(pos.x, pos.y, &(avatar_osd_cntx_p->touch_object[icon_idx])) == MMI_TRUE)
            {
                mmi_avatar_other_icon_press(icon_idx);
                avatar_cntx_p->touch_object = icon_idx;
                return;                
            }
        }
        return;
    ...
    ASSERT(avatar_cntx_p->state != AVATAR_STATE_DISPLAYING && avatar_cntx_p->state != AVATAR_STATE_SELECT_ANIMATION);
    ...
    for (icon_idx = AVATAR_TOUCH_HEAD; icon_idx <= AVATAR_TOUCH_BODY; icon_idx ++)
    {
        ...
        if(avatar_osd_cntx_p->touch_object[icon_idx].is_press == AVATAR_ICON_STATUS_ENABLE &&
           mmi_avatar_touch_scr_hit_test(pos.x, pos.y, &(avatar_osd_cntx_p->touch_object[icon_idx])) == MMI_TRUE)
        {                
            mmi_avatar_top_icon_press(icon_idx);                
            avatar_cntx_p->touch_object = icon_idx;
            return;
        }
    } 
    ...
    /* Bottom icon */
    start_idx = (avatar_cntx_p->item_page[avatar_cntx_p->selected_top_object])*MMI_AVATAR_ITOM_COUNT_PER_PAGE;
    count_of_item = g_avatar_count_of_item[avatar_cntx_p->selected_top_object];
    current_page = avatar_cntx_p->item_page[avatar_cntx_p->selected_top_object];
    current_item = avatar_cntx_p->avatar_list[avatar_cntx_p->selected_avatar_id].selected_item[avatar_cntx_p->selected_top_object]; 
    ...
    for (i=0; i < MMI_AVATAR_ITOM_COUNT_PER_PAGE; i++)
    {
        ...
        else
        {
                ...
                pos.y > offset_y && pos.y < offset_y + MMI_AVATAR_BOTTOM_ICON_SIZE)
            {
                ...
                current_item = current_page * MMI_AVATAR_ITOM_COUNT_PER_PAGE + i;

                /* Update value */
                avatar_cntx_p->avatar_list[avatar_cntx_p->selected_avatar_id].selected_item[avatar_cntx_p->selected_top_object] = current_item; 
                
                avatar_cntx_p->selected_item_idx = i;
                avatar_cntx_p->selected_item_page = current_page;
                ...
                {   
                    mmi_avatar_apply_change(old_item, current_item);
                }
                else
                {
                    mmi_avatar_apply_facial_texture_change();
...
static void mmi_avatar_pen_up_hdlr(mmi_pen_point_struct pos)
{
    ...
    switch(avatar_cntx_p->touch_object)
    {
        ...
        case AVATAR_TOUCH_COLOR:
            avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_COLOR].is_press = AVATAR_ICON_STATUS_ENABLE;
            mmi_avatar_draw_osd();
            gdi_lcd_blt(avatar_cntx_p->osd_layer_handle, avatar_cntx_p->drawing_layer_handle,0,0);   
            break;
        ...
        case AVATAR_TOUCH_LARROW:
            if (avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_LARROW].is_press == AVATAR_ICON_STATUS_CLICKED)
            {
                avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_LARROW].is_press = AVATAR_ICON_STATUS_ENABLE;
        ...
        case AVATAR_TOUCH_RARROW:
            if (avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_RARROW].is_press == AVATAR_ICON_STATUS_CLICKED)
            {
                avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_RARROW].is_press = AVATAR_ICON_STATUS_ENABLE;
                mmi_avatar_bottom_icon_page_change(1);
                mmi_avatar_draw_osd();
                gdi_lcd_blt(avatar_cntx_p->osd_layer_handle, avatar_cntx_p->drawing_layer_handle, 0, 0);   
            }    
...
static void mmi_avatar_set_editor_phase_setting(void)
{
    ...
    if ((avatar_cntx_p->selected_top_object == AVATAR_TOUCH_HAIR) 
        || ((avatar_cntx_p->selected_top_object == AVATAR_TOUCH_DECORATION) && 
            (avatar_cntx_p->avatar_list[avatar_cntx_p->selected_avatar_id].selected_item[AVATAR_TOUCH_DECORATION] ==0)))   
    {
        avatar_cntx_p->is_editor_phase1 = TRUE;
    }
    else
...
/*****************************************************************************
 * FUNCTION
 *  mmi_avatar_update_selected_item
 * DESCRIPTION
 *  update selected item when select prev/next item
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
static void mmi_avatar_selected_item_update(kal_int32 shift)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    avatar_touch_obj_enum icon_idx = avatar_cntx_p->selected_top_object;
    U32 avatar_id = avatar_cntx_p->selected_avatar_id;
    int current_item; 
    ...
    current_item = avatar_cntx_p->avatar_list[avatar_id].selected_item[icon_idx];
    if ((current_item == 0 && shift < 0) || 
        (current_item == g_avatar_count_of_item[icon_idx] - 1 && shift > 0))
    {
        return;
    }        

    /* Update selected element */
    avatar_cntx_p->avatar_list[avatar_id].selected_item[icon_idx] += shift;

    /* Update page */        
    avatar_cntx_p->selected_item_page = avatar_cntx_p->avatar_list[avatar_id].selected_item[icon_idx] / MMI_AVATAR_ITOM_COUNT_PER_PAGE;
    avatar_cntx_p->selected_item_idx = avatar_cntx_p->avatar_list[avatar_id].selected_item[icon_idx] % MMI_AVATAR_ITOM_COUNT_PER_PAGE;
    avatar_cntx_p->item_page[icon_idx] = avatar_cntx_p->selected_item_page;
    avatar_cntx_p->is_highlighted = TRUE;
    
    /* Update left arrow */
    if (avatar_cntx_p->item_page[icon_idx] > 0)
    {
        avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_LARROW].is_press = AVATAR_ICON_STATUS_ENABLE;
    }
    else 
    {
        avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_LARROW].is_press = AVATAR_ICON_STATUS_DISABLE;
    }

    /* Update right arrow */
    if ((1 + avatar_cntx_p->item_page[icon_idx]) * MMI_AVATAR_ITOM_COUNT_PER_PAGE < 
        g_avatar_count_of_item[icon_idx])
    {
        avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_RARROW].is_press = AVATAR_ICON_STATUS_ENABLE;
    }
    else 
    {
        avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_RARROW].is_press = AVATAR_ICON_STATUS_DISABLE;
    }
    ...
    switch(icon_idx)
    {
        ...
        case AVATAR_TOUCH_HAIR:
        case AVATAR_TOUCH_BODY:                        
            mmi_avatar_apply_change(current_item, current_item + shift);
            break;
...
static void mmi_avatar_select_next_item(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_avatar_selected_item_update(1);
}
...
static void mmi_avatar_head_icon_press(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_avatar_top_icon_press(AVATAR_TOUCH_HEAD);
}
...
static void mmi_avatar_hair_icon_press(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_avatar_top_icon_press(AVATAR_TOUCH_HAIR);
}
...
static void mmi_avatar_eye_icon_press(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_avatar_top_icon_press(AVATAR_TOUCH_EYE);
}
...
static void mmi_avatar_brow_icon_press(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_avatar_top_icon_press(AVATAR_TOUCH_BROW);
}
...
static void mmi_avatar_decoration_icon_press(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_avatar_top_icon_press(AVATAR_TOUCH_DECORATION);
}
...
static void mmi_avatar_lip_icon_press(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_avatar_top_icon_press(AVATAR_TOUCH_LIP);
}
...
static void mmi_avatar_body_icon_press(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_avatar_top_icon_press(AVATAR_TOUCH_BODY);
}
...
static void mmi_avatar_stretch_icon_press(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_avatar_other_icon_press(AVATAR_TOUCH_STRETCH);
}
...
static void mmi_avatar_shrink_icon_press(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_avatar_other_icon_press(AVATAR_TOUCH_SHRINK);
}
...
static void mmi_avatar_moveup_icon_press(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_avatar_other_icon_press(AVATAR_TOUCH_MOVEUP);
}
...
static void mmi_avatar_movedown_icon_press(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_avatar_other_icon_press(AVATAR_TOUCH_MOVEDOWN);
}
...
static void mmi_avatar_magnify_icon_press(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_avatar_other_icon_press(AVATAR_TOUCH_MAGNIFY);
}
...
static void mmi_avatar_minify_icon_press(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_avatar_other_icon_press(AVATAR_TOUCH_MINIFY);
}
...
static void mmi_avatar_color_icon_press(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_avatar_other_icon_press(AVATAR_TOUCH_COLOR);    
}
...
static void mmi_avatar_reset_right_icon(void)
{
    ...
    /* Reset state as DISABLE */
    for (icon_idx = AVATAR_TOUCH_MAGNIFY; icon_idx <= AVATAR_TOUCH_COLOR; icon_idx ++)
    {
        avatar_osd_cntx_p->touch_object[icon_idx].is_press = AVATAR_ICON_STATUS_DISABLE;
    }       
}
    ...
    switch(avatar_cntx_p->selected_top_object)
    {
        ...
        case AVATAR_TOUCH_DECORATION:
            if(avatar_cntx_p->avatar_list[avatar_cntx_p->selected_avatar_id].offset_decoration_y > -4)
                avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_MOVEUP].is_press = AVATAR_ICON_STATUS_ENABLE;
            else 
                avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_MOVEUP].is_press = AVATAR_ICON_STATUS_DISABLE;
            if(avatar_cntx_p->avatar_list[avatar_cntx_p->selected_avatar_id].offset_decoration_y < 4)
                avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_MOVEDOWN].is_press = AVATAR_ICON_STATUS_ENABLE;
            else
                avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_MOVEDOWN].is_press = AVATAR_ICON_STATUS_DISABLE;
            
            if (avatar_cntx_p->avatar_list[avatar_cntx_p->selected_avatar_id].s_level_decoration < 0)
                avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_MAGNIFY].is_press = AVATAR_ICON_STATUS_ENABLE;
            else 
                avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_MAGNIFY].is_press = AVATAR_ICON_STATUS_DISABLE;

            if (avatar_cntx_p->avatar_list[avatar_cntx_p->selected_avatar_id].s_level_decoration > -4)
                avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_MINIFY].is_press = AVATAR_ICON_STATUS_ENABLE;
            else 
                avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_MINIFY].is_press = AVATAR_ICON_STATUS_DISABLE;
            break;
        case AVATAR_TOUCH_HAIR:
            //DO NOTHING
            break;
        case AVATAR_TOUCH_EYE:
            ...
            if (avatar_cntx_p->avatar_list[avatar_cntx_p->selected_avatar_id].offset_eye_x > -3)
                avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_STRETCH].is_press = AVATAR_ICON_STATUS_ENABLE;
            else
                avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_STRETCH].is_press = AVATAR_ICON_STATUS_DISABLE;

            if (avatar_cntx_p->avatar_list[avatar_cntx_p->selected_avatar_id].offset_eye_x < 3)
                avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_SHRINK].is_press = AVATAR_ICON_STATUS_ENABLE;
            else 
                avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_SHRINK].is_press = AVATAR_ICON_STATUS_DISABLE;
...
static void mmi_avatar_reset_top_icon(void)
{
    ...
    for (icon_idx = AVATAR_TOUCH_HEAD; icon_idx <= AVATAR_TOUCH_BODY; icon_idx ++)
    {
        avatar_osd_cntx_p->touch_object[icon_idx].is_press = AVATAR_ICON_STATUS_ENABLE;
    } 
}
 ...
 *  mmi_avatar_reset_icon
 * DESCRIPTION
 *  reset icon to enable state if original state is clicked
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
static void mmi_avatar_reset_icon(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Reset state as ENABLED */
    for (icon_idx = AVATAR_TOUCH_MAGNIFY; icon_idx < AVATAR_TOUCH_NUM; icon_idx ++)
    {
        if (avatar_osd_cntx_p->touch_object[icon_idx].is_press == AVATAR_ICON_STATUS_CLICKED)
        {
            avatar_osd_cntx_p->touch_object[icon_idx].is_press = AVATAR_ICON_STATUS_ENABLE;
...
static void mmi_avatar_bottom_icon_page_change(S16 diff)
{
    ...
    /*----------------------------------------------------------------*/
    int icon_idx = avatar_cntx_p->selected_top_object;
    int avatar_id = avatar_cntx_p->selected_avatar_id;
    ...
    avatar_cntx_p->selected_item_idx = avatar_cntx_p->avatar_list[avatar_id].selected_item[icon_idx] % MMI_AVATAR_ITOM_COUNT_PER_PAGE;
#ifdef __MMI_TOUCH_SCREEN__
    avatar_cntx_p->item_page[icon_idx] += diff;
#else
    avatar_cntx_p->item_page[icon_idx] = avatar_cntx_p->selected_item_page;
    
#endif
    if (avatar_cntx_p->selected_item_page == avatar_cntx_p->item_page[icon_idx])
    {
        avatar_cntx_p->is_highlighted = TRUE;
    }
    else
    {
        avatar_cntx_p->is_highlighted = FALSE;
    }
 ...
 *  void
 *****************************************************************************/
static void mmi_avatar_top_icon_press(avatar_touch_obj_enum icon_idx)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /*----------------------------------------------------------------*/
    mmi_avatar_reset_top_icon();
    avatar_cntx_p->selected_top_object = icon_idx;
    avatar_osd_cntx_p->touch_object[icon_idx].is_press = AVATAR_ICON_STATUS_CLICKED;
    mmi_avatar_bottom_icon_page_change(0);
#ifndef __MMI_TOUCH_SCREEN__
    mmi_avatar_set_editor_phase_setting();
#endif
 ...
 *  void
 *****************************************************************************/
static void mmi_avatar_other_icon_press(avatar_touch_obj_enum icon_idx)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (avatar_osd_cntx_p->touch_object[icon_idx].is_press == AVATAR_ICON_STATUS_DISABLE)
        return;
...
static void mmi_avatar_top_larrow_icon_press(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_avatar_other_icon_press(AVATAR_TOUCH_TOP_LARROW);
}
...
static void mmi_avatar_top_rarrow_icon_press(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_avatar_other_icon_press(AVATAR_TOUCH_TOP_RARROW);
}
...
static void mmi_avatar_lsk_press(void)
{
    ...
    avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_LSK].is_press = AVATAR_ICON_STATUS_CLICKED;
    mmi_avatar_draw_osd();
    if (avatar_cntx_p->state == AVATAR_STATE_SELECT_ANIMATION)
        gdi_lcd_blt(avatar_cntx_p->drawing_layer_handle,avatar_cntx_p->osd_layer_handle,0,0);
    else        
...
static void mmi_avatar_rsk_press(void)
{
    ...
    avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_RSK].is_press = AVATAR_ICON_STATUS_CLICKED;
    mmi_avatar_draw_osd();
    if (avatar_cntx_p->state == AVATAR_STATE_DISPLAYING || avatar_cntx_p->state == AVATAR_STATE_SELECT_ANIMATION)
        gdi_lcd_blt(avatar_cntx_p->drawing_layer_handle,avatar_cntx_p->osd_layer_handle,0,0);
    else        
...
static void mmi_avatar_stretch_icon_release(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_STRETCH].is_press == AVATAR_ICON_STATUS_DISABLE)
        return;
...
static void mmi_avatar_shrink_icon_release(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_SHRINK].is_press == AVATAR_ICON_STATUS_DISABLE)
        return;
...
static void mmi_avatar_moveup_icon_release(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_MOVEUP].is_press == AVATAR_ICON_STATUS_DISABLE)
        return;
...
static void mmi_avatar_movedown_icon_release(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_MOVEDOWN].is_press == AVATAR_ICON_STATUS_DISABLE)
        return;
...
static void mmi_avatar_magnify_icon_release(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_MAGNIFY].is_press == AVATAR_ICON_STATUS_DISABLE)
        return;
...
static void mmi_avatar_minify_icon_release(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_MINIFY].is_press == AVATAR_ICON_STATUS_DISABLE)
        return;
...
static void mmi_avatar_top_larrow_icon_release(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_TOP_LARROW].is_press = AVATAR_ICON_STATUS_ENABLE;
    mmi_avatar_draw_osd();
    gdi_lcd_blt(avatar_cntx_p->drawing_layer_handle,avatar_cntx_p->osd_layer_handle,0,0);        
...
static void mmi_avatar_top_rarrow_icon_release(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    avatar_osd_cntx_p->touch_object[AVATAR_TOUCH_TOP_RARROW].is_press = AVATAR_ICON_STATUS_ENABLE;
    mmi_avatar_draw_osd();
    gdi_lcd_blt(avatar_cntx_p->drawing_layer_handle,avatar_cntx_p->osd_layer_handle,0,0);   
...
//    my_time = 0;
    ...
    wgui_fmgr_get_fmgr_cache_struct ((void**)&g_avatar_image_cache_list_buffer,
 ...
 *  void
 *****************************************************************************/
void mmi_avatar_select_path_and_enter (mmi_avatar_app_type_enum app_type, U16 title_icon, avatar_callback func_ptr)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    avatar_cntx_p->title_icon = title_icon;
    mmi_avatar_select_path_and_enter_internal();
}
