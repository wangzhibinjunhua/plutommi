...
static MMI_BOOL is_aborting = MMI_FALSE;
 ...
 *  mmiapi_send_message
 * DESCRIPTION
 *  this function will be called when sending message between MOD.
 * CALLS
 *
 * PARAMETERS
 *  mod_src      [IN]
 ...
 *  void
 *****************************************************************************/
void mmiapi_send_message(module_type mod_src, module_type mod_dest, sap_type msg_sap, msg_type msg_id, local_para_struct *localpara, peer_buff_struct *peerbuff)
{
    struct ilm_struct *ilm_ptr = NULL; 
    ilm_ptr = allocate_ilm(mod_src);

    ilm_ptr->msg_id = msg_id;
    ilm_ptr->local_para_ptr = localpara;
    ilm_ptr->peer_buff_ptr = peerbuff;
...
void mmiapi_sms_send_rsp(srv_sms_callback_struct *result_data)
{
    ...
    if (!IS_BLOCKING(mod_src))
    {
        if (is_aborting)
        {
            is_aborting = MMI_FALSE;
            mmi_frm_send_ilm(mod_src,MSG_ID_MMIAPI_SMS_SEND_ABORT_RSP,NULL,NULL);
        }
        else
        {
	    ...
	    p->vm_id = vm_id;
            mmi_frm_send_ilm(mod_src,MSG_ID_MMIAPI_SMS_SEND_RSP,(local_para_struct*)p,NULL);
        }
    }
...
void mmiapi_sms_send_req(mmiapi_sms_send_req_struct *msg, module_type mod_src)
{
...

    {
        ...
        {
            srv_sms_set_content_dcs(sms_handle, (srv_sms_dcs_enum)(msg->dcs));
        }
        ...
        if (msg->addr.type == MMIAPI_SMS_ADDR_TYPE_INTERNATIONAL)
        {
            address[length] = '+';
            length++;
        }
        ...
        //free_ctrl_buffer(msg->msg); //If free here, it is too earlier so that SMS may send the blank message.
    }
}

/*****************************************************************************
 * FUNCTION
 *  mmiapi_sms_send_abort_req
 * DESCRIPTION
 *
 * CALLS
 *
 ...
 *  void
 *****************************************************************************/
void mmiapi_sms_send_abort_req(mmiapi_sms_send_abort_req_struct *msg, module_type mod_src)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (sending_handle)
    {
        is_aborting = MMI_TRUE;
        srv_sms_abort_send(sending_handle);
    }
    else
    {
        msg_send4(MOD_MMI,mod_src,(sap_type)0,MSG_ID_MMIAPI_SMS_SEND_ABORT_RSP);
    }
}
...
MMI_BOOL mmiapi_sms_ind(srv_sms_event_struct* event_data)
{
    ...
    p->dcs = new_msg->dcs;
    p->con_len = len;
    mod_dest = *((module_type *)(event_data->user_data));
    if (len > (SRV_SMS_MAX_SEG * 160))
    {
        len = SRV_SMS_MAX_SEG * 160;
    }
    if (len > 0)
    {
    ...
    {
        src_mod_id = (module_type)MOD_MMI;
    }
    msg_send5(src_mod_id, mod_dest, INVALID_SAP, MSG_ID_MMIAPI_SMS_IND, (local_para_struct *)p);
    return MMI_TRUE;
}
...
void mmiapi_sms_register_port_req(mmiapi_sms_register_port_req_struct *msg, module_type mod_src)
{
    ...
    if (!IS_BLOCKING(mod_src))
    {
        ...
        if (mod_src == MOD_MMI)
        {   /* Wilson, temp solution to send message to MMI itself. */
            msg_send5(MOD_WAP,mod_src,(sap_type)0,MSG_ID_MMIAPI_SMS_REGISTER_PORT_RSP,(local_para_struct *)p);
        }
        else
        {
           msg_send5(MOD_MMI,mod_src,(sap_type)0,MSG_ID_MMIAPI_SMS_REGISTER_PORT_RSP,(local_para_struct *)p);
        }
    }
...
void mmiapi_sms_get_smsc_addr_rsp(void *data, module_type mod_src, U16 result)
{
    ...
    //{
        ...
        mmi_frm_send_ilm(mod_src,MSG_ID_MMIAPI_SMS_GET_SMSC_ADDR_RSP,(local_para_struct*)p,NULL);
       //}
