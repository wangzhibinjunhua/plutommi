...
MMI_BOOL mmi_fmrdo_hfp_volume_callback(U8 volume, MMI_BOOL is_mute)
{
    ...
    {
        if ((g_fmrdo.in_main_screen == TRUE || g_fmrdo.is_power_on == TRUE) &&
...
void mmi_fmrdo_release_step_down_button(void)
{
    ...
    if (g_fmrdo.is_button_down && g_fmrdo.selected_button == FMRDO_BUTTON_STEP_DOWN)
    {
        ...
        if (g_fmrdo.is_power_on && g_fmrdo.is_search_on)
        {
            ...
            if (is_freq_changed)
	        {
	            mmi_fmrdo_hw_search_frequency_start(freq, FALSE);
			}
			else
			{
            mmi_fmrdo_hw_search_frequency_start(g_fmrdo.frequency, FALSE);
			}
           ...
           if (is_freq_changed)
			...
			{
				mmi_fmrdo_search_frequency_start(g_fmrdo.frequency, FALSE);
...
void mmi_fmrdo_release_step_up_button(void)
{
    ...
    if (g_fmrdo.is_button_down && g_fmrdo.selected_button == FMRDO_BUTTON_STEP_UP)
    {
        ...
        if (g_fmrdo.is_power_on && g_fmrdo.is_search_on)
        {
            ...
            if (is_freq_changed)
	        {
	            mmi_fmrdo_hw_search_frequency_start(freq, TRUE);
			}
			else
			{
            mmi_fmrdo_hw_search_frequency_start(g_fmrdo.frequency, TRUE);
			}
		mmi_fmrdo_update_center_key_status();
        #else /* FM_RADIO_HW_SEARCH */ 
            mmi_fmrdo_redraw_main_channel_name((UI_string_type) GetString(STR_GLOBAL_SEARCHING));     /* show searching text */
          ...
          if(rf_desense_get_curr_mode() != RF_DESENSE_MODE_FM_NO_ANIMATION)
			...
			{
				mmi_fmrdo_search_frequency_start(g_fmrdo.frequency, TRUE);
			}      /* do search in non-blocking mode */
        #endif /* __FM_RADIO_HW_SEARCH__ */ 
        }
        else
...
void mmi_fmrdo_entry_main_record_screen(void)
{
    ...
    mmi_fmrdo_redraw_main_push_button(FMRDO_BUTTON_STEP_DOWN, FALSE);
    mmi_fmrdo_redraw_main_push_button(FMRDO_BUTTON_STEP_UP,FALSE);
#ifndef __MMI_FTE_SUPPORT__
    mmi_fmrdo_redraw_main_push_button(FMRDO_BUTTON_SEARCH_DOWN, FALSE);
    mmi_fmrdo_redraw_main_push_button(FMRDO_BUTTON_SEARCH_UP,FALSE);
#endif
#endif /* __MMI_SLIM_IMG_RES__ */   
 ...
 *  void
 *****************************************************************************/
BOOL mmi_fmrdo_util_check_storage_folder(UI_string_type path, S32 *fs_error, MMI_BOOL show_popup)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
void mmi_fmrdo_twinkle_searching_timer(void)
{
    ...
    if (g_fmrdo.twinkle_frequency_count % 2 == 0)   /* twinkle off */
    {
        mmi_fmrdo_redraw_main_twinkle_searching(FALSE);
        g_fmrdo.twinkle_frequency_count++;
        gui_start_timer(200, mmi_fmrdo_twinkle_searching_timer);
    }
    else    /* twinkle on */
