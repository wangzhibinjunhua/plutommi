    ...
    for (r = 0; r < TOUCH_N_ROWS; r++)
    {
        for (c = 0; c < TOUCH_N_COLS; c++)
        {
            if (IMAGE_SHOW == parallelline_NxMpoint_cntx->gFMPenNByMStatus[r][c])
            {
                ...
                gui_show_image(x, y, (PU8)(wgui_get_list_menu_icon(c, (MMI_ID_TYPE)(IMG_GLOBAL_L1 + c))));               
            }
        }
    }
    gui_pop_clip();
...
extern void mmi_fm_select_pen_n_cross_test(void)
{
   ...
   if (mmi_frm_scrn_enter
    ...
    gui_measure_image((PU8)(wgui_get_list_menu_icon(0, IMG_GLOBAL_L1)), &(parallelline_NxMpoint_cntx->gFMPenNByMImageWidth), &(parallelline_NxMpoint_cntx->gFMPenNByMImageHeight));
