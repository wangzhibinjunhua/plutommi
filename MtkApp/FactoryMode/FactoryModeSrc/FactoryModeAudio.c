...
static void mmi_fm_set_headset(MMI_BOOL HeadsetTestOn, audio_mode_enum mode,kal_bool on, U16 s )
{
    ...
    kal_prompt_trace(MOD_MMI, "mmi_fm_set_headset HeadsetTestOn:%d,mode:%d,on:%d",HeadsetTestOn, mode,on);
    FM_SendSetAudioModeReq(mode);

	/* open loopback */
	kal_sleep_task(kal_milli_secs_to_ticks(800));
	L1SP_SetOutputVolume(196,0);	
    mmi_frm_kbd_set_tone_state(HeadsetTestOn ? MMI_KEY_TONE_DISABLED : MMI_KEY_TONE_ENABLED);
	// mmi_fm_set_loopback(on);
	aud_util_proc_in_med(MOD_MMI,
			mmi_fm_set_loopback,
			on,
...
#define MMI_FM_PEN_CHECK_BOUND(x, y, x1, y1,x2, y2) ((x) >= (x1) && (y) >= (y1) && (x) < (x2) && (y) < (y2))
...
typedef struct
{
    ...
    S32 x2;
    S32 y2;
}mmi_fm_radio_item_sturct;

typedef struct
{
	S16 touch_idx;
	gdi_handle draw_handle;	
	mmi_fm_radio_item_sturct item[4];
	
}mmi_fm_radio_touch_cntx_sturct;

static mmi_fm_radio_touch_cntx_sturct g_fm_radio_touch_cntx;

static void mmi_fm_radio_button_item_redraw(S16 touch_idx, MMI_BOOL IsTouch)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	...
	MMI_ID current_active_grp_id;
	MMI_ID current_active_scrn_id;
	mmi_fm_radio_item_sturct *item;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	mmi_frm_get_active_scrn_id(&current_active_grp_id ,&current_active_scrn_id);
	if (!(current_active_grp_id == g_fm_gourp_id 
		&& current_active_scrn_id == SCR_ID_FM_FM_RADIO_CHANNEL_TEST)) 
		return;
	
	if(touch_idx > 4 || touch_idx < 0)
		return;

	item = &(g_fm_radio_touch_cntx.item[touch_idx]);
	
	gdi_layer_get_base_handle(&base_layer);
	gdi_layer_push_and_set_active(g_fm_radio_touch_cntx.draw_handle);
	gdi_layer_lock_frame_buffer();	
	gdi_layer_push_and_set_clip(item->x1, item->y1, item->x2, item->y2);

	rect_color = gdi_act_color_from_rgb(255, 150, 150, 150);
	
	gdi_draw_solid_rect(item->x1, 
			item->y1, 
			item->x2, 
			item->y2, 
			rect_color);
	...
	if(touch_idx == 0 || touch_idx == 2)	
	{
		gui_measure_string(L"-", &string_w, &string_h);
		gui_move_text_cursor(((LCD_WIDTH + g_fm_radio_touch_cntx.item[touch_idx].x1)/ 2 - string_w) / 2, 
			g_fm_radio_touch_cntx.item[touch_idx].y1 + (30 - string_h) / 2);
		UI_print_text(L"-");
	}
	else
	{
		...
		gui_measure_string(L"+", &string_w, &string_h);
		gui_move_text_cursor(((LCD_WIDTH - g_fm_radio_touch_cntx.item[0].x1)/ 2 + (LCD_WIDTH - string_w)) / 2, 
			g_fm_radio_touch_cntx.item[touch_idx].y1 + (30 - string_h) / 2);
		UI_print_text(L"+");
	}
	...
	gdi_layer_blt(base_layer, g_fm_radio_touch_cntx.draw_handle, NULL, NULL,
					0, 0, LCD_WIDTH, LCD_HEIGHT);

}

static void mmi_fm_radio_pen_up_hdlr(mmi_pen_point_struct point)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	if(g_fm_radio_touch_cntx.touch_idx < 0 || g_fm_radio_touch_cntx.touch_idx > 3)
	{
		wgui_general_pen_up_hdlr(point);
		return;
	}
	switch(g_fm_radio_touch_cntx.touch_idx)
	{
		case 0:
			FmFMRadioLeftArrowHdlr();
			break;
		case 1:
			FmFMRadioRightArrowHdlr();
			break;
		case 2:
			FmFMRadioDownArrowHdlr();
			break;
		case 3:
			FmFMRadioUpArrowHdlr();
			break;
		default:
            ASSERT(0);
	}
...
static void mmi_fm_radio_pen_down_hdlr(mmi_pen_point_struct point)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	...
	for (i = 0 ; i < 4; i++)
	{
		if (MMI_FM_PEN_CHECK_BOUND(point.x, 
			point.y, 
			g_fm_radio_touch_cntx.item[i].x1, 
			g_fm_radio_touch_cntx.item[i].y1, 
			g_fm_radio_touch_cntx.item[i].x2, 
			g_fm_radio_touch_cntx.item[i].y2))
		{
			g_fm_radio_touch_cntx.touch_idx = i;
			mmi_fm_radio_button_item_redraw(i, MMI_TRUE);
			return;
		}
	}
	if( i == 4)
	{
		g_fm_radio_touch_cntx.touch_idx = -1;
...
static void mmi_fm_radio_pen_move_hdlr(mmi_pen_point_struct point)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	...
	i = g_fm_radio_touch_cntx.touch_idx;
	if (!MMI_FM_PEN_CHECK_BOUND(point.x, 
		point.y, 
		g_fm_radio_touch_cntx.item[i].x1, 
		g_fm_radio_touch_cntx.item[i].y1, 
...
static void mmi_fm_draw_radio_scrn()
{    
	/*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    WCHAR* stringbuf;
	S32 string_w, string_h, text_x, text_y;
    fm_fm_radio_struct *fm_fm_radio_cntx;
	gdi_handle base_handle;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	fm_fm_radio_cntx = &(g_fm_contxt->radio_contxt.g_fm_fm_radio_cntx);
	g_fm_radio_touch_cntx.touch_idx = -1;
	text_x = 10;
	text_y = MMI_CONTENT_Y;
	gdi_layer_get_base_handle(&base_handle);

	gdi_layer_set_active(g_fm_radio_touch_cntx.draw_handle);
    ...
    memset(stringbuf, 0, MMI_FM_RADIO_STRING_LEN);

    kal_wsprintf(stringbuf, "Fre:  %d.%d",
        fm_fm_radio_cntx->Currfreq/10, fm_fm_radio_cntx->Currfreq%10);
	
    gui_measure_string(stringbuf, &string_w, &string_h);
	
	text_y +=2;
	
	gui_move_text_cursor((LCD_WIDTH - string_w) / 2, text_y);
	UI_print_text(stringbuf);

	text_y += (string_h + 2);

	g_fm_radio_touch_cntx.item[0].x1 = text_x;
	g_fm_radio_touch_cntx.item[0].y1 = text_y;
	g_fm_radio_touch_cntx.item[0].x2 = (LCD_WIDTH - text_x)/ 2;
	g_fm_radio_touch_cntx.item[0].y2 = text_y + 30;	
	gdi_draw_rect(text_x, text_y, g_fm_radio_touch_cntx.item[0].x2 , g_fm_radio_touch_cntx.item[0].y2, GDI_COLOR_BLACK);

	g_fm_radio_touch_cntx.item[1].x1 = (LCD_WIDTH + text_x)/ 2;
	g_fm_radio_touch_cntx.item[1].y1 = text_y;
	g_fm_radio_touch_cntx.item[1].x2 = LCD_WIDTH - text_x;
	g_fm_radio_touch_cntx.item[1].y2 = text_y + 30;	
	gdi_draw_rect(g_fm_radio_touch_cntx.item[1].x1, 
		g_fm_radio_touch_cntx.item[1].y1, 
		g_fm_radio_touch_cntx.item[1].x2, 
		g_fm_radio_touch_cntx.item[1].y2, 
		GDI_COLOR_BLACK);

	gui_set_font(&MMI_large_font);
	gui_measure_string(L"-", &string_w, &string_h);

	gui_move_text_cursor(((LCD_WIDTH + text_x)/ 2 - string_w) / 2, text_y + (30 - string_h) / 2);
	UI_print_text(L"-");

	gui_measure_string(L"+", &string_w, &string_h);
	gui_move_text_cursor(((LCD_WIDTH - text_x)/ 2 + (LCD_WIDTH - string_w)) / 2, text_y + (30 - string_h) / 2);
	UI_print_text(L"+");
	text_y += 60;

	gui_set_font(&MMI_small_font);
	memset(stringbuf, 0, MMI_FM_RADIO_STRING_LEN);
	kal_wsprintf(stringbuf, "Vol:    %d", fm_fm_radio_cntx->CurrVolume);
	...
	g_fm_radio_touch_cntx.item[2].x1 = text_x;
	g_fm_radio_touch_cntx.item[2].y1 = text_y;
	g_fm_radio_touch_cntx.item[2].x2 = (LCD_WIDTH - text_x)/ 2;
	g_fm_radio_touch_cntx.item[2].y2 = text_y + 30;	
	gdi_draw_rect(text_x, text_y, g_fm_radio_touch_cntx.item[2].x2 , g_fm_radio_touch_cntx.item[2].y2, GDI_COLOR_BLACK);

	g_fm_radio_touch_cntx.item[3].x1 = (LCD_WIDTH + text_x)/ 2;
	g_fm_radio_touch_cntx.item[3].y1 = text_y;
	g_fm_radio_touch_cntx.item[3].x2 = LCD_WIDTH - text_x;
	g_fm_radio_touch_cntx.item[3].y2 = text_y + 30;	
	gdi_draw_rect(g_fm_radio_touch_cntx.item[3].x1, 
		text_y, 
		g_fm_radio_touch_cntx.item[3].x2, 
		g_fm_radio_touch_cntx.item[3].y2, 
		GDI_COLOR_BLACK);
	...
	gdi_layer_blt(base_handle, g_fm_radio_touch_cntx.draw_handle, 0, 0, 0, 0, LCD_WIDTH - 1, LCD_HEIGHT - 1);
	gdi_layer_pop_clip();
	gdi_layer_set_active(base_handle);
	
	OslMfree(stringbuf);	

	mmi_pen_register_up_handler(mmi_fm_radio_pen_up_hdlr);
	mmi_pen_register_down_handler(mmi_fm_radio_pen_down_hdlr);
	mmi_pen_register_move_handler(mmi_fm_radio_pen_move_hdlr);
}
#endif


static void ExitFMRADIOChannelTest(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (g_fm_radio_touch_cntx.draw_handle)
    {
        gdi_layer_free(g_fm_radio_touch_cntx.draw_handle);
        g_fm_radio_touch_cntx.draw_handle = NULL;
    }
#endif
}
...
static void EntryFMRADIOChannelTest(void)
{
	...
	ShowCategory221Screen(
			...
			GDI_COLOR_WHITE,
			NULL);
	gdi_layer_create(0, 0, LCD_WIDTH - 1, LCD_HEIGHT - 1, &g_fm_radio_touch_cntx.draw_handle);

	mmi_fm_draw_radio_scrn();	
