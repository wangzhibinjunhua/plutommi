...
static void mmi_fm_tp_test_draw_result(U32 is_pass, mmi_pen_point_struct *pos)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	if (g_mmi_fm_tp_test_item.layer_handle)
    {
        gdi_layer_free(g_mmi_fm_tp_test_item.layer_handle);
        g_mmi_fm_tp_test_item.layer_handle = 0;
    }
	
    mmi_frm_touch_register_proc(NULL, NULL);    
    if (is_pass)
    {
    	mmi_asc_to_ucs2((PS8) g_fm_contxt->common_contxt.EMFMUnicodeDisplayBuf, (CHAR*) "Pass");
    }
    else
    {
      	mmi_asc_to_ucs2((PS8) g_fm_contxt->common_contxt.EMFMUnicodeDisplayBuf, (CHAR*) "Fail");
    }
	
	ShowCategory7Screen(0, 
		0, 
		STR_GLOBAL_OK, 
		0, 
		STR_GLOBAL_BACK, 
		0, 
		(U8*) g_fm_contxt->common_contxt.EMFMUnicodeDisplayBuf, 
		NULL);
...
/* under construction !*/
#else
	mmi_fm_tp_test_item_enter(GRP_ID_FM_ROOT, MENU_ID_FM_TOUCH_PANEL_LINER_TEST);
