...
void mmi_fm_SaveRemoveAllToNVRAM(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    g_fm_AutoTestListSize = 0;
    g_fm_nvramTestItemArray.count = (U8) g_fm_AutoTestListSize;

	
    /* Shift the list */
	memset(g_fm_nvramTestItemArray.priority, 0, MAX_AUTO_TEST_ITEMS);
...
static MMI_RET mmi_fm_misc_auto_test_confirm_all_proc(mmi_event_struct *evt)
{
    mmi_alert_result_evt_struct *alert = (mmi_alert_result_evt_struct*) evt;
    
    if(MMI_ALERT_CNFM_YES == alert->result)
    {
        mmi_fm_SaveRemoveAllToNVRAM();
    }
    return MMI_RET_OK;
}
...
//    mmi_ret ret = MMI_RET_OK;
    ...
    if (EVT_ID_CUI_MENU_ITEM_SELECT == menu_evt->evt_id)
	{
		...
		case MENU_ID_FM_AUTO_TEST_REMOVE:
			...
			break;
		case MENU_ID_FM_AUTO_TEST_REMOVE_ALL:
            mmi_fm_display_confirm(STR_GLOBAL_DELETE_ALL, mmi_fm_get_root_group(),
                        MMI_EVENT_WARNING,
                        mmi_fm_misc_auto_test_confirm_all_proc);
			break;	
		default:
			break;
		}
    ...
    else if (EVT_ID_CUI_MENU_LIST_ENTRY == menu_evt->evt_id)
    {
        if (0 == g_fm_AutoTestListSize)
        {
			...
			cui_menu_set_item_disabled(menu_evt->sender_id, MENU_ID_FM_AUTO_TEST_REMOVE_ALL, MMI_TRUE);
        }
        else
        {
			...
			cui_menu_set_item_disabled(menu_evt->sender_id, MENU_ID_FM_AUTO_TEST_REMOVE_ALL, MMI_FALSE);
        }
    }
	return MMI_RET_OK;	
}
