...
#ifdef __MMI_BT_ANTI_LOST_BY_RSSI__
		mmi_bt_anti_lost_connect_complete_hdlr(para);
#endif
                break;
            ...
            case SRV_BT_CM_EVENT_DISCONNECT_IND:
				...
				#ifdef __MMI_BT_ANTI_LOST_BY_RSSI__
				mmi_bt_anti_lost_disconn_complete_hdlr(para);
				#endif
                break;
...
#else
                    if(mmi_bt_dialer_get_connecting_flag() || srv_bt_cm_is_profile_connecting())
					{
						mmi_frm_nmgr_popup(
							MMI_SCENARIO_ID_DEFAULT,
							MMI_EVENT_FAILURE,
...
static void* mmi_bt_get_device_image(U32 cod)
{
...
void mmi_bt_entry_bond_result(void *para)
{
    ...
    if (event->result == SRV_BT_CM_RESULT_SUCCESS)
    {
        ...
        {
            MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_CONN_PAIRED_FLAG, g_bt_paired_from_dialer);
            if (MMI_FALSE == mmi_bt_dialer_app_is_connected())
            {
...
{
	#ifdef __MMI_BT_DIALER_SUPPORT__
	if(!mmi_bt_dialer_get_connecting_flag()&&(!mmi_bt_dialer_app_is_connected()))
		{
    mmi_bt_entry_connecting(g_mmi_bt_scr_cntx.mydev_gid);
     kal_prompt_trace(MOD_MMI,"[btconn] mmi_bt_mydev_csk_connect_hdlr......");
    }
    else
	{
	  mmi_frm_nmgr_popup(
...
void mmi_bt_disconnect_complete(void *para)
{
	...
	dev_p = srv_bt_cm_get_dev_info_by_addr(event->dev_addr,SRV_BT_CM_PAIRED_DEV);
...
#endif
#ifdef __MMI_BT_DIALER_SUPPORT__
		if(!mmi_bt_dialer_app_is_connected() && !g_bt_headset_conn_reject)
	{
