...
//extern void mmi_usb_boot_init(void);
extern void srv_usb_detect_ind_hdlr(void *info);
extern void mmi_usb_boot_pwrnon_then_poweron(void);
extern void mmi_usb_boot_show_charge_cmplt(void);
extern void mmi_usb_boot_set_background_still(MMI_BOOL isStill);
...
{
    srv_usb_shutdown_handler(evt);
}

void ShowUsbChgCmpltScr(void)
{
    mmi_usb_boot_show_charge_cmplt();
}

void mmi_usb_set_background_still(MMI_BOOL isStill)
{
    mmi_usb_boot_set_background_still(isStill);
}
