...
void mmi_hidd_init_event_hdlr(void)
{
    ...
    SetProtocolEventHandler(mmi_hidd_unplug_ind_hdlr, MSG_ID_BT_HIDD_UNPLUG_IND);
}
...
void mmi_hidd_connect_cnf_hdlr(void *msg)
{
    ...
    if (msg_p->result == hidd_result_ok)
    {
        ...
        g_hidd_cntx.conn.bd_addr = *((srv_bt_cm_bt_addr *)&msg_p->bt_addr);
...
void mmi_hidd_connect_ind_hdlr(void *msg)
{
    ...
    cnf->result = hidd_result_ok;
    cnf->bt_addr = ind->bt_addr;
    ...
    g_hidd_cntx.last_device = *((srv_bt_cm_bt_addr *)&ind->bt_addr);
...
static void mmi_hid_disc_req_hdlr(srv_bt_cm_bt_addr* addr, U32 conn_id)
{
    ...
    switch (g_hidd_cntx.state)
    {
        ...
        case MMI_BTH_HIDD_STAT_WAITCONNECT:
            ...
            msg->bt_addr.lap = addr->lap;
            msg->bt_addr.uap = addr->uap;
            msg->bt_addr.nap = addr->nap;
...
void mmi_hidd_disconnect_cnf_hdlr(void *msg)
{
    ...
    if (!g_hidd_cntx.conn.active ||
        !HIDD_SAME_DEVICE(g_hidd_cntx.conn.bd_addr, msg_p->bt_addr))
    {
        return;
    }
...
void mmi_hidd_unplug_ind_hdlr(void *msg)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    bt_hidd_unplug_ind_struct *msg_p = (bt_hidd_unplug_ind_struct*)msg;
    ...
    if (HIDD_SAME_DEVICE(msg_p->bt_addr, g_hidd_cntx.last_device))
    {
...
void mmi_hidd_authorize_ind_hdlr(void *msg)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    bt_addr = (srv_bt_cm_bt_addr *)&msg_p->bt_addr;
    ...
    if (srv_bt_cm_dev_property_check(bt_addr, SRV_BT_CM_DEV_PROPERTY_PAIR))
    {
        ...
        srv_bt_cm_get_dev_name((srv_bt_cm_bt_addr*)&msg_p->bt_addr, 
...
static void mmi_hidd_authorize_rsp(srv_bt_cm_bt_addr *addr, U32 prof, U32 result)
{
    ...
    rsp->bt_addr.lap = addr->lap;
    rsp->bt_addr.uap = addr->uap;
    rsp->bt_addr.nap = addr->nap;
    if	(result)
    {
...
void mmi_bth_hidd_interrupt_data_req_hdler(U8 data_len, U8* data_ptr)
{
    ...
    msg_p->bt_addr = *((bt_dm_addr_struct *)&g_hidd_cntx.conn.bd_addr);
...
HIDResult mmi_hidd_connect_last_dev(void)
{
    ...
    switch (g_hidd_cntx.state)
    {
        case MMI_BTH_HIDD_STAT_ACTIVATED:
        {
            ...
            msg->bt_addr = *((bt_dm_addr_struct *)&g_hidd_cntx.last_device);
