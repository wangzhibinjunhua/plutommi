...
extern void mmi_fng_entry_setting_java_screen(void);
extern U32 mmi_my_favorite_javabox_waplink_count(void);
extern void mmi_java_send_cmd_query_req(const kal_wchar*  mids_name, const kal_int32 mids_index, kal_bool extend_mode, java_mmi_cmd_query_rsp_struct **ptrRsp);
...
static kal_int32 Java_CountTimeCmd(kal_int32 argc, kal_char * argv [ ]);
static kal_int32 Java_SnapshotCmd(kal_int32 argc, kal_char * argv [ ]);
static kal_int32 Java_GetVMSolutionCmd(kal_int32 argc, kal_char * argv [ ]);
static kal_int32 Java_GetHardwareVersionCmd(kal_int32 argc, kal_char * argv [ ]);
static kal_int32 Java_GetSoftwareVersionCmd(kal_int32 argc, kal_char * argv [ ]);
static kal_int32 Java_GetChipVersionCmd(kal_int32 argc, kal_char * argv [ ]);
...
//static kal_int32 Java_EntryAppCmd(kal_int32 argc, kal_char * argv [ ]);
/*******************************************************************************/
typedef enum{
    ...
    JAVA_CMD_NAME_COUNTTIME,
    JAVA_CMD_NAME_GETVM,
    JAVA_CMD_NAME_SWVER,
    JAVA_CMD_NAME_HWVER,
    JAVA_CMD_NAME_CHIPVER,
...
static mmi_java_cmd_struct  builtInCmds[]={
    ...
    { "counttime",            Java_CountTimeCmd          },
    { "getvm",                Java_GetVMSolutionCmd      },
    { "swver",                Java_GetSoftwareVersionCmd },
    { "hwver",                Java_GetHardwareVersionCmd },
    { "chipver",              Java_GetChipVersionCmd     },
...
typedef enum {
    ...
    /* Below enum value used for description command running result*/
    JAVA_CMD_JAVABOX_ENTRY_SUCCESS,
    JAVA_CMD_JAVASETTING_ENTRY_SUCCESS,
    JAVA_CMD_SECURITY_ENTRY_SUCCESS,
    JAVA_CMD_UPDATE_ENTRY_SUCCESS,
    JAVA_CMD_RESTORESETTING_ENTRY_SUCCESS,
    JAVA_CMD_SNAPSHOT_SUCCESS,
    JAVA_CMD_PHONEHANG_SUCCESS,
    JAVA_CMD_ClEAR_ALL_SUCCESS,
    JAVA_CMD_FILL_IN_SUCCESS,
    JAVA_CMD_FILL_OUT_SUCCESS,
    JAVA_CMD_FILEDELETED_SUCCESS,
    JAVA_CMD_FILEEXIST_YES,
    JAVA_CMD_FILEEXIST_NO,
    JAVA_CMD_COUNTTIME_MS,
    JAVA_CMD_FREESPACE_SIZE,
    JAVA_CMD_SET_DATETIME_SUCCESS,
    JAVA_CMD_UPLOAD_MMS_SUCCESS,
    JAVA_CMD_DMINSTALL_SUCCESS,
    JAVA_CMD_DMRDELETE_SUCCESS,
    JAVA_CMD_ENTRY_APP_SUCCESS
}Java_cmd_rsp_enum;
...
static const kal_char* Java_cmd_rsp_string[]={
    ...
    "This Application may not exist!"           ,    /*JAVA_CMD_ENTRY_APP_FAILLED*/
    "Entry Java box successfully!"              ,    /*JAVA_CMD_JAVABOX_ENTRY_SUCCESS */
    "Entry Java settings successfully!"         ,    /*JAVA_CMD_JAVASETTINGS_ENTRY_SUCCESS */
    "Entry Java security successfully!"         ,    /*JAVA_CMD_SECURITY_ENTRY_SUCCESS */
    "Update cmd parse successfully!"            ,    /*JAVA_CMD_UPDATE_ENTRY_SUCCESS */
    "RestoreSettings cmd parse successfully!"   ,    /*JAVA_CMD_RESTORESETTING_ENTRY_SUCCESS */
    "Snapshot cmd execute successfully!"        ,    /*JAVA_CMD_SNAPSHOT_SUCCESS */
    "phone and j2me task no hang!"              ,    /*JAVA_CMD_PHONEHANG_SUCCESS*/
    "Clear All mids successfully!"              ,    /*JAVA_CMD_ClEAR_ALL_SUCCESS*/
    "Fill in successfully!"                     ,    /*JAVA_CMD_FILL_IN_SUCCESS*/
    "Fill out successfully!"                    ,    /*JAVA_CMD_FILL_OUT_SUCCESS*/
    "File is deleted successfully!"             ,    /*JAVA_CMD_FILEDELETED_SUCCESS*/
    "File exist!"                               ,    /*JAVA_CMD_FILEEXIST_YES */
    "File No exist!!"                           ,    /*JAVA_CMD_FILEEXIST_NO */
    "Consumed ms"                               ,    /*JAVA_CMD_COUNTTIME_MS*/
    "Byte size of free space!"                  ,    /*JAVA_CMD_FREESPACE_SIZE*/
    "Set vm DateTime successfully!"             ,     /*JAVA_CMD_SET_DATETIME_SUCCESS*/
    "Upload mms file successfully!"             ,     /*JAVA_CMD_UPLOAD_MMS_SUCCESS*/
    "Dm install inject successfully!"             ,     /*JAVA_CMD_UPLOAD_MMS_SUCCESS*/
    "Dm remove inject successfully!"             ,     /*JAVA_CMD_UPLOAD_MMS_SUCCESS*/
    "Entry Application successfuly!"                  /*JAVA_CMD_ENTRY_APP_SUCCESS*/
#endif
};
...
#define JAVA_CMD_RSP(error_code,int_value,string_value) do{\
        MYQUEUE Message;\
        mmi_l4c_java_cmd_rsp_struct *ptrRsp = NULL;\
        ptrRsp = (mmi_l4c_java_cmd_rsp_struct*) OslConstructDataPtr(sizeof(mmi_l4c_java_cmd_rsp_struct));\
        ...
        Message.oslMsgId = MSG_ID_MMI_L4C_JAVA_CMD_RSP;\
        Message.oslDataPtr = (oslParaType*) ptrRsp;\
...
static kal_char *
Java_Strtok_r(kal_char *str, const kal_char *delimiters, kal_char **lasts)
{
    kal_char *ret;
    if (str == NULL)
        str = *lasts;
...
Java_MakeArgv(const kal_char* str, const kal_char* delimiters, kal_char ***argvp)
{
    ...
    if ((tmp = (kal_char*)OslMalloc(sizeof(kal_char)*(strlen(snew) + 1))) == NULL) { 
        *argvp = NULL;
        ...
        strcpy(tmp, snew);
        if (Java_Strtok_r(tmp, delimiters,&lasts) == NULL){
            numtokens = 0;
            OslMfree(tmp);
            return 0;
        }else{
            for (numtokens = 1; Java_Strtok_r(NULL, delimiters,&lasts) != NULL; numtokens++)
                ;  
            ...
            if ((*argvp = (kal_char**)OslMalloc((numtokens + 1)* sizeof(char *))) == NULL) {
                ...
                if (numtokens > 0) {
                    strcpy(tmp, snew); 
                    **argvp = Java_Strtok_r(tmp, delimiters,&lasts);  
                    for (idx = 1; idx < numtokens + 1; idx++)
                    *((*argvp) + idx) = Java_Strtok_r(NULL, delimiters,&lasts);
                }else{           
                    **argvp = NULL; 
                    OslMfree(tmp);
                    return -1;
...
{   
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if(cmd_str!=NULL&&((len = strlen(cmd_str)) > 0))
    {
...
static const kal_wchar * atJava = L"@java";
static const kal_wchar * wtmpFileName = L"javaTest.tmp";
 /****************************************************************************
 * FUNCTION
...
Java_local_install(const kal_int16 drv,const kal_char* filepath )
{
    ...
    len = Java_URLDecodeToUTF8((kal_char *) fileurlptr)+1;
 ...
 Java_JavaBoxCmd(kal_int32 argc, kal_char* argv[])
{
    ...
    /*----------------------------------------------------------------*/
     ...
     EntryScrJavaMenu();
     mids_count = g_java.total_java_app_mids;
     JAVA_CMD_RSP(JAVA_CMD_NO_ERROR,mids_count,Java_cmd_rsp_string[JAVA_CMD_JAVABOX_ENTRY_SUCCESS]);
     return JAVA_CMD_NO_ERROR;
    }
 /*****************************************************************************
 * FUNCTION
 ...
 Java_JavaSettingCmd(kal_int32 argc, kal_char* argv[])
{
    ...
    /*----------------------------------------------------------------*/
     ...
     JAVA_CMD_RSP(JAVA_CMD_NO_ERROR,0,Java_cmd_rsp_string[JAVA_CMD_JAVASETTING_ENTRY_SUCCESS]);
     return JAVA_CMD_NO_ERROR;
        }
 /*****************************************************************************
 * FUNCTION
 ...
 Java_JavaSecurityCmd(kal_int32 argc, kal_char * argv [ ])
{
    ...
    /*----------------------------------------------------------------*/
         ...
         extern void mmi_fng_entry_secuiry_java(void);
#endif
...
#ifdef OGDR_SECURITY_SETTING
     mmi_fng_entry_secuiry_java();
#endif
     JAVA_CMD_RSP(JAVA_CMD_NO_ERROR,0,Java_cmd_rsp_string[JAVA_CMD_SECURITY_ENTRY_SUCCESS]);
     return JAVA_CMD_NO_ERROR;
...
/* under construction !*/
    ...
    ret = mmi_l4c_send_at_cmd_req(pATCmdStr);
    OslMfree(pATCmdStr);
    JAVA_CMD_RSP(JAVA_CMD_NO_ERROR,0,Java_cmd_rsp_string[JAVA_CMD_UPDATE_ENTRY_SUCCESS]);
    return ret;
}
 /****************************************************************************
 * FUNCTION
...
/* under construction !*/
       ...
       ret = mmi_l4c_send_at_cmd_req(pATCmdStr);
       OslMfree(pATCmdStr);
       JAVA_CMD_RSP(JAVA_CMD_NO_ERROR,0,Java_cmd_rsp_string[JAVA_CMD_RESTORESETTING_ENTRY_SUCCESS]);
       return ret;
   }
...
Java_SnapshotCmd(kal_int32 argc, kal_char * argv [ ])
{
    ...
    mmi_asc_to_ucs2((S8 *)wfilepath, (S8 *) filepathname);
...
#if defined(__DYNAMIC_SWITCH_CACHEABILITY__) 
{
...
#ifdef __MTK_TARGET__
    retgdi = gdi_layer_save_jpeg_file (destlayerhandle,(PS8)wfilepath);
#else
    retgdi = gdi_layer_save_bmp_file (destlayerhandle,(PS8)wfilepath);
#endif
    ...
    JAVA_CMD_RSP(JAVA_CMD_NO_ERROR,0,Java_cmd_rsp_string[JAVA_CMD_SNAPSHOT_SUCCESS]);
    return JAVA_CMD_NO_ERROR;
}

/****************************************************************************
 * FUNCTION
 *  Java_GetVMSolutionCmd
 * DESCRIPTION
 *  The implement of GetVM  command. This command is used to get the type of VM current ported on target
 *
 *       getVM
 *      
 *  
 ...
 *****************************************************************************/
static kal_int32 
Java_GetVMSolutionCmd(kal_int32 argc, kal_char * argv [ ])
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    typedef enum {
        ...
        JAVA_VM_IJET,
        JAVA_VM_JBLENDIA
    }Java_vm_solution_enum;
    Java_vm_solution_enum vm_idx = JAVA_VM_UNKNOWN;
    static const kal_char* vm_name[] = {"Unknown","IJET","Jblendia"};
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if(app_stricmp(argv[0],(kal_char *)builtInCmds[JAVA_CMD_NAME_GETVM].cmdName)|| 1!= argc){
        return JAVA_CMD_ARG_ERROR;
    }
    #if defined(__IJET_VM__)
    vm_idx = JAVA_VM_IJET;
    #elif defined(__JBLENDIA__)
    vm_idx = JAVA_VM_JBLENDIA;
    #else
    vm_idx = JAVA_VM_UNKNOWN;
    #endif
    JAVA_CMD_RSP(JAVA_CMD_NO_ERROR,vm_idx,vm_name[vm_idx]);    
    return JAVA_CMD_NO_ERROR;
}
/****************************************************************************
 * FUNCTION
        ...
        /* File check or delete*/
        {
            ...
            mmi_chset_utf8_to_ucs2_string((U8 *)wfilepath,len<<1,(U8 *)(filefullpath));
                if(1==opcode){
                    ...
                    JAVA_CMD_RSP(JAVA_CMD_NO_ERROR,0,Java_cmd_rsp_string[JAVA_CMD_FILEDELETED_SUCCESS]);
                }else{
                if(Java_file_exist((const kal_wchar *) wfilepath)){
                JAVA_CMD_RSP(JAVA_CMD_NO_ERROR,1,Java_cmd_rsp_string[JAVA_CMD_FILEEXIST_YES]);
                }else{                
                JAVA_CMD_RSP(JAVA_CMD_NO_ERROR,0,Java_cmd_rsp_string[JAVA_CMD_FILEEXIST_NO]);/* the file doesn't exist on any driver*/
                }
            }
        }
    } else{
 ...
 *  Java_GetVMSolutionCmd
 * DESCRIPTION
 *  The implement of Java_CountTimeCmd  command. This command is used to get the consuming time between start and stop
 *
 *       countTime start
 *      
 *  
 * [getVM rule]
 *  1.If input cmand is "countTime start" then get and save the current time ticks. 
 *  2.If input cmand is "countTime stop" then get the current time ticks and minus the start's ticks and save it. 
 *  3.Return the consuming time ticks. 
 *
 *  Return 0 on no error or error code.
 *  
 *  Example:
 ...
 *****************************************************************************/
static kal_int32 
Java_CountTimeCmd(kal_int32 argc, kal_char * argv [ ])
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    kal_uint32 currentTicks = 0;
    kal_uint32 consumeTicks = 0;
    kal_uint32 consumeSeconds = 0;
    static kal_uint32 lastTicks = 0;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if(app_stricmp(argv[0],(kal_char*)builtInCmds[JAVA_CMD_NAME_COUNTTIME].cmdName)|| (2!= argc)
        ||((0 == lastTicks) && app_stricmp(argv[1],( char *)"start")
        ||((lastTicks >0) && app_stricmp(argv[1],( char *)"stop")))){
        return JAVA_CMD_ARG_ERROR;
    }
    if((0 == lastTicks) &&(0 == app_stricmp(argv[1],( char *)"start"))){

       kal_get_time((kal_uint32 *)&currentTicks);
       lastTicks = currentTicks;
    }else if((0 < lastTicks) &&(0 == app_stricmp(argv[1],( char *)"stop"))){

       kal_get_time((kal_uint32 *)&currentTicks);
       consumeTicks = currentTicks - lastTicks;
       consumeSeconds = TICK_2_MS(consumeTicks);
       JAVA_CMD_RSP(JAVA_CMD_NO_ERROR,consumeSeconds,Java_cmd_rsp_string[JAVA_CMD_COUNTTIME_MS]);
       lastTicks = 0;
    }else{
        return JAVA_CMD_ARG_ERROR;
    } 
...
/****************************************************************************
 * FUNCTION
 *  Java_GetHardwareVersionCmd
 * DESCRIPTION
 *  The implement of hwver  command. This command is used to get the type of hardware
 *
 *       hwver
 *      
 *  
 ...
 *****************************************************************************/
static kal_int32 
Java_GetHardwareVersionCmd(kal_int32 argc, kal_char * argv [ ])
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    kal_char* hw_ver = NULL;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/

    if((1 != argc)||app_stricmp(argv[0],(kal_char*)builtInCmds[JAVA_CMD_NAME_HWVER].cmdName)) {
        return JAVA_CMD_ARG_ERROR;
    }
    hw_ver = release_hw_ver();
    JAVA_CMD_RSP(JAVA_CMD_NO_ERROR,0,hw_ver);
...
/****************************************************************************
 * FUNCTION
 *  Java_GetChipVersionCmd
 * DESCRIPTION
 *  The implement of chipver  command. This command is used to get the type of hardware
 *
 *       chipver
 *      
 *  
 * [getVM rule]
 *  1.Check args. 
 *  2.Get the version of chip, such as MT6228.
 *  3.send the result to l4c by MSG_ID_MMI_L4C_JAVA_CMD_RSP
 *
 *  Return 0 on no error or error code.
 *  
 ...
 *****************************************************************************/
static kal_int32 
Java_GetChipVersionCmd(kal_int32 argc, kal_char * argv [ ])
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    kal_char* chip_ver = NULL;
    chip_verno_enum chip_ver_no = 0;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/

    if((1 != argc)||app_stricmp(argv[0],(kal_char*)builtInCmds[JAVA_CMD_NAME_CHIPVER].cmdName)) {
        return JAVA_CMD_ARG_ERROR;
    }
    #if (MT6217_SERIES)
        chip_ver = "MT6217_SERIES";
        chip_ver_no = CHIP_VERNO_MT6217;
    #elif(MT6219_SERIES)
        chip_ver = "MT6219_SERIES";
        chip_ver_no = CHIP_VERNO_MT6219;
    #elif(MT6225_SERIES)
        chip_ver = "MT6225_SERIES";
        chip_ver_no = CHIP_VERNO_MT6225;
    #elif(MT6228_SERIES)
        chip_ver = "MT6228_SERIES";
        chip_ver_no = CHIP_VERNO_MT6228;
    #elif(MT6238_SERIES)
        chip_ver = "MT6238_SERIES";
        chip_ver_no = CHIP_VERNO_MT6238;
    #else
        chip_ver = "MoDIS!";
        chip_ver_no = CHIP_VERNO_MODIS;
    #endif
    JAVA_CMD_RSP(JAVA_CMD_NO_ERROR,chip_ver_no,chip_ver);
 ...
 *  Java_GetVMSolutionCmd
 * DESCRIPTION
 *  The implement of swver  command. This command is used to get the type of software version
 *
 *       swver
 *      
 *  
 ...
 *****************************************************************************/
static kal_int32 
Java_GetSoftwareVersionCmd(kal_int32 argc, kal_char * argv [ ])
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    kal_char* sw_ver = NULL;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/

    if((1 != argc)||app_stricmp(argv[0],(kal_char*)builtInCmds[JAVA_CMD_NAME_SWVER].cmdName)) {
        return JAVA_CMD_ARG_ERROR;
    }
    sw_ver = release_verno();
    JAVA_CMD_RSP(JAVA_CMD_NO_ERROR,0,sw_ver);
    return JAVA_CMD_NO_ERROR;
...
Java_PhoneHangCmd(kal_int32 argc, kal_char * argv [ ])
{
    ...
    ret = mmi_java_query_mids_information(NULL, 0);
    memset(&save_mids_infor,0,sizeof(mids_infor_struct));
    JAVA_CMD_RSP(errorCode,0,Java_cmd_rsp_string[JAVA_CMD_PHONEHANG_SUCCESS]);
    return JAVA_CMD_NO_ERROR;
}
...
Java_GetFreeSpaceSizeCmd(kal_int32 argc, kal_char * argv [ ])
{
    ...
    mmi_asc_to_ucs2((S8 *)wfilepath, (S8 *) filepath);
    OslMfree(filepath);
    if (FS_GetDiskInfo((const WCHAR *)wfilepath, &DiskInfo, FS_DI_FREE_SPACE|FS_DI_BASIC_INFO) >= FS_NO_ERROR){
        free_space = DiskInfo.FreeClusters * DiskInfo.SectorsPerCluster* DiskInfo.BytesPerSector;
        OslMfree(wfilepath);
    ...
    JAVA_CMD_RSP(errorCode,free_space,Java_cmd_rsp_string[JAVA_CMD_FREESPACE_SIZE]);
    return JAVA_CMD_NO_ERROR;
}
/****************************************************************************
 * FUNCTION
...
Java_FillDiskDrvCmd(kal_int32 argc, kal_char * argv [ ])
{
    ...
    app_ucs2_strcpy((kal_int8*)wpath,(const kal_int8*)drv);
    app_ucs2_strcat((kal_int8*)wpath,(const kal_int8*)L":\\");
    app_ucs2_strcat((kal_int8*)wpath,(const kal_int8*)atJava);
    app_ucs2_strcat((kal_int8*)wpath,(const kal_int8*)L"\\");
    app_ucs2_strcat((kal_int8*)wpath,(const kal_int8*)wtmpFileName);
    if(0 == mode){
        ...
        if(free_space < left_size){
            ...
            if (offset >0 && FS_Seek(filehandler, offset, FS_FILE_BEGIN) >= 0){
                ...
                JAVA_CMD_RSP(JAVA_CMD_NO_ERROR,0,Java_cmd_rsp_string[JAVA_CMD_FILL_IN_SUCCESS]);
                ret = JAVA_CMD_NO_ERROR;
            }else{
                ret = JAVA_CMD_ERROR;
            }
        ...
        FS_Delete((const WCHAR*)wpath);
        JAVA_CMD_RSP(JAVA_CMD_NO_ERROR,0,Java_cmd_rsp_string[JAVA_CMD_FILL_OUT_SUCCESS]);
    }else{
...
#ifdef __MTK_TARGET__
{
    ...
    applib_dt_utc_sec_2_mytime(u32System, (applib_time_struct *)&utcSystime, KAL_TRUE);
            ...
            JAVA_CMD_RSP(JAVA_CMD_NO_ERROR,0,Java_cmd_rsp_string[JAVA_CMD_SET_DATETIME_SUCCESS]);
            return JAVA_CMD_NO_ERROR;
        }
...
PARSE_DATE:
    /*Parse Year */
    preptr = Java_Strtok_r((kal_char *) strdatetime,(const kal_char *)"/ " ,(kal_char ** )&postptr);
    if(NULL!= preptr){
        kal_int32 intTmp =0;
        ...
        /*Parse Month */
        preptr = Java_Strtok_r(NULL,(const kal_char *)"/" ,(kal_char ** )&postptr);
        if(NULL!=preptr){
...
#endif
	JAVA_CMD_RSP(JAVA_CMD_NO_ERROR,0,Java_cmd_rsp_string[JAVA_CMD_UPLOAD_MMS_SUCCESS]);
...
Java_DmdeleteMidsCmd(kal_int32 argc,kal_char * argv [ ])
{
    ...
    if(mode==0){        
        scomo_info->display_mode =MMI_DMUI_APP_DISPLAY_MODE_DEFAULT;
    }else if(mode == 1){
        scomo_info->display_mode = MMI_DMUI_APP_DISPLAY_MODE_CONFIRM;
    }else if(mode == 2){
        scomo_info->display_mode = MMI_DMUI_APP_DISPLAY_MODE_BACKGROUND;
    }else{
        OslMfree(scomo_info);
        return JAVA_CMD_ARG_ERROR;
    }    
...
#endif
    JAVA_CMD_RSP(JAVA_CMD_NO_ERROR,0,Java_cmd_rsp_string[JAVA_CMD_ClEAR_ALL_SUCCESS]);
    return JAVA_CMD_NO_ERROR;
}
...
Java_LaunchMidCmd(kal_int32 argc, kal_char* argv[])
{
    ...
    if(Java_IsAciiNumeric((const kal_char*)pInStr)){
        ...
        for(midlet_idx = 0; midlet_idx < midlet_cnt;midlet_idx++){
            ...
            if(0 == mmi_ucs2icmp((const S8 *) pInMidletName, (const S8 *) pMidletName)){
               break;
            }
        }
