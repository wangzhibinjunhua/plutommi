...
void mmi_vrsdredial_del_tag(void)
{
    ...
    {
        if (TRUE == mmi_vrsd_del_tag_req(tag_id))
        {
            g_vrsdredial_status = 0; //GUOMING
        }
    }
    else
    {
...
void mmi_vrsdredial_rcg_read_dialed_num_req(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
#if !defined(__MMI_CH_MIXED_CALL_LOG__) && !defined(__MMI_CH_CALL_LOG_SAVE_IN_MEMORY__)
...
void mmi_vrsdredial_rcg_read_dialed_num_rsp(void *info)
{
    ...
    else
    {
    	//failure
		mmi_display_popup(
			(UI_string_type)GetString(STR_ID_VRSD_REDIAL_CALLLOG_EMPTY),
			MMI_EVENT_FAILURE);
...
void mmi_vrsdredial_entry_rcg_found(void)
{
    ...
    /*----------------------------------------------------------------*/
    U8 *guiBuffer;
    S8 str_found[(MMI_PHB_NAME_LENGTH + MAX_CC_ADDR_LEN + 30) * ENCODING_LENGTH];
...
void mmi_vrsdredial_rcg_auto_dial(void)
{
    ...
    if (chis_p->dialedCalls[0].numLength)
    {
        ...
        make_call_para.ucs2_num_uri = (U16*)chis_p->dialedCalls[0].number;
