...
U16 g_mmi_java_scrn_delete_evt_id = EVT_ID_SCENARIO_MAX;
U32 g_mmi_java_task_switch_parent = GRP_ID_ROOT;
extern void jam_enter_terminating_mode(kal_int32 vm_id); 
#ifdef __PLUTO_MMI_PACKAGE__
/*****************************************************************************
 * FUNCTION
...
static PU8 format_asking_string(U16 string_id)
{
    ...
    asking_string_buffer = (kal_wchar*) get_ctrl_buffer((string_len + mmi_ucs2strlen((const CHAR*) mid_name) + 2) << 1);
    mmi_ucs2cpy((CHAR*) asking_string_buffer, GetString(string_id));
    mmi_ucs2cpy((CHAR*) (asking_string_buffer + string_len), (const CHAR*) mid_name);
    string_len += mmi_ucs2strlen((const CHAR*) mid_name);
 ...
 *****************************************************************************/
mmi_ret mmi_java_task_switch_proc(mmi_event_struct *evt)
{
...
#else /* no define J2ME_SUPPORT_BACKGROUND */
...
void mmi_java_resume_dialog_yes_hdlr(void)
{
    ...
    else if (mmi_java_highlight_index == 1)   /* terminate */
    {
        jam_enter_terminate_mode(g_mmi_java_current_selected_vm_id);
        g_mmi_java_current_selected_vm_id = -1;
        mmi_frm_group_close(GRP_ID_JAVA_TASK_SWITCH);
...
void mmi_java_entry_resume_dialog_ext(void)
{
    ...
    text_items[1] = (PU8) GetString(STR_JAVA_TERMINATE);
    if (g_mmi_java_task_switch_parent == GRP_ID_JAVA_TASK_MANAGER)
    {
        text_items[2] = (PU8) GetString(STR_JAVA_TERMINATE_ALL);
        items_number = 3;
    }
...
void mmi_java_entry_resume_dialog(U32 parent) /* called by MMI */
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    g_mmi_java_task_switch_parent = parent;
    mmi_frm_group_create_ex(
        parent,
        GRP_ID_JAVA_TASK_SWITCH,
        mmi_java_task_switch_proc,
...
void mmi_java_entry_force_resume_dialog_int(void)
{
    ...
    //guiBuffer = GetCurrGuiBuffer(SCR_JAVA_RESUME_NOTIFY);
    ...
    ChangeCenterSoftkey(0, IMG_GLOBAL_COMMON_CSK);
#endif
    //SetDelScrnIDCallbackHandler(SCR_JAVA_RESUME_NOTIFY,(HistoryDelCBPtr)mmi_java_force_resume_dialog_del_cb);
...
/*****************************************************************************
 * FUNCTION
 *  mmi_java_force_resume_dialog_proc
 * DESCRIPTION
 * 
 * PARAMETERS
 *  evt     [IN]        The event pass in
 ...
 *  mmi_ret [OUT] the result of event handling
 *****************************************************************************/
mmi_ret mmi_java_force_resume_dialog_proc(mmi_event_struct *evt)
{
    ...
    switch (evt->evt_id)
    {
        case EVT_ID_GROUP_GOBACK_IN_END_KEY:
            mmi_java_force_resume_dialog_del_cb();
            break;
#ifdef __MMI_NCENTER_SUPPORT__
        case EVT_ID_VAPP_NCENTER_DRAG:
            return MMI_RET_ERR;
...
/*****************************************************************************
 * FUNCTION
 *  mmi_java_entry_terminating_leave_proc
 * DESCRIPTION
 *  Proc function of SCR_JAVA_VM_TERMINATING
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
mmi_ret mmi_java_entry_terminating_leave_proc(mmi_event_struct*evt)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
void mmi_java_entry_terminating_internal(void)
{
        ...
        MMI_FRM_FULL_SCRN))
    {
        ...
        mmi_frm_scrn_set_leave_proc(GRP_ID_JAVA_BUSY,SCR_JAVA_VM_TERMINATING,mmi_java_entry_terminating_leave_proc);
        ShowCategory8Screen(
...
void mmi_java_entry_cb_terminate_dialog_internal_entry(void)
{
    ...
    /* Block JPUSH */
    {
        ...
        ShowCategory74Screen(
            ...
            mmi_ucs2strlen((const CHAR*) cb_terminate_string),
            guiBuffer);

        if (cb_del_scr_hdlr)
        {
            //SetDelScrnIDCallbackHandler(SCR_JAVA_TERMINATE_DIALOG2,(HistoryDelCBPtr)mmi_java_cb_terminate_dialog_del_cb_hdlr);
...
void mmi_java_entry_cb_terminate_dialog(mmi_java_terminate_hdlr fp, U16* str, U8 del_scr_hdlr)
{
    ...
    {
        //SetDelScrnIDCallbackHandler(SCR_JAVA_TERMINATE_DIALOG2,NULL);
        mmi_frm_scrn_close_active_id();
    }
    ...
    switch (evt->evt_id)
    {
        ...
        case EVT_ID_SCRN_DEINIT:
            if(g_mmi_java_scrn_delete_callback_exe)
            {
                mmi_java_screen_delete_hdlr(NULL);
#if defined(__PLUTO_MMI_PACKAGE__) && defined(J2ME_SLIM_MEMORY_SUPPORT)
                g_mmi_java_scrn_delete_evt_id = (U16) evt->evt_id;
 ...
 *  void
 *****************************************************************************/
void mmi_java_resume_delete_scrn(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /*----------------------------------------------------------------*/
#if defined(__PLUTO_MMI_PACKAGE__) && defined(J2ME_SLIM_MEMORY_SUPPORT)
    switch (g_mmi_java_scrn_delete_evt_id)
    {
        case EVT_ID_SCRN_DELETE_REQ:
        case EVT_ID_SCRN_GOBACK:
        case EVT_ID_SCRN_DEINIT:
    ...
    g_mmi_java_scrn_delete_evt_id = EVT_ID_SCENARIO_MAX;
#endif
}
...
#endif
        !mmi_java_is_in_voice_call_ext(MMI_FALSE)
    )
    {
    ...
    else
    {
        if (jam_vm_canbe_minimize(vm_id))
        {
            if (!mmi_java_need_block_msg())
            {
                ...
                mmi_confirm_display((WCHAR *)((UI_string_type)get_string(STR_JAVA_MINIMIZE_NOTIFY)), MMI_EVENT_INFO, &arg);
                } while(0);    
