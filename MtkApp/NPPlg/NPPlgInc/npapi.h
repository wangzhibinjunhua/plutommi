...
#if !defined(WINGOGI) && !defined(BREW)
#ifdef _WINDOWS
#   include <windef.h>
#   ifndef XP_WIN
#       define XP_WIN 1
...
#if defined(__MWERKS__) && !defined(EPOC)
...
#if defined(XP_UNIX) && !defined(QWS) && !defined(GTK_PLUGINS)
#ifndef Window
...
#if defined(XP_UNIX) 
#   include <stdio.h>
#   if defined(MOZ_X11) || !defined (NO_X11)
#		include <X11/Xlib.h>
#		include <X11/Xutil.h>
#   endif
#endif
...
typedef struct _NPStream
{
  ...
  uint32 lastmodified;
  void*  notifyData;
  const char* headers; /* read HTTP response headers */
} NPStream;
...
#if defined(XP_UNIX) && !defined(QWS)
/*
...
typedef enum {
  ...
  /* Plugin requests in NPN_SetValue status-free URL requests */
...
typedef enum {
  ...
  /* Get the browser's support of windowless mode */
  NPNVSupportsWindowless = 17,

  /* Get the browser's support of private mode */
...
#elif defined (XP_UNIX) && ( defined(MOZ_X11) || (!defined(QWS) && !defined(GTK_PLUGINS) && !defined(NO_X11)))
...
/*
 * Type definition for buffer length passed to NPN_GetCookie and NPN_SetCookie:
 */
...
#define NPVERS_STREAM_HEADERS           17
...
#if defined(_WINDOWS) && !defined(WIN32) && !defined(BREW)
