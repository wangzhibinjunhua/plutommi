...
#include "IdleAppResDef.h"
#ifndef __FIX_BUG_ABOUT_PQ__
#define __FIX_BUG_ABOUT_PQ__
#endif/*__FIX_BUG_ABOUT_PQ__*/
#ifdef __FIX_BUG_ABOUT_PQ__
#include "mmi_rp_app_gpio_def.h"
kal_bool g_is_need_to_resume_PQ = KAL_FALSE;
#endif
...
MMI_BOOL matv_resz_timer_seted = MMI_FALSE;
...
static MMI_BOOL mmi_mtv_get_picture_quality_status(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    ReadValue(NVRAM_SETTING_PICTURE_QUALITY, &data, DS_BYTE, &error);
    if(data != 0)
    {
        result = MMI_TRUE;
    }
...
static void mmi_mtv_set_picture_quality_status(MMI_BOOL is_enable)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    WriteValue(NVRAM_SETTING_PICTURE_QUALITY, &data, DS_BYTE, &error);
...
mmi_ret mmi_mtv_player_group_proc(mmi_event_struct *evt)
{
    ...
    switch (evt->evt_id)
    {
	...
	#ifdef __FIX_BUG_ABOUT_PQ__
		    if(mmi_mtv_get_picture_quality_status())
		    {
		      gdi_color_engine_custom_off();
			  mmi_mtv_set_picture_quality_status(MMI_FALSE);	
			  g_is_need_to_resume_PQ = KAL_TRUE;
		    }
			break;
	#endif	    	
		...
		#ifdef __FIX_BUG_ABOUT_PQ__
            if(g_is_need_to_resume_PQ == KAL_TRUE)
            {
                //pending some time before DCM Unload;
                gdi_color_engine_custom_on();
                mmi_mtv_set_picture_quality_status(MMI_TRUE);
				g_is_need_to_resume_PQ = KAL_FALSE;
            }				
        #endif
#ifdef __MMI_AP_DCM_MATV__
 ...
 void mmi_mtv_player_exit_app_screen(void)
{
    ...
    if (MMI_TRUE == matv_resz_timer_seted)
    {
        ...
        gui_cancel_timer(mmi_mtv_player_send_msg_to_restart);
 ...
 void mmi_mtv_player_service_ready_hdlr(mdi_mtv_service_info_struct* service_info)
{
    ...
    if(MMI_FALSE == matv_resz_timer_seted)
    {
        ...
        kal_set_timer(matv_resz_cp_ESD_timeout_timer, (kal_timer_func_ptr)resz_cp_ESD_timeout_timer_hdlr,NULL, KAL_TICKS_1_SEC, KAL_TICKS_1_SEC);
 ...
 MMI_BOOL mmi_mtv_player_snapshot(void)
{
    ...
    if (result != FS_NO_ERROR)
    {
		...
		if(result == FS_DRIVE_NOT_FOUND)
			mtv_player_p->err_str_id = STR_GLOBAL_INSERT_MEMORY_CARD;
   ...
   module_type src_mod_id;
	 ...
	 else
	 {
        CalLcdFroceAutoUpdate();
           ...
           gui_cancel_timer(mmi_mtv_player_send_msg_to_restart);
           gui_start_timer(1, mmi_mtv_player_send_msg_to_restart); 	   
#if 0
/* under construction !*/
/* under construction !*/
/* under construction !*/
