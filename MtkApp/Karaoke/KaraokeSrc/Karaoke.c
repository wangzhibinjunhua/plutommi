...
static mmi_ret mmi_karaoke_event_hdlr(mmi_event_struct *event)
{
	...
	switch (event->evt_id)
	{
		case MMI_KARAOKE_EVENT_PREPARE_LYRIC:
		{
            ...
            #endif
			if(!g_karaoke_cntx.need_resume && !g_karaoke_cntx.is_playing_records && !g_karaoke_cntx.is_paused)
			{
		...
		{
			kal_uint8 temp_aud_vol=0,temp_mic_vol=0;
			...
			if(!g_karaoke_cntx.aud_mute)
				temp_aud_vol=g_karaoke_cntx.aud_vol;
			if(!g_karaoke_cntx.mic_mute)
				temp_mic_vol=g_karaoke_cntx.mic_vol;
			...
			media_aud_set_karaoke_digitgain_req(temp_aud_vol*KARAOKE_AUD_GAIN_UNIT,
											temp_mic_vol*KARAOKE_MIC_GAIN_UNIT);
    		...
    		result = mdi_audio_play_file_portion_with_vol_path(
                       ...
                       NULL,
                       (U8)MDI_AUD_VOL_EX_MUTE(MDI_KARAOKE_VOL_EX_MAX),
		...
		case MMI_KARAOKE_EVENT_PLAY_RECORD:
		{
            ...
            #ifdef __KARAOKE_NEED_KEEP_SRC_PATH__
			if(g_karaoke_cntx.remix_mic_mute)
			...
			if(g_karaoke_cntx.remix_aud_mute)
			...
			else
			{
				...
				{
				    mdi_audio_karaoke_remix_turn_off();
					g_karaoke_cntx.filefullname[0]=0;
				}
				if((result != MDI_AUDIO_SUCCESS) && (result != MDI_AUDIO_END_OF_FILE))
				{
				    result = mdi_audio_play_file_portion_with_vol_path(
					   ...
					   mmi_karaoke_play_record_callback_hdlr,
					   NULL,
					   (U8)MDI_AUD_VOL_EX_MUTE(g_karaoke_cntx.vol), /*if no remix(SRC is NULL), set total volume*/
					   MDI_AUD_PTH_EX(MDI_DEVICE_SPEAKER2));
				}
			}
            #else
...
/*****************************************************************************
 * FUNCTION
 *  mmi_karaoke_delay_prepare_lyrics
 * DESCRIPTION
 *  
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
static void mmi_karaoke_delay_prepare_lyrics(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
static void mmi_karaoke_list_generate_finish(void)
{
    ...
    if (this->appended_number)
    {
        ...
        if (g_karaoke_cntx.item == MMI_KARAOKE_ITEM_SONG_LIST)
        {
			...
			list->list_data.total += this->appended_number;
        	mmi_popup_display(
                        (WCHAR*)pop_str, 
                        MMI_EVENT_INFO,
                        NULL);
...
void mmi_karaoke_launch(void)
{
	...
	g_karaoke_cntx.vol = g_karaoke_cntx.remix_mic_vol;
	#endif
	...
	ReadValue(NVRAM_KARAOKE_REMIX_AUD_MUTE, &g_karaoke_cntx.remix_aud_mute, DS_BYTE, &error);
	ReadValue(NVRAM_KARAOKE_REMIX_MIC_MUTE, &g_karaoke_cntx.remix_mic_mute, DS_BYTE, &error);
}
...
static void mmi_karaoke_inc_aud_vol(void)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	kal_uint8 temp_mic_vol=g_karaoke_cntx.mic_vol;
	...
	if(g_karaoke_cntx.aud_vol && g_karaoke_cntx.aud_mute)
...
static void mmi_karaoke_inc_mic_vol(void)
{
	...
	/* Local Variables												  */
	/*----------------------------------------------------------------*/
	kal_uint8 temp_aud_vol=g_karaoke_cntx.aud_vol;
	...
	if(g_karaoke_cntx.mic_vol && g_karaoke_cntx.mic_mute)
...
static void mmi_karaoke_inc_remix_aud_vol(void)
{
	...
	if(g_karaoke_cntx.remix_aud_vol && g_karaoke_cntx.remix_aud_mute)
		g_karaoke_cntx.remix_aud_mute = MMI_FALSE;
...
static void mmi_karaoke_inc_remix_mic_vol(void)
{
	...
	if(g_karaoke_cntx.remix_mic_vol && g_karaoke_cntx.remix_mic_mute)
		g_karaoke_cntx.remix_mic_mute = MMI_FALSE;
...
static void mmi_karaoke_inc_total_vol(void)
{
	...
	{	
		...
		g_karaoke_cntx.vol = KARAOKE_MIC_VOL_DEFAULT;

	if(g_karaoke_cntx.vol && g_karaoke_cntx.remix_mic_mute)
		g_karaoke_cntx.remix_mic_mute = MMI_FALSE;

	g_karaoke_cntx.remix_mic_vol = g_karaoke_cntx.vol;
	mmi_karaoke_update_voice_bar(g_karaoke_cntx.vol);
	
	MMI_TRACE(MMI_MEDIA_TRC_G2_APP, MMI_KARAOKE_INC_TOTAL_VOL, g_karaoke_cntx.vol);
}
...
static void mmi_karaoke_dec_total_vol(void)
{
	...
	MMI_TRACE(MMI_MEDIA_TRC_G2_APP, MMI_KARAOKE_DEC_TOTAL_VOL, g_karaoke_cntx.vol);
}


/*****************************************************************************
 * FUNCTION
 *  mmi_karaoke_set_total_vol
 * DESCRIPTION
 *  
 * PARAMETERS
 *  index[IN]: total volume index to set
 * RETURNS
 * BOOL
 *****************************************************************************/
static void mmi_karaoke_set_total_vol(kal_uint8 index)
{
	...
	if(index >= MDI_KARAOKE_VOL_EX_MIN && index <= MDI_KARAOKE_VOL_EX_MAX)
	{
		g_karaoke_cntx.vol = index;
	}
	else
	{
		g_karaoke_cntx.vol = KARAOKE_REMIX_MIC_VOL_DEFAULT;
	}
	...
	if( g_karaoke_cntx.vol >= MDI_KARAOKE_VOL_EX_MIN && 
		g_karaoke_cntx.vol <= MDI_KARAOKE_VOL_EX_MAX)
		WriteValueSlim(NVRAM_KARAOKE_VOL, &g_karaoke_cntx.vol, DS_BYTE);
	MMI_TRACE(MMI_MEDIA_TRC_G2_APP, MMI_KARAOKE_SET_TOTAL_VOL, g_karaoke_cntx.vol);
}
...
static void mmi_karaoke_inc_vol(void)
{
	...
	else
	{
		...
		{
		    if(0 == g_karaoke_cntx.filefullname[0]) /* SRC file not exsit, we need to adjust total volume, not remix vol */
    ...
    switch (result)
    {
		...
		default:
		{
			...
			if(g_karaoke_cntx.new_play_time != 0)
			    g_karaoke_cntx.is_interrupted = KAL_TRUE;
			else /* The file can't be played at all, so do not enter play record preview screen*/
			{
			    g_karaoke_cntx.is_interrupted = KAL_FALSE;
				mmi_frm_group_close(GRP_ID_KARAOKE_SING);
			}
...
void mmi_karaoke_entry_sing_screen(void)
{
	...
	if(!g_karaoke_cntx.is_singing) /*Get duration first to avoid show wrong duration*/
	{
	...
	{
        StopTimer(KARAOKE_DELAY_PLAY_TIMER);
		StartTimer(KARAOKE_DELAY_PLAY_TIMER, 10, mmi_karaoke_delay_prepare_lyrics);
	}
	else if(!g_karaoke_cntx.need_resume && !g_karaoke_cntx.play_record_for_reviewing)
    {
		g_karaoke_cntx.new_play_time = 0;
		g_karaoke_cntx.origin_enable = KAL_FALSE;
		#ifdef __LINE_IN_SUPPORT__
...
void mmi_karaoke_entry_play_record_screen(void)
{
	...
	if(!g_karaoke_cntx.is_playing_records) /*Get duration first to avoid show wrong duration*/
	...
	else if(!g_karaoke_cntx.is_playing_records)  //(!g_karaoke_cntx.need_resume && !g_karaoke_cntx.is_playing_records)
    {
		g_karaoke_cntx.new_play_time = 0;
		if (!g_karaoke_cntx.is_paused)
		{
...
void mmi_karaoke_pen_move_vol_hdlr(mmi_pen_point_struct pos)
{
		 ...
		 KARAOKE_VOL_MIC_BG_Y <= pos.y && pos.y <= (KARAOKE_VOL_MIC_BG_Y + KARAOKE_VOL_BG_H)))
	{
		...
		{
		    if(0 == g_karaoke_cntx.filefullname[0])
		        mmi_karaoke_set_total_vol(index);
			else
...
void mmi_karaoke_pen_up_vol_hdlr(mmi_pen_point_struct pos)
{
		...
		&&(KARAOKE_VOL_SPK_ICON_Y<=pos.y && pos.y <= (KARAOKE_VOL_SPK_ICON_Y + KARAOKE_VOL_ICON_H))) 
	{
		...
		if(g_karaoke_cntx.is_singing)
		{
			...
			WriteValueSlim(NVRAM_KARAOKE_AUD_MUTE, &g_karaoke_cntx.aud_mute, DS_BYTE);
		}
		else
		{
			g_karaoke_cntx.remix_aud_mute = !g_karaoke_cntx.remix_aud_mute;
			if(g_karaoke_cntx.remix_aud_mute)
			{
			    mmi_karaoke_update_audio_bar(0);
			}
			...
			WriteValueSlim(NVRAM_KARAOKE_REMIX_AUD_MUTE, &g_karaoke_cntx.remix_aud_mute, DS_BYTE);
		}
		...
		&&(KARAOKE_VOL_MIC_ICON_Y <=pos.y && pos.y <= (KARAOKE_VOL_MIC_ICON_Y + KARAOKE_VOL_ICON_H))) 
	{
		...
		if(g_karaoke_cntx.is_singing)
		{
			...
			WriteValueSlim(NVRAM_KARAOKE_MIC_MUTE, &g_karaoke_cntx.mic_mute, DS_BYTE);
		}
		else
		{
			g_karaoke_cntx.remix_mic_mute = !g_karaoke_cntx.remix_mic_mute;
			if(g_karaoke_cntx.remix_mic_mute)
			{
			    mmi_karaoke_update_voice_bar(0);
			}
			...
			{
			    if(0 == g_karaoke_cntx.filefullname[0])
		            mmi_karaoke_set_total_vol(g_karaoke_cntx.vol);
			    else
				    mmi_karaoke_set_remix_mic_vol(g_karaoke_cntx.remix_mic_vol);
			}
			WriteValueSlim(NVRAM_KARAOKE_REMIX_MIC_MUTE, &g_karaoke_cntx.remix_mic_mute, DS_BYTE);
		}
	}
...
static void mmi_karaoke_update_spk_icon(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	MMI_TRACE(MMI_MEDIA_TRC_G2_APP, MMI_KARAOKE_UPDATE_SPK_ICON, g_karaoke_cntx.is_singing,
	g_karaoke_cntx.aud_mute,g_karaoke_cntx.mic_mute,g_karaoke_cntx.remix_aud_mute,g_karaoke_cntx.remix_mic_mute);
	
	gdi_layer_push_and_set_clip(x1,y1,x2,y2);
    gdi_draw_solid_rect(x1,y1,x2,y2,MMI_KARAOKE_VOL_POP_BG_COLOR);
	
	if(g_karaoke_cntx.is_singing && (g_karaoke_cntx.aud_mute||(MDI_KARAOKE_VOL_EX_MIN==g_karaoke_cntx.aud_vol)))
	{
	...
	else if(!g_karaoke_cntx.is_singing && (g_karaoke_cntx.remix_aud_mute||(MDI_KARAOKE_VOL_EX_MIN == g_karaoke_cntx.remix_aud_vol)))
	{
        gdi_image_draw_id(KARAOKE_VOL_SPK_ICON_X,KARAOKE_VOL_SPK_ICON_Y,IMG_ID_KARAOKE_VOL_SPK_MUTE);
		media_aud_karaoke_set_remix_audio_gain_req(0);
	}
...
static void mmi_karaoke_update_mic_icon(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	MMI_TRACE(MMI_MEDIA_TRC_G2_APP, MMI_KARAOKE_UPDATE_MIC_ICON, g_karaoke_cntx.is_singing,
	g_karaoke_cntx.aud_mute,g_karaoke_cntx.mic_mute,g_karaoke_cntx.remix_aud_mute,g_karaoke_cntx.remix_mic_mute);
	
	gdi_layer_push_and_set_clip(x1,y1,x2,y2);
    gdi_draw_solid_rect(x1,y1,x2,y2,MMI_KARAOKE_VOL_POP_BG_COLOR);
	
	if(g_karaoke_cntx.is_singing && (g_karaoke_cntx.mic_mute||(MDI_KARAOKE_VOL_EX_MIN==g_karaoke_cntx.mic_vol)))
	{
	...
	else if(!g_karaoke_cntx.is_singing && (g_karaoke_cntx.remix_mic_mute||(MDI_KARAOKE_VOL_EX_MIN==g_karaoke_cntx.remix_mic_vol)))
	{
	    gdi_image_draw_id(x1,y1,IMG_ID_KARAOKE_VOL_MIC_MUTE);
		if(0 == g_karaoke_cntx.filefullname[0])
		{
		    if(g_karaoke_cntx.is_playing_records)
			    mdi_audio_set_volume(VOL_TYPE_MEDIA,MDI_AUD_VOL_EX_MUTE(0));
		}
		else
            media_aud_karaoke_set_remix_voice_gain_req(0);
	}
...
static void mmi_karaoke_update_audio_bar(kal_uint8 index)
{
	...
	if(g_karaoke_cntx.is_singing && g_karaoke_cntx.aud_mute)
		temp=0;
	else if(!g_karaoke_cntx.is_singing && g_karaoke_cntx.remix_aud_mute)
		temp=0;
	else
...
static void mmi_karaoke_update_voice_bar(kal_uint8 index)
{
	...
	gdi_layer_push_and_set_clip(x1,y1,x2,y2);
	gdi_draw_solid_rect(x1,y1,x2,y2,MMI_KARAOKE_VOL_POP_BG_COLOR);
	if(g_karaoke_cntx.is_singing && g_karaoke_cntx.mic_mute)
		temp=0;
	else if(!g_karaoke_cntx.is_singing && g_karaoke_cntx.remix_mic_mute)
		temp=0;
	else
	    temp = (float)index/MDI_KARAOKE_VOL_EX_MAX;
...
void mmi_karaoke_entry_volume_adjust_screen(void)
{
	...
	{
	    if(0 == g_karaoke_cntx.filefullname[0]) /* SRC file not exsit, we need to show total volume, not remix vol */
			mmi_karaoke_update_voice_bar(g_karaoke_cntx.vol);
		else 
		    mmi_karaoke_update_voice_bar(g_karaoke_cntx.remix_mic_vol);
	}
...
void mmi_karaoke_exit_volume_adjust_screen(void)
{
    ...
    StopTimer(KARAOKE_VOLUME_ADJUST_TIMER);
	if (scrn_id != SCR_ID_INVALID)  /*Volume adjust screen is interrupted, If we're singing, we need goto play record preview screen*/
	{
		if(g_karaoke_cntx.need_resume || g_karaoke_cntx.is_singing)
		{
			g_karaoke_cntx.need_resume=KAL_FALSE;
	...
	else  /*Volume adjust screen is left by timeout*/
	{
	    if(g_karaoke_cntx.is_singing || g_karaoke_cntx.is_playing_records)
            g_karaoke_cntx.need_resume = KAL_TRUE;
		else
