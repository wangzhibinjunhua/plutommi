...
void mmi_entry_audioply_pre_list(void)
{
    ...
    if (mmi_bt_music_check_connection())
    {
        ...
        return;
      #endif//  #if defined(__MMI_A2DP_SUPPORT__) && defined(__BT_SPEAKER_SUPPORT__)
    }
    #endif
...
void mmi_audply_entry_main(void)
{
    ...
    MMI_BOOL is_a2dp_sink_conn = mmi_bt_music_check_connection();
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_MEDIA_TRC_G2_APP,MMI_AUDPLY_TRC_LAUNCH, g_is_launch_from_bt);
    if(!g_is_launch_from_bt && !is_audply_playing && is_a2dp_sink_conn)
    {
