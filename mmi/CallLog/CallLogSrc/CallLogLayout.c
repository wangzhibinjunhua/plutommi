...
#endif
{
    ...
    /*----------------------------------------------------------------*/
#ifndef __MMI_TAB_BAR_SUPPORT_TAB_PAGE__
    mmi_clog_lt_inst_struct *inst;//CHANGE
#endif
    mmi_frm_tab_struct page_info[2];
    U32 index;
...
#endif
    mmi_clog_bt_link_fill_tab_info(inst, page_info, 2);
    if (!srv_bt_cm_is_profile_connected(SRV_BT_CM_PBAPC_CONNECTION)|| g_is_missed_call.is_from_local)
    {
        index = 0;
    }
    else
...
static void mmi_clog_bt_link_lt_enter_tab_page(mmi_scrn_essential_struct *grp_data)
{
   ...
   //WCHAR path[41];
    ...
    index = mmi_frm_scrn_tab_get_active_page_index();
#endif
//  MMI_CLOG_ASSERT(index < g_mmi_clog_bt_link_tab_id.total_num);
   // inst->sel_lt_id = g_mmi_clog_bt_link_tab_id.lt_ids[index];
    ...
    else
    {
        ...
        kal_prompt_trace(MOD_MMI, "[TEST]is_redraw = %d", is_redraw);
        if (is_redraw)
        {
            U8 *gui_buffer = (U8*)mmi_frm_scrn_get_active_gui_buf();
        ...
        else
        {
            ...
            {
                mmi_clog_bt_dialer_show(inst);
                //mmi_clog_bt_link_no_conn_show_list(inst);
            }
            else
            {
                g_mmi_clog_pbapc_cntx.is_from_close_pop = MMI_FALSE;
...
static void mmi_clog_bt_link_conn_profile_lsk_hdlr()
{
    ...
    mmi_bt_dialer_conn_profile(group_id, SRV_BT_CM_PBAPC_CONNECTION, mmi_clog_bt_link_conn_profile_cb);
}
...
void mmi_clog_set_bt_status()
{
    /*g_mmi_clog_bt_add.bt_is_connect = MMI_FALSE;
    kal_prompt_trace(MOD_MMI, "[ClogLayout]mmi_clog_set_bt_status: %d", 
    g_mmi_clog_bt_add.bt_is_connect);*/
    kal_prompt_trace(MOD_MMI, "[Clog]mmi_clog_set_bt_status");
    g_high_index[0] = 1;
    g_high_index[1] = 1;
    g_high_index[2] = 1;
    g_high_index[3] = 1;
...
static void mmi_clog_bt_link_lt_exit_tab_page(mmi_scrn_essential_struct *scr_info)
{
    ...
    MMI_TRACE(MMI_CLOG_TRC_INFO,TRC_MMI_CLOG_PBAPC_ENTER_TAB_PAGE_MENU,sel_menu_id,g_high_index[index]);
    wgui_set_menu_empty_string_by_id(STR_GLOBAL_EMPTY);
}
#endif
...
static void mmi_clog_lt_refresh_sel_index(mmi_clog_lt_inst_struct *inst)
{
    ...
    if (i == max_num)   /* not found, must be deleted, try to highlight next one */
    {
        ...
        if(mmi_clog_get_id_by_idx(inst->sel_lt_id, inst->sel_log_idx))
        {
            // Update
            inst->sel_log_idx = 0;
        }
...
static void mmi_clog_bt_dialer_show(mmi_clog_lt_inst_struct *inst)
{
  ...
  extern  NMGR_HANDLE bt_nmgr_handle ;
    ...
    /*----------------------------------------------------------------*/
#ifdef __MMI_TAB_BAR_SUPPORT_TAB_PAGE__
    index = cui_tab_get_select_index(inst->tab_id);
#else
    index = mmi_frm_scrn_tab_get_active_page_index();
#endif
    g_exist_flag = 1;
    page = (mmi_clog_lt_bt_tab_page_struct *)mmi_clog_lt_bt_link_get_page_by_lt_id(inst, g_mmi_clog_bt_link_tab_id.lt_ids[index]);
    ret = mmi_frm_cb_reg_event(EVT_ID_PHB_PBAPC_NOTIFY, mmi_clog_pbapc_notify_evt_hdlr, (void *)0xbabe0003);
#ifdef __MMI_NO_SHOW_CLOG_WHEN_DISCONNECT__
    mmi_frm_cb_reg_event(EVT_ID_PBAP_NOTIFY, mmi_clog_pbapc_notify_evt_hdlr, (void *)0xbabe0007);
#endif
    MMI_TRACE(MMI_CLOG_TRC_INFO,TRC_MMI_CLOG_PBAPC_ENTER_TAB_PAGE_IS_CONNECT,srv_bt_cm_is_profile_connected(SRV_BT_CM_PBAPC_CONNECTION));
    sel_menu_id = mmi_clog_bt_get_menu_id(inst->sel_lt_id);
#ifdef __MTK_TARGET__
    if(!srv_bt_cm_is_profile_connected(SRV_BT_CM_PBAPC_CONNECTION))
    {
        kal_prompt_trace(MOD_MMI, "mmi_clog_bt_link_lt_enter_tab_page: Other_profile_connect = %d", mmi_bt_dialer_app_is_connected());
    #ifndef __MMI_NO_SHOW_CLOG_WHEN_DISCONNECT__
        if (mmi_bt_dialer_app_is_connected())
        {
            MMI_TRACE(MMI_CLOG_TRC_INFO,TRC_MMI_CLOG_BT_DIALER_ONLY_AGREE,mmi_bt_dialer_app_is_connected());
        ...
        else
        {
            ...
            MMI_TRACE(MMI_CLOG_TRC_INFO, TRC_MMI_CLOG_PATH_IS_EXIST, ret, mmi_clog_lt_get_title_by_lt_id(inst, inst->sel_lt_id));
            if(ret < 0)
            {
                mmi_clog_bt_link_no_conn_show_list(inst, page);
...
#endif
    {
        ...
        kal_prompt_trace(MOD_MMI, "[@@@]mmi_clog_bt_dialer_show():lt_id = %d, bt_nmgr_handle= %d", inst->sel_lt_id,bt_nmgr_handle);
        if (inst->sel_lt_id == MMI_CLOG_LT_ID_MISSED_CALL || inst->sel_lt_id == MMI_CLOG_LT_ID_ALL_CALL)
        {       
            if (srv_bt_noti_update_remote_missed_call())
            {
                if(bt_nmgr_handle)
                {
...
#endif
        pbapc_cntx = mmi_clog_get_pbapc_cntx();
        g_mmi_clog_pbapc_cntx.menu_id = sel_menu_id;
        g_mmi_clog_pbapc_processing_inst.sel_menu_id = sel_menu_id;
        kal_prompt_trace(MOD_MMI, "[TEST]mmi_clog_bt_dialer_show: handle = %x , ret = %d", pbapc_cntx->phapc_op_handle);
        //if (ret < 0)
