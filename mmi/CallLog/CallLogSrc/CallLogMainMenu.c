...
#ifndef __MMI_CLOG_TAB_SUPPORT__
void mmi_clog_hdlr_processing_timeout(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_frm_group_close(GRP_ID_CLOG_PROCESSING);
//	mmi_frm_scrn_close_active_id();
}


mmi_ret mmi_clog_processing_group_proc(mmi_event_struct *evt)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
mmi_ret mmi_clog_processing_screen_proc(mmi_event_struct *evt)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
void mmi_clog_entry_processing(mmi_scrn_essential_struct *data)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    enter_res = mmi_frm_scrn_enter(
                    ...
                    data->scrn_id, 
                    NULL, 
                    (FuncPtr)mmi_clog_entry_processing, 
                    MMI_FRM_UNKNOW_SCRN);

    mmi_frm_scrn_set_leave_proc(data->group_id, data->scrn_id, mmi_clog_processing_screen_proc);
    ...
    ShowCategory8Screen(
        ...
        0,
        0,
        STR_ID_CLOG_CLEARING,
        mmi_get_event_based_image(MMI_EVENT_PROGRESS),
        NULL);
}


static MMI_RET mmi_clog_enter_processing()
{
    if (!mmi_frm_group_is_present(GRP_ID_CLOG_PROCESSING))
    {
        mmi_frm_group_create(GRP_ID_ROOT, GRP_ID_CLOG_PROCESSING, mmi_clog_processing_group_proc, NULL);
        mmi_frm_group_enter(GRP_ID_CLOG_PROCESSING, MMI_FRM_NODE_SMART_CLOSE_FLAG);
    }

    mmi_frm_scrn_first_enter(GRP_ID_CLOG_PROCESSING, SCR_ID_CLOG_PROCESSING, (FuncPtr)mmi_clog_entry_processing, NULL);
}
...
static MMI_RET mmi_clog_group_proc(mmi_event_struct *evt)
{
    ...
    switch (evt->evt_id)
    {
        ...
        case EVT_ID_ALERT_QUIT:
            ...
            {
                mmi_clog_enter_processing();
                mmi_clog_delete_log_list(inst);
                StartTimer(MMI_CLOG_PROCESSING_TIMER, 1000, mmi_clog_hdlr_processing_timeout);
            }
            #endif /* __MMI_CLOG_TAB_SUPPORT__ */
            break;
#endif /*__MMI_CLOG_TAB_SUPPORT__*/
