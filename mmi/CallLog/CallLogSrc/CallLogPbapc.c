...
MMI_BOOL g_only_update_data = MMI_FALSE; 
typedef struct
{
    U16 sel_log_idx;
...
MMI_RET mmi_clog_update_log_by_end_call(mmi_event_struct *evt);
...
{   
    ...
    switch (sel_menu_id)
    {
        ...
        {   
            ...
            kal_prompt_trace(MOD_MMI, "[TEST]mmi_clog_pbapc_select_hdlr: handle = %x", g_mmi_clog_pbapc_cntx.phapc_op_handle);
            lt_storage = mmi_clog_pbapc_menu_to_storage_path(sel_menu_id);
            kal_prompt_trace(MOD_MMI, "[TEST]mmi_clog_pbapc_select_hdlr:PATH=%d, lt_storage = %d",
            g_mmi_clog_pbapc_cntx.path, lt_storage);
            ...
            if (pbapc_cntx->phapc_op_handle == NULL)
            {
                ...
                if (!g_mmi_clog_pbapc_cntx.flag && !g_mmi_clog_pbapc_cntx.is_read)//bg read fail handle is exist
                {
                    mmi_clog_prepare_data(sel_menu_id,gid);
                    
                    MMI_TRACE(MMI_CLOG_TRC_INFO, TRC_MMI_CLOG_BT_DIALER_SELECT_HDLR, lt_storage);
                    mmi_clog_pbapc_read_folder(lt_storage);
                }
                else
                {
                    //g_mmi_clog_pbapc_cntx.parent_id = gid;// for group change.
            ...
            else
            {
                if (!pbapc_cntx->is_only_update)
                {
                    ...
                    if (lt_storage == pbapc_cntx->path && pbapc_cntx->path != SRV_PHB_PBAP_STORAGE_PHONE_CCH)
                    {
                        g_mmi_clog_pbapc_cntx.is_read = MMI_FALSE;
                    ...
                    {
                         g_mmi_clog_pbapc_cntx.flag = 1;// only wait cb
                         g_mmi_clog_pbapc_cntx.is_read = MMI_TRUE;
...
void mmi_clog_prepare_data(MMI_MENU_ID sel_menu_id, MMI_ID gid)
{
    ...
    if (!g_folder_exist)
    {
        g_mmi_clog_pbapc_lt_inst.sel_lt_storage = lt_storage;
    }
    MMI_TRACE(MMI_CLOG_TRC_INFO, TRC_MMI_CLOG_BT_DIALER_IS_IN_BACKWARD, g_only_update_data, mmi_frm_is_in_backward_scenario());

    if ((!g_folder_exist || 
    !srv_bt_cm_is_profile_connected(SRV_BT_CM_PBAPC_CONNECTION)) && !g_only_update_data)// two times 
    {
        g_mmi_clog_pbapc_lt_inst.self_gid = gid;
        mmi_clog_pbapc_processing_show_ex(gid,MMI_CLOG_PBAPC_OP_TYPE_LT_LIST, lt_storage);
    }
...
static pBOOL mmi_clog_pbapc_lt_get_log_item(S32 item_index, UI_string_type str_buff, U8 **img_buff_p, U8 str_img_mask)
{
    ...
    mmi_clog_pbapc_lt_get_string(&g_mmi_clog_pbapc_clog_info_cache[item_index].clog_info, str_buff);    
#endif /**/
    kal_prompt_trace(MOD_MMI, "[CLogPbapc]mmi_clog_pbapc_lt_get_log_item:g_mmi_clog_pbapc_lt_inst.sel_lt_storage = %d", g_mmi_clog_pbapc_lt_inst.sel_lt_storage);
    kal_prompt_trace(MOD_MMI, "[CLogPbapc]mmi_clog_pbapc_lt_get_log_item:clog_info->call_type = %d, item_index = %d", g_mmi_clog_pbapc_clog_info_cache[item_index].clog_info->call_type);
    if (g_mmi_clog_pbapc_lt_inst.sel_lt_storage == SRV_PHB_PBAP_STORAGE_PHONE_CCH)
    {
        *img_buff_p =
        (U8*) 
    ...
    {
        *img_buff_p =
        (U8*) GetImage(mmi_clog_pbapc_lt_get_img_by_lt_storage(g_mmi_clog_pbapc_lt_inst.sel_lt_storage));//?
    } 
    return MMI_TRUE;
}
...
void mmi_clog_pbapc_read_cb(S32 result)
{
    S32 fs_ret = -1;
    MMI_ID scr_id;
    MMI_BOOL is_connect = srv_bt_cm_is_profile_connected(SRV_BT_CM_PBAPC_CONNECTION);
...
#ifndef __MMI_CLOG_TAB_SUPPORT__
   // mmi_clog_pbapc_lt_list_enter(GRP_ID_CLOG_PBAPC, g_mmi_clog_pbapc_processing_inst.sel_lt_storage);//SRV_PHB_PBAP_STORAGE_PHONE_MCH)//
   //mmi_clog_pbapc_lt_list_enter(GRP_ID_CLOG_PBAPC, g_mmi_clog_pbapc_cntx.path);//SRV_PHB_PBAP_STORAGE_PHONE_MCH)//g_mmi_clog_pbapc_cntx.path
    scr_id = mmi_frm_scrn_get_active_id();
    MMI_TRACE(MMI_CLOG_TRC_INFO, TRC_MMI_CLOG_PBAPC_CURR_ACTIVE, scr_id , g_exist_flag, 1);
...
MMI_RET mmi_clog_update_log_by_end_call(mmi_event_struct *evt)
{
    MMI_TRACE(MMI_CLOG_TRC_INFO, TRC_MMI_CLOG_UPDATE_BY_END_CALL, evt->evt_id, mmi_frm_scrn_get_active_id());
//kal_prompt_trace(MOD_MMI, " IS_PRESENT = %d", mmi_frm_group_is_present(GRP_ID_CLOG_LAYOUT));
    if (mmi_frm_scrn_get_active_id() != SCR_ID_CLOG_LAYOUT && !mmi_frm_group_is_present(GRP_ID_CLOG_LAYOUT))
    {
        g_mmi_clog_pbapc_cntx.is_only_update= MMI_TRUE;
        //g_only_update_data = MMI_TRUE;
        //g_folder_exist = MMI_FALSE;
...
static void mmi_clog_pbapc_lt_show_list(void)
{
    ...
    g_mmi_clog_pbapc_lt_inst.total_num = mmi_clog_pbap_current_num;
    is_present = mmi_frm_group_is_present(g_mmi_clog_pbapc_lt_inst.self_gid);
    ...
    //kal_prompt_trace(MOD_MMI, "mmi_clog_pbapc_lt_show_list()UPDATE = %d, is_present= %d",g_only_update_data,is_present);
    scrn_id = mmi_frm_scrn_get_active_id();
    kal_prompt_trace(MOD_MMI, "[CLOG]mmi_clog_pbapc_lt_show_list: g_exist_flag = %d,scr_id = %d",scrn_id, g_exist_flag);
    if (scrn_id == SCR_ID_CLOG_LAYOUT && g_exist_flag)
    {
        is_present = MMI_TRUE;
    }
    MMI_TRACE(MMI_CLOG_TRC_INFO, TRC_MMI_CLOG_BT_DIALER_SHOW_LIST1,g_only_update_data, is_present);
    if (!is_present)
    {
        return;
    }
    if (!g_only_update_data && is_present)
    {
        mmi_clog_pbapc_show_category(1, gui_buffer);
    }
    MMI_TRACE(MMI_CLOG_TRC_INFO, TRC_MMI_CLOG_BT_DIALER_IS_NEED_SEND_REQ,srv_bt_cm_is_profile_connected(SRV_BT_CM_PBAPC_CONNECTION));
...
void mmi_clog_pbapc_processing_abort(void)
{
    ...
    /*----------------------------------------------------------------*/
    MMI_BOOL is_in;
    kal_prompt_trace(MOD_MMI, "[@@@]mmi_clog_pbapc_processing_abort: g_only_update_data = %d, parent_id =%d" , g_only_update_data,(g_mmi_clog_pbapc_processing_inst.parent_gid));
...
static void mmi_clog_pbapc_entry_show_processing(void)
{
    ...
    //mmi_frm_scrn_clear_attribute(mmi_frm_group_get_active_id(), mmi_frm_scrn_get_active_id(), MMI_SCRN_ATTRIB_SMALL_SCRN);
...
/* under construction !*/
#endif
    //SetKeyHandler(mmi_clog_pbapc_close_scrn_by_back, KEY_RSK, KEY_EVENT_DOWN);
