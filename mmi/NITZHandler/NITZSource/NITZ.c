...
U16 mmi_get_update_status_string_from_nitz(void)
	{
		/*----------------------------------------------------------------*/
		/* Local Variables												  */
		/*----------------------------------------------------------------*/
		...
		MMI_BOOL ntp_status = MMI_TRUE;
		MMI_BOOL GPS_status = MMI_TRUE;
		#ifdef __NTPD_SUPPORT__
			ntp_status = srv_ntp_availability_get();
		#endif
		#ifdef __GPS_SUPPORT__
		GPS_status = mmi_gps_setting_get_time_sync();
		#endif
		/*----------------------------------------------------------------*/
		/* Code Body													  */
		/*----------------------------------------------------------------*/
		if(ntp_status && GPS_status && type != SRV_NITZ_OFF)
			{
				ret = STR_GLOBAL_ON;
			}
		else
		...
		//kal_prompt_trace(MOD_MMI," 06987ntp_status=%d, GPS_status = %d, ret = %d", ntp_status, GPS_status, ret);	
		return ret;
	}

void mmi_set_auto_update_time_on(void)
{
	srv_set_apply_nitz_type(SRV_NITZ_ON);
	#ifdef __GPS_SUPPORT__
	mmi_gps_setting_set_time_sync(MMI_TRUE);
	#endif
	#ifdef __NTPD_SUPPORT__
	srv_ntp_availability_set(MMI_TRUE); 
...
void mmi_set_auto_update_time_off(void)
{
	...
	#endif
	#ifdef __GPS_SUPPORT__
	mmi_gps_setting_set_time_sync(MMI_FALSE);
	#endif
}
/*****************************************************************************
 * FUNCTION
...
static mmi_ret mmi_nitz_group_proc(mmi_event_struct* evt)
{
	...
	switch(evt->evt_id)
	{
	     case EVT_ID_CUI_MENU_LIST_ENTRY:
        {
            if (menu_evt->parent_menu_id == MENU_ID_NITZ_APP)
            {
				...
				cui_menu_set_item_hidden(cui_id, MENU_ID_NITZ_APP_CONFIRM_FIRST, MMI_TRUE);//mtk06987
    ...
    case EVT_ID_CUI_MENU_ITEM_SELECT:
		...
		case EVT_ID_CUI_MENU_ITEM_CSK_SELECT:
        {
            if (menu_evt->parent_menu_id == MENU_ID_NITZ_APP)
            {
				if(0 != (menu_evt->highlighted_menu_id - MENU_ID_NITZ_APP_OFF))
					{
						mmi_set_auto_update_time_on();
					}
				else
					{
						mmi_set_auto_update_time_off();
					}
				#if 0
/* under construction !*/
				#endif
...
static void mmi_nitz_update_datetime(void)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	U32 tick_count_2, diff_second, utcSec;
    ...
    if (mmi_dt_is_valid(nitz_timeP))
{
		...
		utcSec = applib_dt_mytime_2_utc_sec((applib_time_struct*)nitz_timeP, MMI_FALSE);
    ...
    else if (mmi_nitz_check_tz(tz_valid_flag))
    {
		...
		utcSec = applib_dt_mytime_2_utc_sec(&resultTime, MMI_FALSE);
        WriteValueSlim(NVRAM_RTC_TIME, &utcSec, DS_DOUBLE);
	...
	memset(&g_nitz_cntx, 0x00, sizeof(srv_nitz_context_struct));
	srv_set_nitz_state(SRV_NITZ_IDLE);
	update_datetime_mode = MMI_UPDATE_DATETIME_NITZ;
