...
static mmi_ret mmi_alm_pwron_proc(mmi_event_struct* evt)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
static void mmi_reminder_exit_pwron_confirm(void)
{
    ...
    if (srv_reminder_is_expiring())
	{
        ...
        mmi_frm_scrn_close(GRP_ID_ALM_AUDIO_OPTION, SCR_ID_ALM_PWRON_CONFIRM);	
	}
}
...
static void mmi_reminder_entry_pwron_confirm(void)
{
	 ...
	 mmi_frm_group_create_ex(GRP_ID_ROOT, GRP_ID_ALM_AUDIO_OPTION, mmi_alm_pwron_proc, NULL,MMI_FRM_NODE_SMART_CLOSE_FLAG);
	 ...
	 kal_wap_trace(MOD_MMI,TRACE_GROUP_2,"[ALARM]mmi_reminder_entry_pwron_confirm entry");
	...
	/*
	 ...
	 ShowCategory163Screen(
		   ...
		   0,
		   (UI_string_type) GetString(ALARMS_POWER_ON),
		   mmi_get_event_based_image(MMI_EVENT_QUERY),
		   NULL);
