...
void display_nsm_mypicture_list(void)
{
    ...
    sprintf(DirPath, "%c:\\%s", SRV_FMGR_PUBLIC_DRV, (S8*) "Images\\");
    ...
    mmi_fmgr_select_path_and_enter(
        APP_MESSAGES,
        FMGR_SELECT_FILE | FMGR_FLAG_CHECK_SENDABLE,
        filter,
...
U16 mmi_nsm_save_buffer_in_file(S8 *fullfilenameInUnicode)
{
    ...
    {
        sprintf(temp, "%c:\\%s", SRV_FMGR_PUBLIC_DRV, (S8*) MYPICTURE_DIR_PATH);
    }
    if (mmi_nsm_get_msg_type() == RINGTONE_MESSAGE)
    {
        sprintf(temp, "%c:\\%s", SRV_FMGR_PUBLIC_DRV, (S8*) MY_TONE_PATH);
    }
