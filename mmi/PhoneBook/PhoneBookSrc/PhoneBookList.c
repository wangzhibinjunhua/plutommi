...
/* Global Variables                                               */
/*----------------------------------------------------------------*/
kal_bool set_first = KAL_FALSE;
kal_bool action_modify = KAL_FALSE;//KAL_TRUE;
...
/* Tab style */
    ...
    tab_pages_info_array[tab_num].tab_icon = (U8*)GetImage(IMG_ID_PHB_TAB_BT_LIST);
    tab_pages_info_array[tab_num].tab_string = NULL;
#endif
    ...
    kal_wap_trace(MOD_MMI_COMMON_APP, MMI_PHB_PBAPC_TRACE_GROUP, "[MMI_PHB_PBAPC] mmi_phb_launch_entry--phb_list: %d, tab: %d, connect: %d\n",
                                                                    phb_pbapc_cntx_p->phb_list, tab_index, srv_bt_cm_is_profile_connected(SRV_BT_CM_PBAPC_CONNECTION));
#endif
...
#if defined(__MMI_PHB_QUICK_SEARCH__)
    pbap_tab_pages_info_array[0].screen_id = SCR_ID_PHB_QUICK_SEARCH_LIST;
    pbap_tab_pages_info_array[0].tab_entry_func = mmi_phb_entry_quick_search_list_tab;
#else
    pbap_tab_pages_info_array[0].screen_id = SCR_PBOOK_LIST;
    pbap_tab_pages_info_array[0].tab_entry_func = mmi_phb_list_entry_main;
#endif

    //pbap_tab_pages_info_array[0].tab_icon = (U8*)GetImage(IMG_ID_PHB_CSK_ADD);
    //pbap_tab_pages_info_array[0].tab_icon = GetImage(IMG_SCR_PBOOK_CAPTION);
    pbap_tab_pages_info_array[0].tab_icon = NULL;
    pbap_tab_pages_info_array[0].tab_string = GetString(STR_ID_PHB_PBAPC_LOCAL);

    pbap_tab_pages_info_array[1].screen_id = SCR_ID_PHB_PBAP_LIST;
    pbap_tab_pages_info_array[1].tab_entry_func = mmi_phb_pbapc_entry_list_tab;

    //pbap_tab_pages_info_array[1].tab_icon = (U8*)GetImage(IMG_ID_PHB_CSK_EDIT);
    //pbap_tab_pages_info_array[1].tab_icon = GetImage(IMG_GLOBAL_BACK);
    pbap_tab_pages_info_array[1].tab_icon = NULL;
    pbap_tab_pages_info_array[1].tab_string = GetString(STR_ID_PHB_PBAPC_BT);
            ...
            U16 contact_count)
{
	...
	action_modify = KAL_FALSE;//KAL_TRUE;
...
static void mmi_phb_quick_search_center_key_handlr(void)
{
    ...
    /* Code Body                                                      */
	/*----------------------------------------------------------------*/
	key_length = (U8) mmi_ucs2strlen((CHAR*) g_mmi_phb_list_p->qsearch_input_buffer);
#ifndef __MMI_MAINLCD_96X64__
	if (g_phb_quick_search_enter == MMI_PHB_QUICK_SEARCH_MAIN && key_length == 0)
#else
	if (g_phb_quick_search_enter == MMI_PHB_QUICK_SEARCH_MAIN && key_length == 0 &&g_mmi_phb_list_p->contact_count==0)
...
static void mmi_phb_entry_quick_search_list(void)
{
    ...
    else
    {
        ...
        {
            gui_buffer = NULL;
			if(action_modify == KAL_FALSE && g_mmi_phb_list_p->qsearch_input_buffer[0] != 0)
			{
					...
					filter = mmi_phb_list_build_filter(
							...
							g_mmi_phb_list_p->filter,
							&g_mmi_phb_list_p->group_filter,
							(U16*)g_mmi_phb_list_p->qsearch_input_buffer,
							(U16)mmi_imm_get_curr_input_mode());
					set_first = KAL_TRUE;
			}
			else
            ...
            else
            {
                g_mmi_phb_list_p->list_index = mmi_phb_calc_list_index(
                                                ...
                                                g_mmi_phb_list_p->contact_count);
#ifndef __MMI_MAINLCD_96X64__
				if(action_modify == KAL_TRUE ||(action_modify == KAL_FALSE && set_first == KAL_FALSE))
				{
					g_mmi_phb_list_p->list_index++;
				}
#endif
			...
			if(action_modify == KAL_TRUE || g_mmi_phb_list_p->qsearch_input_buffer[0] == 0)
