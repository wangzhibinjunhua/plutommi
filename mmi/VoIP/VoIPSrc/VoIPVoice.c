...
void mmi_voip_switch_session(mmi_voip_call_state_enum currState)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_VOIP_TRC_GRP, TRC_MMI_VOIP_SWITCH_SESSION, currState);
...
S32 mmi_voip_create_rtp(voip_sdp_struct *sdp)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_VOIP_TRC_GRP, TRC_MMI_VOIP_CREATE_RTP);
    ...
    data_acnt = cbm_set_app_id(g_voip_cntx_p->prof_setting_info.saved_prof[profIndex].comm_info.dataAcct, g_voip_cntx_p->app_id);
    ...
    rtp_voip_init((S8)rtpHandle, (U16)sdpCodec, (U16)sdp->modeset[0]);
    MMI_PRINT(MOD_MMI_COMMON_APP, MMI_VOIP_TRC_GRP, "\n[mmi_voip_create_rtp] Use to Lock Resource...Start");
    mmi_voip_control_rtp(rtpHandle, VOIP_RTP_DIRECTION_SENDRECV);
    mmi_voip_control_rtp(rtpHandle, VOIP_RTP_DIRECTION_INACTIVE);
...
void mmi_voip_close_rtp(S32 rtpHandle)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_VOIP_TRC_GRP, TRC_MMI_VOIP_CLOSE_RTP, rtpHandle, voip_curr_voip_state((S8) rtpHandle));
...
void mmi_voip_switch_rtp(MMI_BOOL isSuspend, S32 callId, S32 dialogId)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_VOIP_TRC_GRP, TRC_MMI_VOIP_SWITCH_RTP, isSuspend, callId, dialogId);
    ...
    //i = g_voip_cntx_p->dtmf_queue_info.timerStatus;
    //memset(&g_voip_cntx_p->dtmf_queue_info, 0, sizeof(mmi_voip_dtmf_queue_struct));
    //g_voip_cntx_p->dtmf_queue_info.timerStatus = i;
    ...
    if (callId != -1)
    {
        ...
        if (dialogId != -1)
        {
            ...
            if (rtpHandle != -1)
            {
                ...
                if(isSuspend == TRUE)
                {
                    g_voip_cntx_p->call_list_info.call_info[callIndex].dialog_info[dialogIndex].sdp_info.direction = VOIP_RTP_DIRECTION_INACTIVE;
                }
                rtpDirection = g_voip_cntx_p->call_list_info.call_info[callIndex].dialog_info[dialogIndex].sdp_info.direction;
                /*
                rtpDirection =
        ...
        for (i = 0; i < g_voip_cntx_p->call_list_info.numTotal; i++)
        {
            for (j = 0; j < g_voip_cntx_p->call_list_info.call_info[i].numDialog; j++)
            {
                ...
                if (rtpHandle != -1)
                {
                    ...
                    if(isSuspend == TRUE)
                    {
                        g_voip_cntx_p->call_list_info.call_info[i].dialog_info[j].sdp_info.direction = VOIP_RTP_DIRECTION_INACTIVE;
                    }
                    rtpDirection = g_voip_cntx_p->call_list_info.call_info[i].dialog_info[j].sdp_info.direction;
                    /*
                    rtpDirection =
...
void mmi_voip_control_rtp(S32 rtpHandle, S32 rtpDirection)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    kal_uint8 voip_state = voip_curr_voip_state((S8) rtpHandle);

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_VOIP_TRC_GRP, TRC_MMI_VOIP_CONTROL_RTP, rtpHandle, rtpDirection, voip_state);

    switch (rtpDirection)
    {
        case VOIP_RTP_DIRECTION_SENDRECV:                   /*  send and receive */
            switch (voip_state)   /* ensure not to set encoding / decoding twice */
            {
                case VOIP_IDLE:
...
void mmi_voip_remove_mixer_before_close(S32 callIndex)
{
    ...
    for (i = 0; i < VOIP_MAX_NUM_DIALOG; i++)
    {
        ...
        {
            MMI_TRACE(MMI_VOIP_TRC_GRP, TRC_MMI_VOIP_REMOVE_MIXER_BEFORE_CLOSE, callIndex, i);
 ...
 *  void
 *****************************************************************************/
void mmi_voip_dtmf_key_down(U16 keyCode)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_VOIP_TRC_GRP, TRC_MMI_VOIP_DTMF_KEY_DOWN, g_voip_cntx_p->dtmf_queue_info.timerStatus);
...
void mmi_voip_dtmf_key_up(void)
{
    ...
    MMI_TRACE(MMI_VOIP_TRC_GRP, TRC_MMI_VOIP_DTMF_KEY_UP, keyCode, keyType, g_voip_cntx_p->dtmf_queue_info.timerStatus);
...
void mmi_voip_send_dtmf_start(U16 keyCode)
{
    ...
    for (i = 0; i < numDialog; i++)
    {
        ...
        if (g_voip_cntx_p->call_list_info.call_info[currhiliteTab].dialog_info[i].sdp_info.direction == VOIP_RTP_DIRECTION_SENDRECV)
        {
            ...
            g_voip_cntx_p->dtmf_queue_info.rtpHandle[i] = rtpHandle[i];
            ...
            for (i = 0; i < numDialog; i++)
            {
                if (rtpHandle[i] != -1)
                {
                    mdi_audio_speech_dtmf_start(
                        (S8)rtpHandle[i],
                        (U8)mmi_voip_get_dtmf_keycode_enum(&keyCode),
                        (U8)RTP_DTMF_TYPE_INBAND);
                }
            }
        }
...
void mmi_voip_send_dtmf_stop(void)
{
    ...
    for(i = 0; i < VOIP_MAX_NUM_DIALOG; i++)
    {
        U16 rtp_status = voip_curr_voip_state(g_voip_cntx_p->dtmf_queue_info.rtpHandle[i]);
        if(rtp_status == VOIP_ENCODE || rtp_status == VOIP_ENCODE_DECODE)
        {
            rtpHandle[i] = g_voip_cntx_p->dtmf_queue_info.rtpHandle[i];
        }
        else
        {
            rtpHandle[i] = -1;
        }
        g_voip_cntx_p->dtmf_queue_info.rtpHandle[i] = -1;
    }
    if (g_voip_cntx_p->call_setting_info.saved_setting.dtmf != VOIP_DTMF_TYPE_NONE)
    {
        if (g_voip_cntx_p->call_misc_info.isMute == FALSE)
        ...
        for (i = 0; i < VOIP_MAX_NUM_DIALOG; i++)
        {
            if (rtpHandle[i] != -1)
            {
                mdi_audio_speech_dtmf_stop((S8)rtpHandle[i]);
            }
        }
    }
