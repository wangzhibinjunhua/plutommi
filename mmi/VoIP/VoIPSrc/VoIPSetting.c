...
void mmi_voip_highlight_status(void)
{
    ...
    SetRightSoftkeyFunction(GoBackHistory, KEY_EVENT_UP);
    SetKeyHandler(mmi_voip_entry_status, KEY_RIGHT_ARROW, KEY_EVENT_DOWN);
    SetKeyHandler(GoBackHistory, KEY_LEFT_ARROW, KEY_EVENT_DOWN);
}
...
void mmi_voip_highlight_summary(void)
{
    ...
    SetRightSoftkeyFunction(GoBackHistory, KEY_EVENT_UP);
    SetKeyHandler(mmi_voip_entry_summary, KEY_RIGHT_ARROW, KEY_EVENT_DOWN);
    SetKeyHandler(GoBackHistory, KEY_LEFT_ARROW, KEY_EVENT_DOWN);
}
...
/*****************************************************************************
 * FUNCTION
 *  mmi_voip_highlight_prof_edit_rtp
 * DESCRIPTION
 *  Highlight handler of VoIP\Profile Setting\Profile X\Edit\RTP Setting
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void mmi_voip_highlight_prof_edit_rtp(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    g_voip_cntx_p->prof_setting_info.currScr = SCR_ID_VOIP_PROF_RTP;
...
void mmi_voip_highlight_full_screen_editor(void)
{
    ...
    SetRightSoftkeyFunction(GoBackHistory, KEY_EVENT_UP);
#ifdef __MMI_TOUCH_SCREEN__
    mmi_imm_set_events(MMI_FALSE, g_voip_disable_new_line);
#endif
}
...
void mmi_voip_entry_main_menu(void)
{
    ...
    {
        mmi_frm_hide_menu_item(MENU_ID_VOIP_STATUS);
        mmi_frm_hide_menu_item(MENU_ID_VOIP_CALL_SETTING);
    }
    else
    {
        mmi_frm_unhide_menu_item(MENU_ID_VOIP_STATUS);
        mmi_frm_unhide_menu_item(MENU_ID_VOIP_CALL_SETTING);
    }
    ...
    if (g_voip_cntx_p->reg_state_info == MMI_VOIP_REG_STATE)
    {
        ...
        {
            if (nStrItemList[i] == STR_GLOBAL_LOGIN)
            {
                nStrItemList[i] = STR_GLOBAL_LOGOUT;   /* change login string to logout if VoIP is login */
            }
        }
    }
...
void mmi_voip_entry_status(void)
{
    ...
    {
        mmi_display_popup(
            get_string(STR_ID_VOIP_NO_ACTIVATED_PROFILE),
            MMI_EVENT_FAILURE);
...
static MMI_ID g_mmi_voip_aa_id = 0;
static mmi_ret mmi_voip_always_ask_cui_proc(mmi_event_struct *evt) 
{
    cui_evt_cbm_bearer_select_struct *cbm_event = (cui_evt_cbm_bearer_select_struct*) evt;

    switch(evt->evt_id)
    {
        case EVT_ID_CUI_CBM_OK:
            mmi_voip_account_select_notify_cb(cbm_event->selected_account_id, KAL_TRUE);
            cui_cbm_always_ask_close(g_mmi_voip_aa_id);
            GoBackHistory();
            break;
        case EVT_ID_CUI_CBM_CANCEL:
        case EVT_ID_CUI_CBM_FAIL:
            mmi_voip_account_select_notify_cb(cbm_event->selected_account_id, KAL_FALSE);
            cui_cbm_always_ask_close(g_mmi_voip_aa_id);
            GoBackHistory();
            break;
    }
...
mmi_ret mmi_voip_bearer_info_ind(mmi_event_struct *param)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    switch (bearer_info_p->state)
    {
        ...
        {/*
            mmi_evt_mmi_voip_abort_struct evt;
            MMI_FRM_INIT_EVENT(&evt, EVT_ID_VOIP_ABORT);
            mmi_frm_post_event((mmi_event_struct *)&evt, mmi_voip_abort_post_event_hdlr, NULL);
            */
        }
        break;
...
mmi_ret mmi_voip_bear_event_callback(mmi_event_struct* para)
{
    srv_cbm_bearer_event_struct *info = (srv_cbm_bearer_event_struct*) para;
    cui_cbm_parameter_struct param;
    MMI_ID g_app_id;

    if(info->type == SRV_CBM_BEARER_ALWAYS_ASK)
    {
        g_app_id = mmi_frm_group_create(GRP_ID_ROOT, GRP_ID_VOIP_PROF_EDIT_DA_ALWAYS_ASK, mmi_voip_always_ask_cui_proc, NULL);
        mmi_frm_group_enter(g_app_id, 0);
        g_mmi_voip_aa_id = cui_cbm_always_ask_create(g_app_id);
        param.account_id = info->account_id;//g_voip_cntx_p->prof_setting_info.saved_prof[g_voip_cntx_p->prof_setting_info.currprofIndex].comm_info.dataAcct;
        param.res.icon_id = IMG_ID_VOIP_MAIN;
        param.res.str_id = STR_ID_VOIP_MAIN;
        cui_cbm_always_ask_set_param(g_mmi_voip_aa_id, &param);
        cui_cbm_always_ask_set_bearer_filter(g_mmi_voip_aa_id, SRV_CBM_BEARER_WIFI);
        cui_cbm_always_ask_run(g_mmi_voip_aa_id);
        srv_cbm_register_bearer_info(g_voip_cntx_p->app_id, 
            SRV_CBM_DEACTIVATED | SRV_CBM_DEACTIVATING | SRV_CBM_ACTIVATING |SRV_CBM_ACTIVATED,
            SRV_CBM_BEARER_WIFI, 
            mmi_voip_bearer_info_ind,
            NULL);
    }
    return SRV_CBM_OK;
}
...
/*****************************************************************************
 * FUNCTION
 *  mmi_voip_account_select_notify_cb
 * DESCRIPTION
 *  Entry function when auto login timer expiry.
 * PARAMETERS
 *  account_id        the data account id user select in always-ask interface
 *  is_valid          if user select a data account id in always-ask interface
 * RETURNS
 *  void
 *****************************************************************************/
void mmi_voip_account_select_notify_cb (kal_uint32 account_id, kal_bool is_valid)
{
    if(is_valid)
    {
        /* User actually choose a data account */
        if(srv_dtcnt_get_bearer_type(account_id, SRV_DTCNT_ACCOUNT_PRIMARY) == DATA_ACCOUNT_BEARER_CSD)
        {
            GoBackHistory();
            mmi_voip_entry_abort_login();
        }
        ...
        {
            g_voip_cntx_p->prof_setting_info.currDataAcct = account_id;
        }
    }
    else
    {
        /* User cancel the selection */
        /* when stay in always ask, wlan access point screen
           if press end key, then will delete the screen,
           in delete callback, this function will call, then will enter voip processing screen
           use post event to prevent insert screen during delete back to idle */
        mmi_evt_mmi_voip_abort_struct evt;
        MMI_FRM_INIT_EVENT(&evt, EVT_ID_VOIP_ABORT);
        mmi_frm_post_event((mmi_event_struct *)&evt, mmi_voip_abort_post_event_hdlr, NULL);
    }
...
void mmi_voip_entry_auto_login(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_COMMON_TRC_G5_CALL, TRC_MMI_VOIP_AUTO_LOGIN);
    ...
    if ((profIndex != -1) && (g_voip_cntx_p->prof_setting_info.saved_prof[profIndex].comm_info.autoLogin == 1)) /* auto login */
    {
        ...
        {
            MMI_TRACE(MMI_COMMON_TRC_G5_CALL, TRC_MMI_VOIP_AUTO_LOGIN_START_TIMER);
        ...
        {
            MMI_TRACE(MMI_COMMON_TRC_G5_CALL, TRC_MMI_VOIP_AUTO_LOGIN_SHOW_PROCESSING);
            mmi_voip_set_processing_parameter(
                STR_GLOBAL_LOGIN,
                STR_ID_VOIP_CONNECT_TO_SERVER,
                IMG_GLOBAL_PROGRESS,
    ...
    {
        MMI_TRACE(MMI_COMMON_TRC_G5_CALL, TRC_MMI_VOIP_NO_AUTO_LOGIN);
    }
}
...
void mmi_voip_entry_abort_login(void)
{
    ...
    g_voip_cntx_p->proc_scr_info.cause = MMI_VOIP_PROC_CAUSE_ABORT_LOGIN;
    mmi_voip_deregister_req();
}
...
/*****************************************************************************
 * FUNCTION
 *  mmi_voip_abort_post_event_hdlr
 * DESCRIPTION
 *  abprt post event handler
 * PARAMETERS
 *  para        event struct
 * RETURNS
 *  void
 *****************************************************************************/
mmi_ret mmi_voip_abort_post_event_hdlr(mmi_event_struct *para)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/ 
    MMI_TRACE(MMI_VOIP_TRC_GRP, TRC_MMI_VOIP_ABORT_POST_EVENT_HDLR);
    mmi_voip_entry_abort_login(); /*the currect active screen must be incall screen, so no need check again*/
    return MMI_RET_OK;
}
...
void mmi_voip_entry_summary(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    EntryNewScreen(SCR_ID_VOIP_SUMMARY, NULL, mmi_voip_entry_summary, NULL);
...
void mmi_voip_entry_prof_activate(void)
{
    ...
    mmi_display_popup(
        get_string(STR_GLOBAL_ACTIVATED),
        MMI_EVENT_SUCCESS);
...
void mmi_voip_pre_entry_prof_edit_inlineitems(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if(mmi_dtcnt_ready_check(mmi_voip_entry_prof_edit_inlineitems, MMI_FALSE))
    {
...
void mmi_voip_entry_prof_edit_inlineitems(void)
{
    ...
    switch (currScreen)
    {
        ...
        case SCR_ID_VOIP_PROF_NAT:
            ...
            break;
        case SCR_ID_VOIP_PROF_RTP:
            if (inputBuffer == NULL)
            {
                mmi_voip_update_prof_rtp_cache_to_disp(profIndex);
            }
            titleId = STR_ID_VOIP_PROF_RTP_PORT_RANGE;
            imgId = IMG_ID_VOIP_PROF_RTP_PORT;
            numItems = mmi_voip_setup_prof_rtp_inlineitems();
            break;
        default:
            MMI_ASSERT(0);
    }
    ...
    if (currScreen == SCR_ID_VOIP_PROF_COMMON)
    {
        if (cbm_is_always_ask_account(g_voip_cntx_p->prof_setting_info.disp_prof.dataacctIndex) ||
            mmi_voip_get_decoded_dtnct_id(g_voip_cntx_p->prof_setting_info.disp_prof.dataacctIndex)== CBM_DEFAULT_ACCT_ID )
        {
            if (inputBuffer != NULL)
            {
                g_voip_cntx_p->prof_setting_info.disp_prof.selectionIndex = 0;
            ...
            DisableInlineItem(&(wgui_inline_items[MMI_VOIP_PROF_COMMON_AUTOLOGIN]), MMI_VOIP_PROF_COMMON_AUTOLOGIN);
        }
    }
...
void mmi_voip_exit_prof_edit_inlineitems(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    GenericExitInlineScreen(SCR_ID_VOIP_PROF_EDIT_INLINEITEMS, mmi_voip_entry_prof_edit_inlineitems);
}
...
void mmi_voip_update_prof_nat_cache_to_disp(S32 profIndex)
{
    ...
    mmi_voip_prof_ip_three_digit_string(
...
/*****************************************************************************
 * FUNCTION
 *  mmi_voip_update_prof_rtp_cache_to_disp
 * DESCRIPTION
 *  Copy g_voip_cntx_p->prof_setting_info.saved_prof[index] to
 *  g_voip_cntx_p->prof_setting_info.disp_prof for RTP Setting.
 * PARAMETERS
 *  profIndex           [IN]            Index of intended to update profile
 * RETURNS
 *  void
 *****************************************************************************/
void mmi_voip_update_prof_rtp_cache_to_disp(S32 profIndex)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    memset(g_voip_cntx_p->prof_setting_info.disp_prof.start_port, 0, (MMI_VOIP_MAX_PORT_LEN * ENCODING_LENGTH));
    gui_itoa(
        g_voip_cntx_p->prof_setting_info.saved_prof[profIndex].rtp_info.start_port,
        (UI_string_type)g_voip_cntx_p->prof_setting_info.disp_prof.start_port,
        10);

    memset(g_voip_cntx_p->prof_setting_info.disp_prof.end_port, 0, (MMI_VOIP_MAX_PORT_LEN * ENCODING_LENGTH));
    gui_itoa(
        g_voip_cntx_p->prof_setting_info.saved_prof[profIndex].rtp_info.end_port,
        (UI_string_type)g_voip_cntx_p->prof_setting_info.disp_prof.end_port,
        10);
}
...
/*****************************************************************************
 * FUNCTION
 *  mmi_voip_update_prof_rtp_disp_to_cache
 * DESCRIPTION
 *  Copy g_voip_cntx_p->prof_setting_info.disp_prof to
 *  g_voip_cntx_p->prof_setting_info.saved_prof[index] for RTP Setting.
 * PARAMETERS
 *  profIndex           [IN]            Index of intended to update profile
 * RETURNS
 *  void
 *****************************************************************************/
void mmi_voip_update_prof_rtp_disp_to_cache(S32 profIndex)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /*----------------------------------------------------------------*/
    g_voip_cntx_p->prof_setting_info.saved_prof[profIndex].rtp_info.start_port
        = gui_atoi((UI_string_type) g_voip_cntx_p->prof_setting_info.disp_prof.start_port);
    g_voip_cntx_p->prof_setting_info.saved_prof[profIndex].rtp_info.end_port
        = gui_atoi((UI_string_type) g_voip_cntx_p->prof_setting_info.disp_prof.end_port);
}
...
S32 mmi_voip_setup_prof_common_inlineitems(void)
{
    ...
    mmi_dtcnt_get_full_account_name(g_voip_cntx_p->prof_setting_info.disp_prof.dataacctIndex,
            (S8*)g_voip_cntx_p->prof_setting_info.disp_prof.dataacctName,
            MAX_DATA_ACCOUNT_NAME_LEN, MMI_DTCNT_GET_ACCT_NAME_TYPE_SINGLE); 
/*
    acct_data.dest_len = (MAX_DATA_ACCOUNT_NAME_LEN + 1) * ENCODING_LENGTH;
    acct_data.dest = (S8*)g_voip_cntx_p->prof_setting_info.disp_prof.dataacctName;
...
S32 mmi_voip_setup_prof_acct_inlineitems(void)
{
    ...
    SetInlineItemCaption(
        ...
        (U8*) GetString(STR_GLOBAL_PASSWORD));
 ...
 *  Number of inline items used in NAT Setting.
 *****************************************************************************/
S32 mmi_voip_setup_prof_rtp_inlineitems(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Start Port Caption*/
    SetInlineItemCaption(
        wgui_inline_items + MMI_VOIP_PROF_RTP_PORT_START_CAP,
        (U8*) GetString(STR_ID_VOIP_PROF_RTP_PORT_START));

    /* Start Port */
    SetInlineItemActivation(wgui_inline_items + MMI_VOIP_PROF_RTP_PORT_START, INLINE_ITEM_ACTIVATE_WITHOUT_KEY_EVENT, 0);
    SetInlineItemTextEdit(
        wgui_inline_items + MMI_VOIP_PROF_RTP_PORT_START,
        g_voip_cntx_p->prof_setting_info.disp_prof.start_port,
        MMI_VOIP_MAX_PORT_LEN,
        IMM_INPUT_TYPE_NUMERIC);
    //RightJustifyInlineItem(wgui_inline_items + MMI_VOIP_PROF_RTP_PORT_START);
    EnableInlineItemBoundary(wgui_inline_items + MMI_VOIP_PROF_RTP_PORT_START);
    DisableInlineItemHighlight(wgui_inline_items + MMI_VOIP_PROF_RTP_PORT_START);

    /* End Port Caption*/
    SetInlineItemCaption(
        wgui_inline_items + MMI_VOIP_PROF_RTP_PORT_END_CAP,
        (U8*) GetString(STR_ID_VOIP_PROF_RTP_PORT_END));

    /* End Port */
    SetInlineItemActivation(wgui_inline_items + MMI_VOIP_PROF_RTP_PORT_END, INLINE_ITEM_ACTIVATE_WITHOUT_KEY_EVENT, 0);
    SetInlineItemTextEdit(
        wgui_inline_items + MMI_VOIP_PROF_RTP_PORT_END,
        g_voip_cntx_p->prof_setting_info.disp_prof.end_port,
        MMI_VOIP_MAX_PORT_LEN,
        IMM_INPUT_TYPE_NUMERIC);
    //RightJustifyInlineItem(wgui_inline_items + MMI_VOIP_PROF_RTP_PORT_END);
    EnableInlineItemBoundary(wgui_inline_items + MMI_VOIP_PROF_RTP_PORT_END);
    DisableInlineItemHighlight(wgui_inline_items + MMI_VOIP_PROF_RTP_PORT_END);
    /* EnableInlineItemDefaultText(); */

    return MMI_VOIP_PROF_RTP_PORT_INLINE_TOTAL;
}
...
static MMI_ID mmi_voip_dtcnt_select_id;

static mmi_ret mmi_voip_dtcnt_cui_proc(mmi_event_struct *evt) 
{
    ...
    switch (evt->evt_id)
    {
        case CUI_DTCNT_SELECT_EVENT_RESULT_OK:
            g_voip_cntx_p->prof_setting_info.currDataAcct = event->account_id;
            g_voip_cntx_p->prof_setting_info.disp_prof.dataacctIndex = event->account_id;

            mmi_dtcnt_get_full_account_name(event->account_id,
                    (S8*)g_voip_cntx_p->prof_setting_info.disp_prof.dataacctName,
                    MAX_DATA_ACCOUNT_NAME_LEN, MMI_DTCNT_GET_ACCT_NAME_TYPE_SINGLE);              

                cui_dtcnt_select_close(mmi_voip_dtcnt_select_id);
...
void mmi_voip_entry_prof_data_acct(void)
{
    ...
    /*----------------------------------------------------------------*/
    MMI_ID ret_id;
    para.account_id = g_voip_cntx_p->prof_setting_info.disp_prof.dataacctIndex;
    para.AppMenuID = MENU_ID_VOIP_PROF_SETTING;
    para.app_id = g_voip_cntx_p->app_id;
    para.bearers = DATA_ACCOUNT_BEARER_WIFI | DATA_ACCOUNT_BEARER_ALWAYS_ASK;
    ...
    para.icon_id = GetRootTitleIcon(MENU_ID_VOIP_MAIN);

    //mmi_dtcnt_select_account(mmi_voip_prof_data_acct_callback, MENU_ID_VOIP_MAIN, DATA_ACCOUNT_BEARER_GPRS|DATA_ACCOUNT_BEARER_WIFI, 0, 0, DTCNT_SELECT_DEFAULT);

    ret_id = mmi_frm_group_create(GRP_ID_ROOT, GRP_ID_VOIP_PROF_EDIT_DA, mmi_voip_dtcnt_cui_proc, NULL);
    if (!ret_id)
    {
       mmi_display_popup((UI_string_type)get_string(STR_GLOBAL_NOT_AVAILABLE),MMI_EVENT_FAILURE);
       return;
    }
    ret_id = mmi_frm_group_enter(GRP_ID_VOIP_PROF_EDIT_DA, MMI_FRM_NODE_SMART_CLOSE_FLAG);
    if (!ret_id)
    {
        mmi_display_popup((UI_string_type)get_string(STR_GLOBAL_NOT_AVAILABLE),MMI_EVENT_FAILURE);
        return;
    ...
    mmi_voip_dtcnt_select_id = cui_dtcnt_select_create(GRP_ID_VOIP_PROF_EDIT_DA);
    if(mmi_voip_dtcnt_select_id != GRP_ID_INVALID)
    {
        cui_dtcnt_select_set_param(mmi_voip_dtcnt_select_id, &para);
        cui_dtcnt_select_run(mmi_voip_dtcnt_select_id);
    }
    else
    {
        mmi_frm_group_close(GRP_ID_VOIP_PROF_EDIT_DA);
    }
}
...
void mmi_voip_prof_data_acct_callback(U32 index)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    g_voip_cntx_p->prof_setting_info.currDataAcct = index;
    g_voip_cntx_p->prof_setting_info.disp_prof.dataacctIndex = index;

    //Leo
    //mmi_dtcnt_register_deletion_notify(g_voip_cntx_p->app_id, mmi_voip_get_decoded_dtnct_id(index), mmi_voip_dtcnt_delete_cb);
 ...
 *  mmi_voip_prof_ip_three_digit_string
 * DESCRIPTION
 *  Convert ip integer value to three digit string with prefix zero.
 * PARAMETERS
 *  ipStr           [IN/OUT]        Three digit ip string in unicode
 *  ipInt           [IN]            Ip integer
 * RETURNS
 *  void
 *****************************************************************************/
void mmi_voip_prof_ip_three_digit_string(U8 *ipStr, U8 ipInt)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (ipInt < 10)
    {
        sprintf((S8*)zeroPrefix, "00%d", ipInt);
        mmi_asc_to_ucs2((S8*)ipStr, (S8*)zeroPrefix);
    }
    else if (ipInt < 100)
    {
        sprintf((S8*)zeroPrefix, "0%d", ipInt);
        mmi_asc_to_ucs2((S8*)ipStr, (S8*)zeroPrefix);
    }
    else
    {
        sprintf((S8*)zeroPrefix, "%d", ipInt);
        mmi_asc_to_ucs2((S8*)ipStr, (S8*)zeroPrefix);
    }
}
...
U16 mmi_voip_prof_validate_activation(S32 profIndex)
{
    ...
    if (voip_custom_get_p2p_call_state() == FALSE)
    {
        ...
        if(g_voip_cntx_p->prof_setting_info.saved_prof[g_voip_cntx_p->prof_setting_info.currprofIndex].comm_info.dataAcct == MMI_DTCNT_INVALID_ID)
        {
            errorstrId = mmi_dtcnt_get_invalid_name_str_id();
        }
    }
...
U16 mmi_voip_prof_validate_inputs(void)
{
    ...
    U16 currScreen = g_voip_cntx_p->prof_setting_info.currScr;
    U16 errorstrId = 0;
    MMI_BOOL is_always_ask = MMI_FALSE;
    ...
    switch (currScreen)
    {
        case SCR_ID_VOIP_PROF_COMMON:
            ...
            is_always_ask = cbm_is_always_ask_account(g_voip_cntx_p->prof_setting_info.disp_prof.dataacctIndex);
            if (is_always_ask && g_voip_cntx_p->prof_setting_info.disp_prof.selectionIndex == 1) /* auto-login */
            {
                errorstrId = STR_ID_VOIP_INVALID_AUTO_LOGIN;
            }
        ...
        case SCR_ID_VOIP_PROF_NAT:
            ...
            break;
        case SCR_ID_VOIP_PROF_RTP:
            if (mmi_voip_validate_port(g_voip_cntx_p->prof_setting_info.disp_prof.start_port) == FALSE ||
                mmi_voip_validate_port(g_voip_cntx_p->prof_setting_info.disp_prof.end_port) == FALSE)
            {
                errorstrId = STR_ID_VOIP_INVALID_PORT_NUMBER;
            }
            start_port = gui_atoi((UI_string_type)g_voip_cntx_p->prof_setting_info.disp_prof.start_port);
            end_port = gui_atoi((UI_string_type)g_voip_cntx_p->prof_setting_info.disp_prof.end_port);
            
            if(end_port < start_port)
            {
                errorstrId = STR_ID_VOIP_INVALID_PORT_NUMBER;
            }
            else if(end_port - start_port < 12)
            {
                errorstrId = STR_ID_VOIP_INVALID_PORT_NUMBER;
            }
            break;
...
void mmi_voip_entry_prof_save(void)
{
    ...
    switch (currScreen)
    {
        ...
        case SCR_ID_VOIP_PROF_NAT:
            ...
            break;
        case SCR_ID_VOIP_PROF_RTP:
            mmi_voip_update_prof_rtp_disp_to_cache(profIndex);
            break;
        default:
            MMI_ASSERT(0);
    }
...
void mmi_voip_dtcnt_delete_cb(U8 app_id, U32 account_id, U8 bearer)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                                                  */
    /*----------------------------------------------------------------*/
    ...
    for(idx = 0; idx < MMI_VOIP_MAX_PROFILE_NUM; idx ++)
    {
        if(mmi_voip_get_decoded_dtnct_id(g_voip_cntx_p->prof_setting_info.saved_prof[idx].comm_info.dataAcct) == account_id)
        {
            g_voip_cntx_p->prof_setting_info.saved_prof[idx].comm_info.dataAcct = MMI_DTCNT_INVALID_ID;
            g_voip_cntx_p->prof_setting_info.currDataAcct = MMI_DTCNT_INVALID_ID;

            if(g_voip_cntx_p->prof_setting_info.actprofIndex == idx)
            {
