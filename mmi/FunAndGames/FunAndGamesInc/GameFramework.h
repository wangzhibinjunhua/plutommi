...
#define VibratorOff()   srv_vibrator_off()
#define PlayVibratorOnce()  srv_vibrator_play_once()

#define GFX_ABS(a)  ((a) < 0 ? -(a) : (a))

#define __GAME_DEBUG__
#ifdef __GAME_DEBUG__
#ifndef __MTK_TARGET__
#define GAME_TRACE(format, ...) printf((format), ##__VA_ARGS__)
#else
#define GAME_TRACE(...) kal_prompt_trace(MOD_IDLE, __VA_ARGS__)
#endif
#else
#define GAME_TRACE(format, ...)
#endif 
...
typedef struct
{
    ...
    U16 level_nvram_id;                             /* current lvl idnex stored in NVRAM */
...
*****************************************************************************/
     ...
     extern S16 mmi_gfx_cal_grade(S16 OrgGrade, S16 AddValue); /*calculate grade to avoid overflow */
     ...
     extern MMI_BOOL mmi_gfx_is_background_call(void);
...
do{                                                     \
   if(GFX.is_vib_on && GFX.is_first_time_enter_gameover && !mmi_gfx_is_background_call())\
   {                                                    \
      PlayVibratorOnce();                               \
   }                                                    \
}while(0)
...
#define GFX_OPEN_MULTICHANNEL_MIDI(raw_data, size, repeats,handleno)            \
   do{                                                                          \
      ...
      {                                                                         \
      handleno=mdi_audio_mma_open_string(0,                                     \
                                      (void*) raw_data,                         \
                                      (U32)size,                                \
                                      MDI_FORMAT_SMF,                           \
                                      repeats,                                  \
...
#define GFX_OPEN_MULTICHANNEL_WAV(raw_data, size, repeats,handleno) \
      do{                                                           \
      if(GFX.is_sound_effect_on && !mmi_gfx_is_background_call())   \
      {                                                             \
      handleno=mdi_audio_mma_open_string(0,                         \
                                      (void*) raw_data,             \
                                      (U32)size,                    \
...
extern U32 mmi_gfx_get_cur_time(void);
