...
#include "BtcmSrvGprot.h" /* SRV_BT_CM_HSP_CONNECTION */
...
#if (defined( __MMI_BT_DIALER_SUPPORT__) && defined(__MMI_TELEPHONY_SUPPORT__))
MMI_BOOL is_show_call_select_srcn = MMI_FALSE;
...
U16 mmi_ucm_call_launch(U16 caller_gid, mmi_ucm_make_call_para_struct *make_call_para)
{
	...
	if (!mmi_ucm_is_emergency_number(make_call_para->dial_type, make_call_para->ucs2_num_uri)
		&& !is_show_call_select_srcn && parent_gid != GRP_ID_UCM_MO_SELECT)
	{
		kal_prompt_trace(MOD_MMI, "[UCM] @@@@ mmi_ucm_call_launch, will enter menu");
		...
		memset(&g_make_call_para_bt, 0, sizeof(mmi_ucm_make_call_para_struct));
		...
		mmi_frm_group_create_ex(GRP_ID_ROOT, GRP_ID_UCM_MO_SELECT, mmi_ucm_call_select_sg_proc, NULL, MMI_FRM_NODE_NONE_FLAG);
		menu_id = cui_menu_create(GRP_ID_UCM_MO_SELECT,
