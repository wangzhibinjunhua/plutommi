...
#endif 
#ifdef __MMI_BT_DIALER_SUPPORT__
static MMI_BOOL is_menu_selected = MMI_FALSE;
static U32 call_select_menu_index = 0;
...
void mmi_ucm_exec_dial_cb_by_result(srv_ucm_result_enum result)
{
    ...
    if (g_ucm_p->mo_info.callback != NULL)
    {
    	...
    	if (g_make_call_para_bt.adv_para.after_make_call_callback == NULL)
		{
			kal_prompt_trace(MOD_MMI, "[UCM] @@@@ global callback is NULL ui");
			g_ucm_p->mo_info.callback = NULL;
		}
		else
	#endif /* __MMI_BT_DIALER_SUPPORT__ && __MMI_TELEPHONY_SUPPORT__ */
		{
	    	kal_prompt_trace(MOD_MMI, "[UCM] @@@@ ucm callback ui");
	        make_call_cb_para.result = result;
	        make_call_cb_para.user_data = g_ucm_p->mo_info.callback_para;
	        g_ucm_p->mo_info.callback(&make_call_cb_para);
	        g_ucm_p->mo_info.callback = NULL; /* Make sure the callback is called once */
		#if (defined ( __MMI_BT_DIALER_SUPPORT__ ) && defined (__MMI_TELEPHONY_SUPPORT__))
			g_make_call_para_bt.adv_para.after_make_call_callback = NULL;
		#endif /* __MMI_BT_DIALER_SUPPORT__ && __MMI_TELEPHONY_SUPPORT__ */
		}
    }
#if (defined (__MMI_BT_DIALER_SUPPORT__) && defined(__MMI_TELEPHONY_SUPPORT__))
...
void mmi_ucm_incoming_call_sendkey(void)
{
	...
	if ((mmi_ucm_query_call_count_by_call_state_bt(
			(srv_ucm_bt_call_state_enum)(SRV_UCM_BT_ACTIVE_STATE | SRV_UCM_BT_HOLD_STATE), NULL) > 0)
...
#if (defined( __MMI_BT_DIALER_SUPPORT__) && defined(__MMI_TELEPHONY_SUPPORT__))
void mmi_ucm_call_select_sendkey_hdlr(void)
{
	if (call_select_menu_index == 0)
	{
		mmi_ucm_call_launch(0,&g_make_call_para_bt);
	}
	else if (call_select_menu_index == 1 && srv_bt_cm_is_profile_connected(SRV_BT_CM_HSP_CONNECTION))
	{
		mmi_ucm_call_launch_set_para(g_make_call_para_bt.ucs2_num_uri, 
								g_make_call_para_bt.adv_para.after_make_call_callback,
								g_make_call_para_bt.adv_para.after_callback_user_data);
		g_make_call_para_bt.adv_para.after_make_call_callback = NULL;
...
mmi_ret mmi_ucm_call_select_menu_proc(mmi_event_struct *evt)
{
	...
	switch (menu_evt->evt_id)
	{
		case EVT_ID_CUI_MENU_LIST_ENTRY:
		{
			is_show_call_select_srcn = MMI_FALSE;
			is_menu_selected = MMI_FALSE;
			call_select_menu_index = 0;
			if (srv_sim_ctrl_is_inserted(MMI_SIM1) && srv_mode_switch_is_network_service_available())
			{
				cui_menu_set_item_disabled(menu_evt->sender_id, MENU_ID_UCM_FROM_LOCAL_CALL, MMI_FALSE);
			}
		#if (MMI_MAX_SIM_NUM == 2)
			...
			{
				cui_menu_set_item_disabled(menu_evt->sender_id, MENU_ID_UCM_FROM_LOCAL_CALL, MMI_TRUE);
			}
			if (!srv_bt_cm_is_profile_connected(SRV_BT_CM_HSP_CONNECTION)
				|| !mmi_ucm_is_valid_num_bt(g_make_call_para_bt.ucs2_num_uri))
			{
				cui_menu_set_item_disabled(menu_evt->sender_id, MENU_ID_UCM_FROM_BT_CALL, MMI_TRUE);
			}
			else
			{
				cui_menu_set_item_disabled(menu_evt->sender_id, MENU_ID_UCM_FROM_BT_CALL, MMI_FALSE);
			}
		...
		{
			is_menu_selected = MMI_TRUE;
            switch(menu_evt->highlighted_menu_id)
            {   
                case MENU_ID_UCM_FROM_LOCAL_CALL:
                {
					kal_prompt_trace(MOD_MMI,"[UCM] @@@@ MENU_ID_UCM_FROM_LOCAL_CALL");
					mmi_ucm_call_launch(0,&g_make_call_para_bt);
            	}
					break;
				case MENU_ID_UCM_FROM_BT_CALL:
				{
					kal_prompt_trace(MOD_MMI,"[UCM] @@@@ MENU_ID_UCM_FROM_BT_CALL");
					mmi_ucm_call_launch_set_para(g_make_call_para_bt.ucs2_num_uri, 
											g_make_call_para_bt.adv_para.after_make_call_callback,
											g_make_call_para_bt.adv_para.after_callback_user_data);
					g_make_call_para_bt.adv_para.after_make_call_callback = NULL;
		...
		case EVT_ID_CUI_MENU_CLOSE_REQUEST:
		{
			if (g_make_call_para_bt.ucs2_num_uri != NULL)
			{
				kal_prompt_trace(MOD_MMI,"[UCM] @@@@ EVT_ID_CUI_MENU_CLOSE_REQUEST, free num");
				OslMfree(g_make_call_para_bt.ucs2_num_uri);
				g_make_call_para_bt.ucs2_num_uri = NULL;
			}
			if (g_make_call_para_bt.adv_para.after_make_call_callback != NULL)
			{
				kal_prompt_trace(MOD_MMI,"[UCM] @@@@ EVT_ID_CUI_MENU_CLOSE_REQUEST, call back");
				make_call_cb_para.result = SRV_UCM_RESULT_CANCEL;
				make_call_cb_para.user_data = g_make_call_para_bt.adv_para.after_callback_user_data;
				g_make_call_para_bt.adv_para.after_make_call_callback(&make_call_cb_para);
				g_make_call_para_bt.adv_para.after_make_call_callback = NULL;
				if (g_ucm_p->mo_info.callback != NULL)
				{
					g_ucm_p->mo_info.callback = NULL;
			...
			if (!is_menu_selected)
			{
				kal_prompt_trace(MOD_MMI,"[UCM] @@@@ EVT_ID_CUI_MENU_CLOSE_REQUEST, GRP_ID_UCM_MO_SELECT=%d",is_menu_selected);
				mmi_frm_group_close(GRP_ID_UCM_MO_SELECT);
			}
		}
			break;
...
void mmi_ucm_entry_in_call(mmi_scrn_essential_struct* scr_info)
{
  ...
  if (group_info.call_state == SRV_UCM_ACTIVE_STATE)
...
#endif /* __MMI_UCM_SINGLE_CALL_SCREEN_ENHANCE__ */
    {
		...
		if(g_ucm_p->call_misc.reset_hilite)
		{
			g_ucm_p->call_misc.hilite_index = 0;
			mmi_ucm_set_hilight_index(g_ucm_p->call_misc.hilite_tab, g_ucm_p->call_misc.hilite_index);
			gui_buffer = NULL;
		}
		g_ucm_p->call_misc.reset_hilite = MMI_FALSE;
...
#if (defined( __MMI_BT_DIALER_SUPPORT__) && defined(__MMI_TELEPHONY_SUPPORT__))
	is_show_call_select_srcn = MMI_TRUE;
#endif /* __MMI_BT_DIALER_SUPPORT__ && __MMI_TELEPHONY_SUPPORT__*/
...
#if (defined(__MMI_BT_DIALER_SUPPORT__) && defined(__MMI_TELEPHONY_SUPPORT__))
mmi_ret mmi_ucm_call_select_sg_proc(mmi_event_struct *evt)
{
	mmi_ucm_after_make_call_cb_struct make_call_cb_para;
	...
	switch(evt->evt_id)
	{
		...
		case EVT_ID_CUI_MENU_CLOSE_REQUEST:
		{
			mmi_ucm_call_select_menu_proc(evt);
		}
		break;
        case EVT_ID_GROUP_DEINIT:
        {
			if (g_make_call_para_bt.ucs2_num_uri != NULL)
			{
				kal_prompt_trace(MOD_MMI,"[UCM] @@@@ EVT_ID_GROUP_DEINIT, free num");
				OslMfree(g_make_call_para_bt.ucs2_num_uri);
				g_make_call_para_bt.ucs2_num_uri = NULL;
			}
			if (g_make_call_para_bt.adv_para.after_make_call_callback != NULL)
			{
				kal_prompt_trace(MOD_MMI,"[UCM] @@@@ EVT_ID_GROUP_DEINIT, call back");
				make_call_cb_para.result = SRV_UCM_RESULT_CANCEL;
				make_call_cb_para.user_data = g_make_call_para_bt.adv_para.after_callback_user_data;
				g_make_call_para_bt.adv_para.after_make_call_callback(&make_call_cb_para);
				g_make_call_para_bt.adv_para.after_make_call_callback = NULL;
