...
void mmi_bt_rmtcap_preview_req()
{
    ...
    result = srv_bt_noti_send_cmd_ext(g_rmtcap_cmd_header, g_mmi_rmtcap_cntx_p->cmd, cmd_len);
...
#ifdef __MTK_TARGET__
    if (!srv_bt_noti_check_connection())
    {
        S32 list_not_ready;
...
void mmi_btrmtcap_entry_main_message_menu()
{
    ...
    {
        mmi_bt_music_disconnect(mmi_btrmtcap_bt_music_disconn_cb);
        return;
    }
    else
#endif
