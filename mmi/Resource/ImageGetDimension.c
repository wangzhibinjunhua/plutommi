...
*****************************************************************************/
#ifdef __RESOURCE_GEN_ 
#define IMGDIM_TAG       "IMAGEGETDIMENSION"
#define IMGDIM_LOG_D(format, args...) RES_LOG_D((IMGDIM_TAG), (format) , ##args)
#define IMGDIM_LOG_V(format, args...) RES_LOG_V((IMGDIM_TAG), (format) , ##args)
#define IMGDIM_LOG_W(format, args...) RES_LOG_W((IMGDIM_TAG), (format) , ##args)
#define IMGDIM_LOG_E(format, args...) RES_LOG_E((IMGDIM_TAG), (format) , ##args)
#else
#define IMGDIM_LOG_D(format, ...) printf((format), ##__VA_ARGS__)
#define IMGDIM_LOG_V(format, ...) printf((format), ##__VA_ARGS__)
#define IMGDIM_LOG_W(format, ...) printf((format), ##__VA_ARGS__)
#define IMGDIM_LOG_E(format, ...) printf((format), ##__VA_ARGS__)
#endif  //__RESOURCE_GEN_
...
static MMI_BOOL image_bmp_get_dimension(S8 *filename, U32 *width, U32 *height)
{
    ...
    if (fp == NULL)
    {
        ...
        fprintf(stderr, "\t%s\n", strerror(errno));
        IMGDIM_LOG_W("image_bmp_get_dimension: open file failed,\t FILE:%s", filename);

        return MMI_FALSE;
    }
    if (fread(buf, sizeof(char), BMP_INFO_SIZE, fp) != BMP_INFO_SIZE)
    {
        if (strstr(filename, "EMPTYIMAGE.BMP") == NULL)
        {
            fprintf(stderr, "Read file err:\t%s\n", filename);
        }
        IMGDIM_LOG_W("image_bmp_get_dimension: Read file failed, \t FILE:%s", filename);
        MYFCLOSE(fp);
        return MMI_FALSE;
    }
    src = buf;
    ...
    *height = biHeight;
    ...
    if (bfType != 0x4d42)	/* BM */
    {
        fprintf(stderr, "Non-standard file err:\t%s\n", filename);
        IMGDIM_LOG_W("image_bmp_get_dimension: not standard file ( not BMP file) , FILE:\t%s", filename);
        return MMI_FALSE;
    }
    ...
    {
        fprintf(stderr, "Non-standard file err:\t%s\n", filename);
        IMGDIM_LOG_W("image_bmp_get_dimension: not standard file ( core info header, currently not support) , FILE:\t%s", filename);
        return MMI_FALSE;
    }
...
static MMI_BOOL image_gif_get_dimension(S8 *filename, U32 *width, U32 *height)
{
    ...
    if (fp == NULL)
    {
        fprintf(stderr, "Open file err:\t%s\n", filename);
        //fprintf(stderr, "\t%s\n", strerror(errno));
        IMGDIM_LOG_W("image_gif_get_dimension:open file failed,\t FILE: %s", filename);
        return MMI_FALSE;
    }
    if (fread(buf, sizeof(char), GIF_INFO_SIZE, fp) != GIF_INFO_SIZE)
    {
        ...
        IMGDIM_LOG_W("image_gif_get_dimension: Read file failed, \t FILE:%s", filename);
        MYFCLOSE(fp);
        return MMI_FALSE;
    }
    src = buf;
    ...
    if (header != 0x464947) /* 'GIF' */
    {
        *width = 0;
        *height = 0;
        IMGDIM_LOG_W("image_gif_get_dimension: Not GIF File, \t FILE:%s", filename);
        return MMI_FALSE;
    }
...
static MMI_BOOL image_gif_get_frame_count(S8 *filename, U32 *framecount)
{
    ...
    if (fp == NULL)
    {
        fprintf(stderr, "Open file err:\t%s\n", filename);
        fprintf(stderr, "\t%s\n", strerror(errno));
        IMGDIM_LOG_W("image_gif_get_frame_count: open file failed,\t FILE: %s", filename);        
        return MMI_FALSE;
    }
    ...
    if (buf == NULL)
    {
        ...
        IMGDIM_LOG_W("image_gif_get_frame_count: Memory allocation failed,\t FILE: %s, Request %d bytes", filename, file_size*sizeof(U8));        
        MYFCLOSE(fp);
        return MMI_FALSE;
    }
    if (fread(buf, sizeof(char), file_size, fp) != file_size)
    {
        ...
        IMGDIM_LOG_W("image_gif_get_frame_count: Read file failed, \t FILE:%s", filename);
        MYFCLOSE(fp);
        return MMI_FALSE;
    }
    src = buf;
    ...
    *framecount = 0;
    ...
    /* read the GIF file signature */
    {
        ...
        {
            IMGDIM_LOG_W("image_gif_get_frame_count: GIF file signature check failed, \t FILE:%s", filename);
            return MMI_FALSE;
        }
    }
    ...
    /* skip dimension value */
    {
        ...
        {
            IMGDIM_LOG_W("image_gif_get_frame_count: dimension value is not valid: (%d, %d, %d, %d ), \tFILE:%s",img_w, img_h,img_w,img_h,filename);
            return MMI_FALSE;
        }
    }
        ...
        case ',':
            /* gdi_image_gif_dummy_decoder(); */
            {
...

                {
                    ...
                    {
                        IMGDIM_LOG_W("image_gif_get_frame_count: buffer overflow caused by src, \tFILE:%s",filename);
                        return MMI_FALSE;
                    }
                ...
                while (!finished_decoding)
                {
                    ...
                    else
                    {
                        ...
                        while (code >= GIF_new_codes)
                        {
                            ...
                            {
                                IMGDIM_LOG_W("image_gif_get_frame_count: stack_pointer overflow\tFILE:%s",filename);
                                return MMI_FALSE;
                            }
...
static MMI_BOOL image_png_get_dimension(S8 *filename, U32 *width, U32 *height)
{
    ...
    if (fp == NULL)
    {
        fprintf(stderr, "Open file err:\t%s\n", filename);
        fprintf(stderr, "\t%s\n", strerror(errno));
        IMGDIM_LOG_W("image_png_get_dimension: open file failed,\t FILE: %s", filename);
    ...
    if (fread(buf, sizeof(char), PNG_INFO_SIZE, fp) != PNG_INFO_SIZE)
    {
        ...
        IMGDIM_LOG_W("image_png_get_dimension: Read file failed, \t FILE:%s", filename);
    ...
    IMGDIM_LOG_W("image_png_get_dimension: get dimension failed, \t FILE:%s", filename);
    return MMI_FALSE;
}
...
static MMI_BOOL image_jpg_get_dimension(S8 *filename, U32 *width, U32 *height)
{
    ...
    if (fp == NULL)
    {
        fprintf(stderr, "Open file err:\t%s\n", filename);
        fprintf(stderr, "\t%s\n", strerror(errno));
        IMGDIM_LOG_W("image_jpg_get_dimension: open file failed,\t FILE: %s", filename);   
        return MMI_FALSE;
    }
    ...
    {
        IMGDIM_LOG_W("image_jpg_get_dimension: Not Valid JPEG, Header Bytes: (0X%2x, 0X%2x),\t FILE; ",jpg_byte1,jpg_byte2,filename);
        MYFCLOSE(fp);
        return MMI_FALSE;
    }
...
/* under construction !*/
    ...
    {
        IMGDIM_LOG_W("Image_Get_Dimensions: file types not support\t FILE: %s", filename);   
...
*****************************************************************************/
S32 Image_Test(S8 *filename, U32 *width, U32 *height, int disable_enfb)
{
    ...
    *width = *height = 0;
    ...
    /* if vector font, return true */
    if (strncmp(ext, "TTF", 3) == 0)
    {
        extern S32 g_mmi_res_font_flags;

        if (g_mmi_res_font_flags & MMI_RES_FLAG_FONT_IS_ENFB)
        {
            return ENFB_IMAGE_FONT;
        }
        else
        {
            return ENFB_IMAGE_NONE;
