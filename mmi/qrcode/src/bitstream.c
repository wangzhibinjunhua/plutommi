...
{
	BitStream *bstream;
	kal_prompt_trace(MOD_BT,"BitStream_new");

	bstream = (BitStream *)hc_alloc_mem(sizeof(BitStream));
...
{
	unsigned char *data;
	kal_prompt_trace(MOD_BT,"BitStream_allocate");

	if(bstream == NULL) {
		return -1;
	}

	data = (unsigned char *)hc_alloc_mem(length);
	if(data == NULL) {
		return -1;
	}

	if(bstream->data) {
		hc_free_mem(bstream->data);
	}
...
{
	unsigned char *data;
	kal_prompt_trace(MOD_BT,"BitStream_append");
	...
	data = (unsigned char *)hc_alloc_mem(bstream->length + arg->length);
	if(data == NULL) {
		return -1;
	}
...
unsigned char *BitStream_toByte(BitStream *bstream)
{
	...
	data = (unsigned char *)hc_alloc_mem((size + 7) / 8);
	if(data == NULL) {
		return NULL;
	}
