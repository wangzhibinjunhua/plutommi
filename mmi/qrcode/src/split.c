...
static int Split_eat8(const char *string, QRinput *input, QRencodeMode hint)
{
	...
	kal_prompt_trace(MOD_BT,"Split_eat8 l8 %d",l8);
	...
	while(*p != '\0') {
		...
		if(mode == QR_MODE_NUM) {
			...
			p++;
			kal_prompt_trace(MOD_BT,"Split_eat8 ret");
		}
	}

	run = p - string;
	kal_prompt_trace(MOD_BT,"Split_eat8 run %d",run);
	ret = QRinput_append(input, QR_MODE_8, run, (unsigned char *)string);
	kal_prompt_trace(MOD_BT,"Split_eat8 ret %d",ret);
	if(ret < 0) return -1;
		...
		QRencodeMode hint)
{
	...
	if(mode == QR_MODE_NUM) {
		...
		kal_prompt_trace(MOD_BT,"Split_splitString length %d",length);
		...
		QRencodeMode hint, int casesensitive)
{
	char *newstr;
	int ret;
	kal_prompt_trace(MOD_BT,"Split_splitStringToQRinput");
	...
	if(!casesensitive) {
		...
		kal_prompt_trace(MOD_BT,"Split_splitStringToQRinput newstr %s",newstr);
		if(newstr == NULL) 
			return -1;
		ret = Split_splitString(newstr, input, hint);
		kal_prompt_trace(MOD_BT,"Split_splitStringToQRinput Split_splitString");
		hc_free_mem(newstr);
	} else {
