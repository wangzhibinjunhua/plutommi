...
static QRinput_List *QRinput_List_newEntry(QRencodeMode mode, int size, const unsigned char *data)
{
	...
	entry = (QRinput_List *)hc_alloc_mem(sizeof(QRinput_List));
	if(entry == NULL) return NULL;

	entry->mode = mode;
	entry->size = size;
	if(size > 0) {
		entry->data = (unsigned char *)hc_alloc_mem(size);
		if(entry->data == NULL) {
			hc_free_mem(entry);
			return NULL;
		}
...
static QRinput_List *QRinput_List_dup(QRinput_List *entry)
{
	...
	n = (QRinput_List *)hc_alloc_mem(sizeof(QRinput_List));
	if(n == NULL) return NULL;

	n->mode = entry->mode;
	n->size = entry->size;
	n->data = (unsigned char *)hc_alloc_mem(n->size);
	if(n->data == NULL) {
		hc_free_mem(n);
		return NULL;
	}
...
QRinput *QRinput_new2(int version, QRecLevel level)
{
	...
	kal_prompt_trace(MOD_BT,"QRinput_new2 %d",sizeof(QRinput));

	input = (QRinput *)hc_alloc_mem(sizeof(QRinput));

	if(input == NULL)
		return NULL;
	kal_prompt_trace(MOD_BT,"QRinput_new2 input");
...
static int QRinput_convertData(QRinput *input)
{
	int bits;
	int ver;
	kal_prompt_trace(MOD_BT,"QRinput_convertData");
...
static int QRinput_appendPaddingBit(BitStream *bstream, QRinput *input)
{
	...
	int padlen;
	kal_prompt_trace(MOD_BT,"QRinput_appendPaddingBit");
	...
	if(padlen > 0) {
		padbuf = (unsigned char *)hc_alloc_mem(padlen);
		if(padbuf == NULL) {
			ret = -1;
			goto DONE;
		}
...
static int QRinput_insertFNC1Header(QRinput *input)
{
	...
	kal_prompt_trace(MOD_BT,"QRinput_insertFNC1Header");
 ...
 BitStream *QRinput_mergeBitStream(QRinput *input)
{
	...
	QRinput_List *list;
	int ret;
	kal_prompt_trace(MOD_BT,"QRinput_mergeBitStream");
 ...
 BitStream *QRinput_getBitStream(QRinput *input)
{
	BitStream *bstream;
	int ret;
	kal_prompt_trace(MOD_BT,"QRinput_getBitStream");
...
unsigned char *QRinput_getByteStream(QRinput *input)
{
	BitStream *bstream;
	unsigned char *array;
	kal_prompt_trace(MOD_BT,"QRinput_getByteStream");
...
static QRinput_InputList *QRinput_InputList_newEntry(QRinput *input)
{
	...
	entry = (QRinput_InputList *)hc_alloc_mem(sizeof(QRinput_InputList));
	if(entry == NULL) return NULL;
...
QRinput_Struct *QRinput_Struct_new(void)
{
	...
	s = (QRinput_Struct *)hc_alloc_mem(sizeof(QRinput_Struct));
...
static int QRinput_List_shrinkEntry(QRinput_List *entry, int bytes)
{
	...
	data = (unsigned char *)hc_alloc_mem(bytes);
