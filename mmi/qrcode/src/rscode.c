...
//#define NULL ((void *)0)
  ...
  rs = (RS *)hc_alloc_mem(sizeof(RS));//calloc
  if(rs == NULL)
    goto done;
  ...
  rs->alpha_to = (data_t *)hc_alloc_mem(sizeof(data_t)*(rs->nn+1));
  if(rs->alpha_to == NULL){
    hc_free_mem(rs);
    rs = NULL;
    goto done;
  }
  rs->index_of = (data_t *)hc_alloc_mem(sizeof(data_t)*(rs->nn+1));
  if(rs->index_of == NULL){
    hc_free_mem(rs->alpha_to);
    hc_free_mem(rs);
    rs = NULL;
    goto done;
  }
  ...
  rs->genpoly = (data_t *)hc_alloc_mem(sizeof(data_t)*(nroots+1));
  if(rs->genpoly == NULL){
    hc_free_mem(rs->alpha_to);
    hc_free_mem(rs->index_of);
    hc_free_mem(rs);
