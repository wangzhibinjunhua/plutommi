...
static int RSblock_init(RSblock *blocks, int spec[5], unsigned char *data, unsigned char *ecc)
{
	...
	RS *rs;
	int el, dl;
	kal_prompt_trace(MOD_BT,"RSblock_init");
 ...
 QRRawCode *QRraw_new(QRinput *input)
{
	QRRawCode *raw;
	int spec[5], ret;
	kal_prompt_trace(MOD_BT,"QRraw_new");

	raw = (QRRawCode *)hc_alloc_mem(sizeof(QRRawCode));
	...
	raw->ecccode = (unsigned char *)hc_alloc_mem(raw->eccLength);
	if(raw->ecccode == NULL) {
		hc_free_mem(raw->datacode);
		hc_free_mem(raw);
		return NULL;
	}

	raw->blocks = QRspec_rsBlockNum(spec);
	raw->rsblock = (RSblock *)hc_alloc_mem(raw->blocks*sizeof(RSblock));
	if(raw->rsblock == NULL) {
		QRraw_free(raw);
		return NULL;
	}
 ...
 unsigned char QRraw_getCode(QRRawCode *raw)
{
	int col, row;
	unsigned char ret;
	kal_prompt_trace(MOD_BT,"QRraw_getCode");
 ...
 void QRraw_free(QRRawCode *raw)
{
	if(raw != NULL) {
		hc_free_mem(raw->datacode);
		hc_free_mem(raw->ecccode);
		hc_free_mem(raw->rsblock);
		hc_free_mem(raw);
	}
}
 ...
 MQRRawCode *MQRraw_new(QRinput *input)
{
	...
	raw = (MQRRawCode *)hc_alloc_mem(sizeof(MQRRawCode));
	if(raw == NULL) return NULL;
	...
	raw->rsblock = (RSblock *)hc_alloc_mem(sizeof(RSblock));
	if(raw->rsblock == NULL) {
		MQRraw_free(raw);
		return NULL;
	}
...
{
	FrameFiller *filler;
	kal_prompt_trace(MOD_BT,"FrameFiller_new");

	filler = (FrameFiller *)hc_alloc_mem(sizeof(FrameFiller));
...
{
	QRcode *qrcode;
	kal_prompt_trace(MOD_BT,"QRcode_new");

	qrcode = (QRcode *)hc_alloc_mem(sizeof(QRcode));
...
QRcode *QRcode_encodeMask(QRinput *input, int mask)
{
	...
	kal_prompt_trace(MOD_BT,"QRcode_encodeMask");
	...
	if(mask == -2) { // just for debug purpose
		masked = (unsigned char *)hc_alloc_mem(width * width);
...
static QRcode *QRcode_encodeStringReal(const char *string, int version, QRecLevel level, int mqr, QRencodeMode hint, int casesensitive)
{
	...
	kal_prompt_trace(MOD_BT,"Split_splitStringToQRinput ret %d ",ret);
	if(ret < 0) {
		QRinput_free(input);
		return NULL;
	}
	code = QRcode_encodeInput(input);
	kal_prompt_trace(MOD_BT,"Split_splitStringToQRinput ret %s ",code->data);
	QRinput_free(input);
...
static QRcode_List *QRcode_List_newEntry(void)
{
	...
	entry = (QRcode_List *)hc_alloc_mem(sizeof(QRcode_List));
