...
S32 mmi_umms_app_ui_callback_inbox_notification_options_decide(U16 scr_id)
{
	...
	cui_menu_set_item_disabled(mmi_frm_group_get_active_id(),MENU_ID_UMMS_INBOX_OPTIONS_DOWNLOAD,MMI_FALSE);
	...
	if(num_item==0 && (srv_mms_bgsr_get_msg_status(scr_info->msg_id) == SRV_MMS_BGSR_STATUS_DOWNLOADING))
	{
        mmi_frm_unhide_menu_item(MENU_ID_UMMS_INBOX_OPTIONS_DOWNLOAD);
		cui_menu_set_item_disabled(mmi_frm_group_get_active_id(),MENU_ID_UMMS_INBOX_OPTIONS_DOWNLOAD,MMI_TRUE);
	}
#endif 
    return num_item;
