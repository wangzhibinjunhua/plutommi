...
MMI_UMMS_ERROR mmi_umms_app_ui_plug_in_memory_status_pre_func(U16 scr_id)
{
    ...
    if (property_screen == NULL)
    {
		...
		else if (scr_id == SCR_ID_UMMS_MMS_MESSAGE_STATUS_PHONE)
		{
            property_screen->scrDB->EntryFunction = mmi_umms_app_if_show_phone_memory_status_screen;
            property_screen->scrDB->RObj->title_str_id = STR_ID_UMMS_MMS_PREFER_STORAGE_PHONE;
            property_screen->scrDB->RObj->menu_item_id = MENU_ID_UMMS_MMS_SETTINGS_MEMORY_STATUS_PHONE;
        }
    #else
        property_screen->scrDB->EntryFunction = mmi_umms_app_if_show_memory_status_screen;
        property_screen->scrDB->RObj->menu_item_id = MENU_ID_UMMS_MMS_SETTINGS_MEMORY_STATUS;
        property_screen->scrDB->RObj->title_str_id = STR_GLOBAL_MEMORY_STATUS;      /* String id of title */
    #endif /* __MMI_MMS_MEMORY_CARD_STORAGE_SUPPORT__ */ 
    #endif /* __MMI_MMS_SIMPLIFIED_UI_SUPPORT__ */ 
