...
*****************************************************************************/
	...
	}mmi_restore_confirm_user_data_struct;

static mmi_restore_confirm_user_data_struct g_mmi_restore_confirm_user_data; 	
#endif
...
static void mmi_restore_confirm_user_data_init(mmi_restore_confirm_user_data_struct * arg)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
static mmi_ret mmi_restore_comfirm_proc(mmi_event_struct *evt)
{
    switch (evt->evt_id)
    {
        case EVT_ID_ALERT_QUIT:
        {
            ...
            mmi_restore_confirm_user_data_struct * confirm_data = 
                (mmi_restore_confirm_user_data_struct *)alert->user_tag;
...
static void mmi_restore_entry_op_delete_confirm(MMI_ID group_id)
{
	 mmi_confirm_property_struct arg;

	 mmi_confirm_property_init(&arg, CNFM_TYPE_OKCANCEL);
     arg.parent_id = group_id;
     arg.callback = (mmi_proc_func)mmi_restore_comfirm_proc;
	 arg.softkey[0].str = (WCHAR *)((UI_string_type) GetString(STR_ID_RESET_ICON_STRING));
	 arg.f_enter_history = 1;
     mmi_restore_confirm_user_data_init(&g_mmi_restore_confirm_user_data);
     g_mmi_restore_confirm_user_data.sg_id = group_id;
     g_mmi_restore_confirm_user_data.LSK_function = RstStartRestore;
	 //g_mmi_restore_confirm_user_data.RSK_function = GoBackHistory;

     arg.user_tag = (void *)&g_mmi_restore_confirm_user_data;

     mmi_confirm_display(
            (WCHAR *) get_string(STR_ID_RESTORE_CONFIRM),
            MMI_EVENT_QUERY,
            &arg);
}

static mmi_ret mmi_entry_restore_screen_proc(mmi_event_struct *param)
{
    return MMI_RET_OK;    
}
/*****************************************************************************
...
static void mmi_restore_entry_restore_screen(void)
{
    ...
    mmi_restore_entry_op_delete_confirm(GRP_ID_PHNSET_RESTORE);
...
static void EntryRstRestoreProcessing(void)
{
    ...
    ShowCategory66Screen(
        ...
        0,
        0,
	   	(PU8) GetString(STR_ID_RESET_CONFIRM),
