...
{
    g_sms_bt_ui_p->is_on_request = MMI_FALSE;
    g_sms_bt_ui_p->need_refresh[MMI_SMS_BT_BOX_INBOX] = MMI_FALSE;
    if(data->result == MMI_TRUE)
    {
        mmi_sms_bt_entry_folder_userdata *userdata = data->user_data;        
        g_sms_bt_ui_p->last_entered = SRV_SMS_BTMAPC_BOX_INBOX;
        if((((*g_sms_um_ui_p).cc_cntx).grp_table[0].grp_id != 0) && 
            g_sms_bt_ui_p->curr_entered == SRV_SMS_BTMAPC_BOX_INBOX)
        {
            mmi_sms_um_bt_ui_cc_scrn_cntx_struct *scrn_cntx;
            ...
            mmi_sms_um_bt_ui_sh_main_msg_entry_folder(SRV_UM_MSG_BTMAPC_BOX_INBOX, userdata->scrn_id);
                    ...
                    g_sms_bt_ui_p->curr_entered != SRV_SMS_BTMAPC_BOX_NONE)
        {
            SrvSmsCallbackFunc func = mmi_sms_bt_get_callback_hdlr(g_sms_bt_ui_p->curr_entered);
            g_sms_bt_ui_p->is_on_request = MMI_TRUE;
            g_sms_bt_ui_p->sms_handle = srv_sms_btmapc_get_msg_list(MMI_TRUE, g_sms_bt_ui_p->curr_entered, func, (void *)&g_ef_usdata);            
        }
    }
    else
    {        
...
{
    g_sms_bt_ui_p->is_on_request = MMI_FALSE;
    g_sms_bt_ui_p->need_refresh[MMI_SMS_BT_BOX_OUTBOX] = MMI_FALSE;
    if(data->result == MMI_TRUE)
    {
        mmi_sms_bt_entry_folder_userdata *userdata = data->user_data;
        kal_prompt_trace(MOD_MMI, "Akhil result TRUE");        
        g_sms_bt_ui_p->last_entered = SRV_SMS_BTMAPC_BOX_OUTBOX;
        if((((*g_sms_um_ui_p).cc_cntx).grp_table[0].grp_id != 0) &&
...
{
    g_sms_bt_ui_p->is_on_request = MMI_FALSE;
    g_sms_bt_ui_p->need_refresh[MMI_SMS_BT_BOX_UNSENT] = MMI_FALSE;
    if(data->result == MMI_TRUE)
    {
        mmi_sms_bt_entry_folder_userdata *userdata = data->user_data;       
        g_sms_bt_ui_p->last_entered = SRV_SMS_BTMAPC_BOX_UNSENT;
        if((((*g_sms_um_ui_p).cc_cntx).grp_table[0].grp_id != 0) &&
            (g_sms_bt_ui_p->curr_entered == SRV_SMS_BTMAPC_BOX_UNSENT))
        {
            mmi_sms_um_bt_ui_cc_scrn_cntx_struct *scrn_cntx;
...
{
    g_sms_bt_ui_p->is_on_request = MMI_FALSE;
    g_sms_bt_ui_p->need_refresh[MMI_SMS_BT_BOX_DRAFTS] = MMI_FALSE;
    if(data->result == MMI_TRUE)
    {
        mmi_sms_bt_entry_folder_userdata *userdata = data->user_data;
        kal_wap_trace(MOD_MMI_COMMON_APP,TRACE_GROUP_6,"Akhil result TRUE");        
        g_sms_bt_ui_p->last_entered = SRV_SMS_BTMAPC_BOX_DRAFTS;
        if((((*g_sms_um_ui_p).cc_cntx).grp_table[0].grp_id != 0) &&
            (g_sms_bt_ui_p->curr_entered == SRV_SMS_BTMAPC_BOX_DRAFTS))
        {
            mmi_sms_um_bt_ui_cc_scrn_cntx_struct *scrn_cntx;
...
void mmi_sms_um_bt_ui_sh_main_msg_entry_inbox(void)
{
    ...
    g_sms_bt_ui_p->is_bt_connected = srv_bt_mapc_adp_is_connected();
        
    #ifndef __MTK_TARGET__
    g_sms_bt_ui_p->is_bt_connected = MMI_TRUE;
    #endif
    if (!g_sms_bt_ui_p->is_bt_connected)
    {
    ...
    if (g_sms_bt_ui_p->need_refresh[MMI_SMS_BT_BOX_INBOX] == MMI_TRUE ||
        g_sms_bt_ui_p->last_entered != SRV_SMS_BTMAPC_BOX_INBOX ||
        g_sms_bt_ui_p->is_on_request == MMI_TRUE)
    {
        if (g_sms_um_ui_main_menu_scrn_id == 0)
        ...
        if (!g_sms_bt_ui_p->is_on_request)
        {
            g_sms_bt_ui_p->is_on_request = MMI_TRUE;
            g_sms_bt_ui_p->sms_handle = srv_sms_btmapc_get_msg_list(MMI_TRUE, SRV_SMS_BTMAPC_BOX_INBOX, mmi_sms_bt_get_inbox_list_cb, (void *)userdata);            
        }
        mmi_sms_um_bt_ui_ut_send_init_event(scrn_cntx->child_scrn_id);
    }
    else
    {
        ...
        //scrn_cntx->child_scrn_id = mmi_sms_um_bt_ui_if_entry_processing(scrn_cntx->scrn_id, scrn_cntx->group_id, MMI_FALSE);
        g_sms_bt_ui_p->sms_handle = srv_sms_btmapc_get_msg_list(MMI_FALSE, SRV_SMS_BTMAPC_BOX_INBOX, mmi_sms_bt_get_inbox_list_cb, (void *)userdata);
    }    
...
void mmi_sms_um_bt_ui_sh_main_msg_entry_draft(void)
{
    ...
    if (1)
    {
        ...
        {
            g_sms_bt_ui_p->is_on_request = MMI_TRUE;
            g_sms_bt_ui_p->sms_handle = srv_sms_btmapc_get_msg_list(MMI_TRUE, SRV_SMS_BTMAPC_BOX_DRAFTS, mmi_sms_bt_get_drafts_list_cb, (void *)userdata);            
        }
        mmi_sms_um_bt_ui_ut_send_init_event(scrn_cntx->child_scrn_id);
    }
    else
    {
        g_sms_bt_ui_p->sms_handle = srv_sms_btmapc_get_msg_list(MMI_FALSE, SRV_SMS_BTMAPC_BOX_DRAFTS, mmi_sms_bt_get_drafts_list_cb, (void *)userdata);
    }
...
void mmi_sms_um_bt_ui_sh_main_msg_entry_outbox(void)
{
    ...
    if (1)
    {
        ...
        #endif
            g_sms_bt_ui_p->is_on_request = MMI_TRUE;
            g_sms_bt_ui_p->sms_handle = srv_sms_btmapc_get_msg_list(MMI_TRUE, SRV_SMS_BTMAPC_BOX_UNSENT, mmi_sms_bt_get_unsent_list_cb, (void *)userdata);            
        }
        mmi_sms_um_bt_ui_ut_send_init_event(scrn_cntx->child_scrn_id);
    }
    else
    {
        g_sms_bt_ui_p->sms_handle = srv_sms_btmapc_get_msg_list(MMI_FALSE, SRV_SMS_BTMAPC_BOX_UNSENT, mmi_sms_bt_get_unsent_list_cb, (void *)userdata);
    }
...
void mmi_sms_um_bt_ui_sh_main_msg_entry_sent(void)
{
    ...
    if (1)
    {
        ...
        {
            g_sms_bt_ui_p->is_on_request = MMI_TRUE;
            g_sms_bt_ui_p->sms_handle = srv_sms_btmapc_get_msg_list(MMI_TRUE, SRV_SMS_BTMAPC_BOX_OUTBOX, mmi_sms_bt_get_outbox_list_cb, (void *)userdata);           
        }
        mmi_sms_um_bt_ui_ut_send_init_event(scrn_cntx->child_scrn_id);
    }
    else
    {
        g_sms_bt_ui_p->sms_handle = srv_sms_btmapc_get_msg_list(MMI_FALSE, SRV_SMS_BTMAPC_BOX_OUTBOX, mmi_sms_bt_get_outbox_list_cb, (void *)userdata);
    }
...
void mmi_sms_um_bt_ui_sh_general_folder_tab_entry(mmi_scrn_essential_struct* scr_ess)
{
    ...
    if (g_sms_bt_ui_p->mmi_need_update_list)
    {        
    ...
    if (scrn_cntx->screen_hdlr.is_scrn_displayed == MMI_FALSE || g_sms_bt_ui_p->mmi_need_update_list)
    {
        ...
        #endif
        g_sms_bt_ui_p->mmi_need_update_list = MMI_FALSE;
        mmi_sms_um_bt_ui_cc_send_event(scrn_cntx->scrn_id, MMI_UM_UI_EVENT_CHECK, NULL);
    #ifdef __MMI_UM_CONVERSATION_BOX_ENHANCE__
        if (MMI_UM_FOLDER_MODE_THREAD_MSG == mmi_sms_um_bt_ui_fsm_ef_get_folder_mode(&scrn_cntx->fsm))
        {
    ...
    {
        if (g_sms_bt_ui_p->need_refresh[mmi_box_type] && 
            g_sms_bt_ui_p->is_bt_connected &&
            !g_sms_bt_ui_p->is_on_request)
        {   
            mmi_sms_bt_entry_folder_userdata *userdata;
            ...
            scrn_cntx->child_scrn_id = mmi_sms_um_bt_ui_if_entry_processing(scrn_cntx->scrn_id, scrn_cntx->group_id, MMI_FALSE);
            g_sms_bt_ui_p->is_on_request = MMI_TRUE;
            g_sms_bt_ui_p->sms_handle = srv_sms_btmapc_get_msg_list(MMI_TRUE,
...
S32 mmi_sms_um_bt_ui_sh_get_async_item(S32 start_index, gui_iconlist_menu_item *menu_data, S32 num_of_item)
{
    ...
    if (result == MMI_FALSE)
    {
        ...
        if (scrn_cntx->screen_hdlr.is_scrn_displayed == MMI_TRUE || 
            (g_sms_um_ui_main_menu_scrn_id == 0 && g_sms_bt_ui_p->mmi_need_update_list))
        {
