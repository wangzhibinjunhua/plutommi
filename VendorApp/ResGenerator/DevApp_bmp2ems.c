...
static U8 EMS_load(bytestream *file, char *filename)
{
    ...
    sprintf(cmdbuf, "%s %s %s", "..\\..\\Customer\\ResGenerator\\convert.exe", filename, ".\\temp\\temp.rgb");
    system(cmdbuf);
    file_ptr = fopen(".\\temp\\temp.rgb", "rb");
    if (file_ptr == NULL)
    {
...
int bmp2ems(char *in_filename, char *out_filename, int binary_flag, int verbose_flag)
{
    ...
    if (binary_flag == 1)
    {
        ...
        while (!out_file)
        {
            //printf("\nErr opening output file %s. Dumping image data to console ...", out_filename);
            Sleep(50);
            out_file = fopen(out_filename, "ab");
        }
        #if 0
